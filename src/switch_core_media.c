...
#ifndef SWITCH_MEDIA_EXTENSION_MID
#define SWITCH_MEDIA_EXTENSION_MID  0x0F
#endif
...
#define SWITCH_SDP_MID_MAX_LEN 255

typedef struct {
	uint8_t send_ext_id;
	uint8_t recv_ext_id;
	char mid[SWITCH_SDP_MID_MAX_LEN + 1];
} switch_mid_negotiation_t;
...
struct switch_rtp_engine_s {
	...
	switch_rtp_t *unbundled_rtp_session;
	switch_rtp_write_state_t *bundle_write_state;
	switch_mid_negotiation_t pending_mid;
	uint8_t active_mid_send_ext_id;
	char active_mid[SWITCH_RTP_MID_MAX_LEN + 1];
...
/* This gate keys on the first VP8 payload descriptor start packet
...
static switch_bool_t vp8_frame_is_keyframe(switch_frame_t *frame)
{
...
static switch_bool_t engine_uses_vp8(switch_rtp_engine_t *engine)
{
...
static void pre_dtls_track_deferred_write(switch_core_session_t *session, switch_rtp_engine_t *engine, switch_frame_t *frame, const char *path)
{
	if (!engine->pre_dtls_buf) {
		...
		/* Re-arm after an ICE candidate switch, DTLS re-handshake, or another
		 * post-readiness write that produced no packet. */
...
struct switch_media_handle_s {
	...
	smh_flag_t flags;
	switch_bundle_group_t bundle;
	switch_bundle_group_t pending_bundle;
...
#define TRICKLE_EVENT_SUBCLASS "sofia::trickle-ice"
...
SWITCH_DECLARE(switch_status_t) switch_core_media_trickle_remote_candidate(switch_core_session_t *session, const char *mid,int mline_index, const char *cand_line, int end_of_candidates)
{
	...
	if (!session || (!end_of_candidates && (!cand_line || !*cand_line))) {
	...
	if (end_of_candidates) {
		/* Handle end of candidates case - no candidate line processing needed */
		switch_core_media_trickle_end_of_candidates(session, mline_index, mid);
		st = SWITCH_STATUS_SUCCESS;
	} else {
		/* Process the candidate line */
		if (strncasecmp(cand_line, "a=candidate:", 12) == 0) {
			/* ok, nothing to do */
		} else if (strncasecmp(cand_line, "candidate:", 10) == 0) {
			char buf[1200];
			switch_snprintf(buf, sizeof(buf), "a=%s", cand_line);
			cand_line = switch_core_strdup(switch_core_session_get_pool(session), buf);
		} else {
			char buf[1200];
			switch_snprintf(buf, sizeof(buf), "a=candidate:%s", cand_line);
			cand_line = switch_core_strdup(switch_core_session_get_pool(session), buf);
		}

		st = switch_core_media_trickle_add_remote_candidate(session, mline_index, mid, cand_line);
		if (st == SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
							  "%s Added remote trickled ICE candidate (mid=%s, m=%d)\n",
							  channel ? switch_channel_get_name(channel) : "unknown",
							  mid ? mid : "(null)", mline_index);
		} else {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
							  "%s Failed to add remote trickled ICE candidate (mid=%s, m=%d)\n", channel ? switch_channel_get_name(channel) : "unknown",
							  mid ? mid : "(null)", mline_index);
			return st;
		}
	}
...
SWITCH_DECLARE(switch_status_t) switch_core_media_trickle_ingest_sdpfrag(switch_core_session_t *session, const char *frag_text)
{
	const char *p, *line;
	char mid[64] = {0};
	int mline_index = -1,  saw_candidate = 0, eoc = 0;
	switch_status_t overall = SWITCH_STATUS_FALSE;

	if (!session || !frag_text) {
		return SWITCH_STATUS_FALSE;
	}
	...
	while (p && *p) {
		...
		if (!strncasecmp(line, "a=mid:", 6)) {
			switch_copy_string(mid, line + 6, sizeof(mid));
		} else if (!strncasecmp(line, "a=end-of-candidates", 20)) {
			eoc = 1;
		} else if (!strncasecmp(line, "a=candidate:", 12)) {
			if (switch_core_media_trickle_remote_candidate(session, mid[0] ? mid : NULL,
			                                              mline_index, line, 0) == SWITCH_STATUS_SUCCESS) {
				overall = SWITCH_STATUS_SUCCESS;
				saw_candidate = 1;
			}
		}
	...
	if (!saw_candidate && eoc) {
		switch_core_media_trickle_remote_candidate(session, mid[0] ? mid : NULL, mline_index, "candidate:", 1);
		overall = SWITCH_STATUS_SUCCESS;
	}
																	   ...
																	   switch_core_session_t *other_session, switch_t38_options_t *t38_options)
{
	...
	if (remote_host && remote_port && !strcmp(remote_host, a_engine->cur_payload_map->remote_sdp_ip) &&
		...
		} else if (switch_rtp_set_remote_address(a_engine->rtp_session, a_engine->cur_payload_map->remote_sdp_ip,
...
static void switch_core_session_get_recovery_crypto_key(switch_core_session_t *session, switch_media_type_t type)
{
	...
	switch_rtp_engine_t *engine;
	char *keyvar, *tagvar, *ctypevar, *lastkeyvar;
	...
	if (type == SWITCH_MEDIA_TYPE_AUDIO) {
		...
		lastkeyvar = "rtp_last_audio_local_crypto_key";
		...
		lastkeyvar = "rtp_last_video_local_crypto_key";
		...
		lastkeyvar = "rtp_last_text_local_crypto_key";
	} else return;
...
static void switch_core_session_apply_crypto(switch_core_session_t *session, switch_media_type_t type)
{
	...
	if (engine->ssec[engine->crypto_type].remote_crypto_key && switch_channel_test_flag(session->channel, CF_SECURE)) {
		...
		switch_rtp_add_crypto_key(engine->rtp_session, SWITCH_RTP_CRYPTO_SEND, engine->ssec[engine->crypto_type].crypto_tag, &engine->ssec[engine->crypto_type]);
...
static void switch_core_session_parse_crypto_prefs(switch_core_session_t *session)
{
	...
	} else if (smh->crypto_mode != CRYPTO_MODE_FORBIDDEN && !zstr(suites)) {
																...
																switch_sdp_type_t sdp_type)
{
	...
	if (!(smh = session->media_handle) || switch_channel_test_flag(session->channel, CF_AVPF)) {
		return 0;
	}
	...
	if (engine->ssec[engine->crypto_type].remote_crypto_key && switch_rtp_ready(engine->rtp_session)) {
		...
		if (crypto && engine->crypto_type != CRYPTO_INVALID
				&& !strcmp(crypto, engine->ssec[engine->crypto_type].remote_crypto_key)) {
			got_crypto++;
		} else {
			const char *a = switch_stristr("AE", engine->ssec[engine->crypto_type].remote_crypto_key);
			const char *b = switch_stristr("AE", crypto);

			if (sdp_type == SDP_TYPE_REQUEST && !vval) {
				goto end;
			}
	...
	for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {
		...
		if (use_alias || switch_stristr(SUITES[j].name, crypto)) {
			...
			if (sdp_type == SDP_TYPE_REQUEST) {
				engine->ssec[j].remote_crypto_tag = crypto_tag;
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Saved remote crypto_tag %d for suite %s\n", crypto_tag, vval);
			}
			break;
		}
...
SWITCH_DECLARE(void) switch_core_session_check_outgoing_crypto(switch_core_session_t *session, int sdp_type)
{
	...
	switch_media_handle_t *smh;
	int i = 0, idx = 0, ctype = 0;
	switch_rtp_engine_t *audio_engine = NULL, *video_engine = NULL, *text_engine = NULL;
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "CRYPTO: Checking outgoing crypto (%s)\n", sdp_type == SDP_TYPE_RESPONSE ? "SDP_TYPE_RESPONSE" : "SDP_TYPE_REQUEST");

	audio_engine = &smh->engines[SWITCH_MEDIA_TYPE_AUDIO];
	video_engine = &smh->engines[SWITCH_MEDIA_TYPE_VIDEO];
	text_engine = &smh->engines[SWITCH_MEDIA_TYPE_TEXT];

	for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {

		ctype = smh->crypto_suite_order[i];
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "CRYPTO: Checking outgoing crypto: %s (i=%d)\n", SUITES[ctype].name, i);

		idx = sdp_type == SDP_TYPE_RESPONSE ? audio_engine->ssec[ctype].remote_crypto_tag : i + 1;

		if (sdp_type == SDP_TYPE_RESPONSE && idx > 0) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "CRYPTO: Checking outgoing crypto: using restored remote crypto tag (i=%d, idx=%d) for suite %s\n", i, idx, SUITES[ctype].name);
		}

		if (idx > 0) {
			if (SWITCH_STATUS_SUCCESS == switch_core_media_build_crypto(session->media_handle, SWITCH_MEDIA_TYPE_AUDIO, idx, ctype, SWITCH_RTP_CRYPTO_SEND, 0, 0)) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "CRYPTO: Built outgoing crypto audio (i=%d, at index=%d)\n", i, idx);
			}
		}

		idx = sdp_type == SDP_TYPE_RESPONSE ? video_engine->ssec[ctype].remote_crypto_tag : i + 1;

		if (idx > 0) {
			if (SWITCH_STATUS_SUCCESS == switch_core_media_build_crypto(session->media_handle, SWITCH_MEDIA_TYPE_VIDEO, idx, ctype, SWITCH_RTP_CRYPTO_SEND, 0, 0)) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "CRYPTO: Built outgoing crypto video (i=%d, at index=%d)\n", i, idx);
			}
		}

		idx = sdp_type == SDP_TYPE_RESPONSE ? text_engine->ssec[ctype].remote_crypto_tag : i + 1;

		if (idx > 0) {
			if (SWITCH_STATUS_SUCCESS == switch_core_media_build_crypto(session->media_handle, SWITCH_MEDIA_TYPE_TEXT, idx, ctype, SWITCH_RTP_CRYPTO_SEND, 0, 0)) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "CRYPTO: Built outgoing crypto text (i=%d, at index=%d)\n", i, idx);
			}
		}
	}
}
...
static switch_bool_t switch_core_media_bundle_negotiated(switch_media_handle_t *smh);
...
static void merge_codec_string(switch_core_session_t *session, const char *preferred_codecs, const char *codecs, bool order_by_rate, char **codec_lists)
{
	...
	if (order_by_rate && pref_codecs_count > 0 && codecs_count > 0) {
		...
		if ((codec_array_count = switch_loadable_module_get_codecs_sorted(codec_array, fmtp, SWITCH_MAX_CODECS, tmp_codecs, codecs_count)) <= 0) {
			goto end;
		}

		// If priority codec is the same as the first codec in the list, we can just return the current codec order
		if (!strcasecmp(priority_codec_array[0]->iananame, codec_array[0]->iananame)) {
			goto end;
		}
		...
		// Start looking for closest sampling rate
		for (i = 0; i < codec_array_count; ++i) {
			uint32_t priority_rate = priority_codec_array[0]->actual_samples_per_second;
			uint32_t current_rate = codec_array[i]->actual_samples_per_second;
			uint32_t closest_rate = closest_rate_index >= 0 ? codec_array[closest_rate_index]->actual_samples_per_second : 0;
			
			if (current_rate == priority_rate) {
				closest_rate_index = i;
				break;
			} else if (priority_rate >= 16000) {
				if (current_rate >= 16000 && closest_rate_index < 0) {
					closest_rate_index = i;
				} else if (current_rate >= 16000 && closest_rate_index >= 0) {
					if (current_rate < closest_rate) {
						closest_rate_index = i;
					}
				}
			} else {
		...
		if (closest_rate_index > 0) {
			// Shift the found codec to the top of the list
			char *tmp = tmp_codecs[closest_rate_index];
			for (i = closest_rate_index; i > 0; --i) {
				tmp_codecs[i] = tmp_codecs[i-1];
			}
			tmp_codecs[0] = tmp;
			k++; // Increment to inform that there are reorder of codecs
		}
	}
...
static void prepare_merge_codec_string(switch_core_session_t *session, const char *preferred_codecs, const char *codecs, char **codec_lists)
{
	const char *val = NULL;
	switch_media_handle_t *smh = NULL;
	switch_bool_t merge_by_order = SWITCH_FALSE;
	switch_bool_t merge_by_sampling_rate = SWITCH_FALSE;
	...
	if (!zstr(val)) {
		if (switch_true(val) || !strcasecmp(val, "order")) {
			merge_by_order = SWITCH_TRUE;
		} else if (!strcasecmp(val, "sampling-rate")) {
			merge_by_sampling_rate = SWITCH_TRUE;
		} else {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING, "Invalid value for merge_inbound_outbound_codecs: %s\n", val);
		}
	} else {
		if (smh->media_flags[SCMF_MERGE_INBOUND_OUTBOUND_CODEC_BY_ORDER]) {
			merge_by_order = SWITCH_TRUE;
		} else if (smh->media_flags[SCMF_MERGE_INBOUND_OUTBOUND_CODEC_BY_SAMPLING_RATE]) {
			merge_by_sampling_rate = SWITCH_TRUE;
		}
	}

	if (merge_by_order || merge_by_sampling_rate) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Merging preferred codec (%s) to target codec (%s) by %s\n"
			, preferred_codecs, codecs, merge_by_sampling_rate ? "sampling-rate" : "order");
		merge_codec_string(session, preferred_codecs, codecs, merge_by_sampling_rate, codec_lists);
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Merge codec final result from (%s) to (%s)\n", codecs, (*codec_lists ? *codec_lists : codecs));
	} else {
		*codec_lists = NULL;
	}
}
...
SWITCH_DECLARE(int) switch_core_media_retrieve_codecs(switch_core_session_t *session, const switch_codec_implementation_t **array)
{
 ...
 ready:
	...
	if (ocodec) {
		...
		prepare_merge_codec_string(session, ocodec, codec_string, &merge_codec);
		if (merge_codec) {
...
/* BUNDLE drain thread states */
#define BUNDLE_DRAIN_INACTIVE   0
#define BUNDLE_DRAIN_STARTING   1
#define BUNDLE_DRAIN_RUNNING    2
#define BUNDLE_DRAIN_STOPPING   3
...
static switch_status_t bundle_frame_dup(switch_frame_t *orig, switch_frame_t **clone)
{
...
static void bundle_audio_queue_cleanup_locked(switch_core_session_t *session, switch_media_handle_t *smh, switch_rtp_engine_t *v_engine)
{
	...
	if (v_engine->bundle_audio_read_consumers) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
			"BUNDLE audio queue cleanup continuing with %u active consumers\n",
	...
	bundle_audio_queue_flush(v_engine->bundle_audio_read_fb);
}

/* Continuous drain thread for BUNDLE socket.
 ...
 * Video packets are cloned and queued to v_engine->read_fb.
 *
...
static void *SWITCH_THREAD_FUNC bundle_drain_thread_func(switch_thread_t *thread, void *obj)
{
	...
	switch_media_handle_t *smh;
	switch_rtp_engine_t *a_engine, *v_engine;
	switch_frame_t drain_frame;
	uint8_t drain_buf[SWITCH_RTP_MAX_BUF_LEN];

	if (!session || !(smh = session->media_handle)) {
		return NULL;
	}
	...
	while (v_engine->bundle_drain_state == BUNDLE_DRAIN_RUNNING
		   ...
		   && smh->media_flags[SCMF_RUNNING]
		   && switch_rtp_ready(a_engine->rtp_session)
		   && v_engine->bundled_with_audio) {

		switch_status_t st;
		switch_bundle_mline_t *mline;
		switch_media_type_t demux_type = SWITCH_MEDIA_TYPE_TOTAL;
		char mid[SWITCH_RTP_MID_MAX_LEN + 1] = "";

		/* Zero out the local frame for each read */
		memset(&drain_frame, 0, sizeof(drain_frame));
		drain_frame.data = drain_buf;
		drain_frame.buflen = sizeof(drain_buf);
		drain_frame.datalen = 0;

		/* Blocking single-packet read. This is the sole reader. */
		st = switch_rtp_zerocopy_read_frame(a_engine->rtp_session, &drain_frame,
										   SWITCH_IO_FLAG_SINGLE_READ | SWITCH_IO_FLAG_BUNDLE_DRAIN);

		if (v_engine->bundle_drain_state != BUNDLE_DRAIN_RUNNING) {
			break;
		}

		if (st != SWITCH_STATUS_SUCCESS || drain_frame.datalen == 0) {
		...
		if (!switch_test_flag(&drain_frame, SFF_RAW_RTP)) {
			continue;
		}

		if (!drain_frame.buflen) {
			drain_frame.buflen = SWITCH_RTP_MAX_BUF_LEN;
		}

		/* Hold the negotiated BUNDLE generation while snapshotting its RTP MID. */
		switch_mutex_lock(smh->sdp_mutex);
		switch_rtp_copy_received_mid(a_engine->rtp_session, mid, sizeof(mid));
		mline = switch_bundle_group_demux_rtp(&smh->bundle, mid, drain_frame.ssrc,
											 drain_frame.payload, NULL);
		if (mline) demux_type = mline->media_type;
		switch_mutex_unlock(smh->sdp_mutex);

		if (demux_type == SWITCH_MEDIA_TYPE_TOTAL) {
			continue;
		}
...
/* Start the BUNDLE drain thread.
 * Caller must hold smh->bundle_drain_mutex or be in single-threaded init. */
static void bundle_drain_thread_start(switch_core_session_t *session)
{
	...
	/* Preconditions */
	if (!v_engine->bundled_with_audio || !a_engine->rtp_session || !v_engine->read_fb) {
		return;
	}
	...
	/* Create audio read frame buffer if not yet present.
	 * Audio queue depth 200: ~4 seconds at 20ms ptime, enough to absorb bursts. */
	if (!v_engine->bundle_audio_read_fb) {
		if (switch_frame_buffer_create(&v_engine->bundle_audio_read_fb, 200) != SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
				"BUNDLE failed to create audio drain frame buffer\n");
			return;
		}
	}
	...
	if (switch_thread_create(&v_engine->bundle_drain_thread, thd_attr,
							  bundle_drain_thread_func, session, pool) != SWITCH_STATUS_SUCCESS) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
			"BUNDLE failed to create drain thread\n");
		v_engine->bundle_drain_state = BUNDLE_DRAIN_INACTIVE;
...
/* Stop the BUNDLE drain thread.
 * Safe to call even if thread is not running. Does NOT hold bundle_drain_mutex during join. */
static void bundle_drain_thread_stop(switch_core_session_t *session)
{
	switch_media_handle_t *smh = session->media_handle;
	switch_rtp_engine_t *a_engine, *v_engine;
	switch_thread_t *thd;
	...
	/* Wake the blocking RTP read so the thread notices STOPPING.
	 * Do NOT hold any mutex during this. */
	if (switch_rtp_ready(a_engine->rtp_session)) {
...
static switch_bool_t switch_core_media_route_bundled_rtp(switch_media_handle_t *smh, switch_rtp_engine_t *engine, switch_media_type_t type)
{
	...
	if (!smh || !engine || type != SWITCH_MEDIA_TYPE_AUDIO) {
		return SWITCH_FALSE;
	}
	...
	switch_rtp_copy_received_mid(engine->rtp_session, mid, sizeof(mid));
...
#define SWITCH_BUNDLE_AUDIO_DRAIN_MAX 128

/* Drain remaining socket packets after a video packet was routed.
 * Uses NOBLOCK reads so we never block on the timer.
 * Video packets are cloned and enqueued to the video frame buffer.
 * Audio packets are absorbed into the JB by rtp_common_read internally.
 * Returns SWITCH_TRUE if an audio frame was extracted from JB during drain,
 * in which case engine->read_frame contains the audio data. */
static switch_bool_t switch_core_media_drain_bundled_rtp(switch_media_handle_t *smh, switch_rtp_engine_t *engine)
{
	...
	if (!smh || !engine || !switch_core_media_bundle_negotiated(smh) || !switch_rtp_ready(engine->rtp_session)) {
		return SWITCH_FALSE;
	}
	...
	while (budget-- > 0) {
		...
		st = switch_rtp_zerocopy_read_frame(engine->rtp_session, &engine->read_frame,
											SWITCH_IO_FLAG_NOBLOCK | SWITCH_IO_FLAG_SINGLE_READ);

		if (st != SWITCH_STATUS_SUCCESS || engine->read_frame.datalen == 0) {
			break;
		}

		/* CNG frames come from JB-absorbed audio with no ready output; skip */
		if (switch_test_flag(&engine->read_frame, SFF_CNG)) {
			continue;
		}

		if (!switch_test_flag(&engine->read_frame, SFF_RAW_RTP)) {
			continue;
		}

		/* Ensure buflen is set for any downstream frame_buffer_dup */
		if (!engine->read_frame.buflen) {
			engine->read_frame.buflen = SWITCH_RTP_MAX_BUF_LEN;
		}
															 ...
															 switch_io_flag_t flags, int stream_id, switch_media_type_t type)
{
	...
	while (smh->media_flags[SCMF_RUNNING] && engine->read_frame.datalen == 0) {
		...
		if (type == SWITCH_MEDIA_TYPE_VIDEO && engine->bundled_with_audio && engine->read_fb) {
			...
			/* Drain thread is the sole socket reader.
		...
		/* When drain thread is active, audio path reads from queue above
		 * and should not reach this point. Old route+drain fallback only runs
		 * when drain thread is inactive. */
		if (smh->engines[SWITCH_MEDIA_TYPE_VIDEO].bundle_drain_state != BUNDLE_DRAIN_RUNNING
			&& switch_core_media_route_bundled_rtp(smh, engine, type) == SWITCH_TRUE) {
			/* Video packet routed. Drain remaining socket to video queue
			 * without 20ms timer penalty per packet. */
			if (switch_core_media_drain_bundled_rtp(smh, engine) == SWITCH_TRUE) {
				...
				/* No audio yet. One more NOBLOCK read to extract from JB. */
				engine->read_frame.datalen = 0;
				engine->read_frame.flags = SFF_NONE;
				status = switch_rtp_zerocopy_read_frame(engine->rtp_session, &engine->read_frame,
														  SWITCH_IO_FLAG_NOBLOCK | SWITCH_IO_FLAG_SINGLE_READ);
				if (status == SWITCH_STATUS_SUCCESS && engine->read_frame.datalen > 0
					&& switch_test_flag(&engine->read_frame, SFF_RAW_RTP)
		...
		if (fire_readable && status == SWITCH_STATUS_SUCCESS && !switch_test_flag((&engine->read_frame), SFF_CNG)) {
			...
			switch_channel_set_flag(session->channel, CF_MEDIA_READABLE_FIRED);
			switch_core_session_media_readable(session);
			switch_channel_clear_flag(session->channel, CF_MEDIA_TIMEOUT_FIRED);
			switch_rtp_reset_media_timer(engine->rtp_session);
		}
		...
		if (type != SWITCH_MEDIA_TYPE_TEXT && engine->reset_codec > 0) {
			...
			if (type == SWITCH_MEDIA_TYPE_AUDIO && switch_core_codec_ready(&engine->read_codec)) {
				switch_clear_flag(&engine->read_codec, SWITCH_CODEC_FLAG_RESET_PENDING);
			}
												  ...
												  switch_frame_t *frame, switch_io_flag_t flags, int stream_id, switch_media_type_t type)
{
	...
	if (type == SWITCH_MEDIA_TYPE_VIDEO && engine->bundled_with_audio) {
		...
		switch_mutex_lock(smh->sdp_mutex);
		mid_ext_id = engine->active_mid_send_ext_id;
		switch_copy_string(mid, engine->active_mid, sizeof(mid));
		switch_mutex_unlock(smh->sdp_mutex);
		...
		if (zstr(mid) || !mid_ext_id) {
			 ...
			 * of flushing stale startup frames. */
			if (engine->pre_dtls_buf && engine->pre_dtls_buf->count > 0) {
				uint32_t dropped_pre_dtls;
			...
			} else if (engine->pre_dtls_wait_keyframe && !pre_dtls_keyframe_gate(session, engine, frame, "bundle")) {
				video_suppressed = 1;
			} else {
...
enqueue_pre_dtls_pkt:
				...
				pre_dtls_track_deferred_write(session, engine, frame, "bundle");
			}
		}
	} else {
		...
		if (type == SWITCH_MEDIA_TYPE_VIDEO && !switch_test_flag(frame, SFF_CNG)) {
			...
			} else if (engine->pre_dtls_wait_keyframe && !pre_dtls_keyframe_gate(session, engine, frame, "non-bundle")) {
				defer_writable = 1;
				status = SWITCH_STATUS_SUCCESS;
			} else {
			...
			if (nb_do_enqueue) {
				pre_dtls_track_deferred_write(session, engine, frame, "non-bundle");
			}
																				...
																				size_t resolved_addr_size)
{
	...
	/* First try to parse as IP address - check both IPv4 and IPv6 */
	if (cand_addr && (is_valid_ipv4((char *)cand_addr) || is_valid_ipv6((char *)cand_addr))) {
		switch_copy_string(resolved_addr, cand_addr, resolved_addr_size);
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG1, "Is an IP address: %s\n", resolved_addr);
	} else if (cand_addr && ice_resolve) {
		/* Not a direct IP, try to resolve as hostname */
		if (switch_resolve_host(cand_addr, resolved_addr, resolved_addr_size) == SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO, "Resolved %s to %s\n", cand_addr, resolved_addr);
		} else {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
							  "Drop %s Candidate cid: %d addr: %s (cannot resolve)\n",
							  media_type_str, cid+1, cand_addr);
			return SWITCH_FALSE;
		}
	} else {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
						  "Drop %s Candidate cid: %d addr: %s (not an IP address)\n",
						  media_type_str, cid+1, cand_addr ? cand_addr : "(null)");
		return SWITCH_FALSE;
	}

	/* Check if network path exists (rtpip4 or rtpip6 configured) */
	if (!ip_possible(smh, resolved_addr)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
						  "Drop %s Candidate cid: %d addr: %s (no network path)\n",
						  media_type_str, cid+1, resolved_addr);
		return SWITCH_FALSE;
	}

	/* Check for mDNS candidates if ignore_ice_mdns is enabled */
	if (ignore_ice_mdns && is_mdns(resolved_addr)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
						  "Drop %s Candidate cid: %d addr: %s (ignore mdns)\n",
						  media_type_str, cid+1, resolved_addr);
		return SWITCH_FALSE;
	}
...
static int trickle_merge_rtp_remote_into_engine(switch_media_handle_t *smh, switch_media_type_t type)
{
	switch_core_session_t *session = smh ? smh->session : NULL;
	switch_rtp_engine_t *engine;
	switch_rtp_t *rtp;
	int added = 0;
	...
	if (added) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
		                  "Trickle ICE: merged %d remote candidate(s) from RTP into engine list (%s)\n",
		                  added,
...
static const char *resolve_candidate_acl(switch_core_session_t *session, switch_bool_t trickle_on)
{
	const char *cand_acl = NULL;

	if (!session) {
		return NULL;
	}

	cand_acl = switch_channel_get_variable(session->channel, "apply-candidate-acl");
	if (cand_acl && *cand_acl) {
		return cand_acl;
	}

	if (trickle_on) {
		const char *global_default = switch_core_get_variable("rtp_trickle_ice_default_acl");
		if (global_default && *global_default) {
...
static switch_status_t check_ice(switch_media_handle_t *smh, switch_media_type_t type, sdp_session_t *sdp, sdp_media_t *m, switch_sdp_type_t sdp_type)
{
	...
	cand_acl = resolve_candidate_acl(smh->session, trickle_on);
	if (cand_acl && *cand_acl) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG, "ICE: using candidate ACL \"%s\" (trickle:%s)\n", cand_acl, trickle_on ? "on" : "off");
	} else {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG, "ICE: no candidate ACL resolved (trickle:%s) — will allow by default.\n", trickle_on ? "on" : "off");
	}
	...
	for (attr_idx = 0; attr_idx < 2 && !(ice_seen && cand_seen); attr_idx++) {
		for (attr = attrs[attr_idx]; attr; attr = attr->a_next) {
			...
			if (!strcasecmp(attr->a_name, "group") && attr->a_value && !strncasecmp(attr->a_value, "BUNDLE", 6)) {
			...
			} else if (!strcasecmp(attr->a_name, "end-of-candidates")) {
					const char *mid_hint = (type == SWITCH_MEDIA_TYPE_AUDIO) ? "audio" : (type == SWITCH_MEDIA_TYPE_VIDEO) ? "video" : NULL;
					int mline_index_hint = (type == SWITCH_MEDIA_TYPE_AUDIO) ? 0 : (type == SWITCH_MEDIA_TYPE_VIDEO) ? 1 : -1;
					switch_core_media_trickle_end_of_candidates(smh->session, mline_index_hint, mid_hint);
					if (attr_idx == 1) {
						switch_channel_set_variable(smh->session->channel, "rtp_in_session_eoc", "true");
					} else {
						if (type == SWITCH_MEDIA_TYPE_AUDIO) {
							switch_channel_set_variable(smh->session->channel, "rtp_in_audio_eoc", "true");
						} else if (type == SWITCH_MEDIA_TYPE_VIDEO) {
							switch_channel_set_variable(smh->session->channel, "rtp_in_video_eoc", "true");
						} else if (type == SWITCH_MEDIA_TYPE_TEXT) {
							switch_channel_set_variable(smh->session->channel, "rtp_in_text_eoc", "true");
						}
					}
					if (engine) {
						engine->ice_in.end_of_candidates = 1;
					}
				...
				/* when BUNDLE is reusing the audio RTP session for video,
				...
				{
					const char *media_type_str = (type == SWITCH_MEDIA_TYPE_VIDEO) ? "video" : "audio";
					if (!switch_core_media_validate_ice_candidate_address(smh, fields[4], ignore_ice_mdns, ice_resolve, media_type_str, cid, con_addr, sizeof(con_addr))) {
						continue;
					}
				}
				...
				{
					int ia, allowed = 0;
					for (ia = 0; ia < engine->cand_acl_count; ia++) {
						if (switch_check_network_list_ip(con_addr, engine->cand_acl[ia])) { 
							allowed = 1; 
							break; 
						}
					}
	...
	if (trickle_on) {
		int merged = 0;
		if (switch_true(switch_channel_get_variable(smh->session->channel, "rtp_trickle_ice"))) {
			merged = trickle_merge_rtp_remote_into_engine(smh, type);
			if (merged > 0) {
				cand_seen = 1;
				engine->new_ice = 1;
			}
		...
		if (!engine->cand_acl_count) {
			const char *def_acl = switch_core_get_variable("rtp_trickle_ice_default_acl");

			if (!def_acl || !*def_acl) {
				def_acl = "wan.auto";
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_WARNING, "ICE: no candidate ACL configured for trickle; defaulting to \"%s\"\n", def_acl);
			} else {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG, "ICE: using default trickle candidate ACL \"%s\"\n", def_acl);
			}

			if (engine->cand_acl_count < SWITCH_MAX_CAND_ACL) {
				engine->cand_acl[engine->cand_acl_count++] = switch_core_session_strdup(smh->session, def_acl);
			}
		}
	}
	...
	for (cid = 0; cid < MAX_CAND_IDX_COUNT; cid++) {
		...
		for (ai = 0; ai < engine->cand_acl_count; ai++) {
			for (i = 0; i < engine->ice_in.cand_idx[cid]; i++) {
				...
				/* Determine the address to use for ACL check */
				{
					const char *check_addr = engine->ice_in.cands[i][cid].con_addr;
					char resolved_addr[256] = "";

					/* For trickle ICE, validate and resolve the candidate address */
					if (trickle_on) {
						const char *media_type_str = (type == SWITCH_MEDIA_TYPE_VIDEO) ? "video" : "audio";

						if (!switch_core_media_validate_ice_candidate_address(smh, check_addr, ignore_ice_mdns, ice_resolve, media_type_str, cid, resolved_addr, sizeof(resolved_addr))) {
							continue;
						}
						/* Use the resolved address for ACL check */
						check_addr = resolved_addr;
					}

					/* Perform ACL check with the appropriate address (resolved for trickle, raw for traditional) */
					if (switch_check_network_list_ip(check_addr, engine->cand_acl[ai])) {
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
										"Choose %s candidate, index %d, %s:%d%s%s%s\n",
										cid ? "rtcp" : "rtp", i,
										engine->ice_in.cands[i][cid].con_addr,
										engine->ice_in.cands[i][cid].con_port,
										trickle_on ? " (resolved: " : "",
										trickle_on ? check_addr : "",
										trickle_on ? ")" : "");
						...
						if (cid == 0 && engine->rtcp_mux > 0 && engine->ice_in.cand_idx[1] < MAX_CAND) {
							...
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
											...
											check_addr, engine->ice_in.cands[i][cid].con_port);
 ...
 done_choosing:
	...
	if (!engine->ice_in.is_chosen[0]) {
		...
		if (trickle_accept_enabled(smh->session)) {
			switch_rtp_engine_t *a_engine = engine;
			if (a_engine && a_engine->rtp_session) { /* RFC 8863 PAC */
				const char *pac_ms_str = switch_channel_get_variable_dup(smh->session->channel, "rtp_ice_pac_ms", SWITCH_FALSE, -1);
				uint32_t pac_ms = (uint32_t)(pac_ms_str ? atoi(pac_ms_str) : ICE_PAC_DEFAULT_MS);
				const char *until_str = switch_channel_get_variable(smh->session->channel, "rtp_ice_pac_until_us");
				switch_time_t now_us = switch_time_now();
				switch_time_t until_us = (until_str && *until_str) ? (switch_time_t)switch_atoul(until_str) : 0;
				if (switch_true(switch_channel_get_variable_dup(smh->session->channel, "rtp_in_session_eoc", SWITCH_FALSE, -1)) ||
					((type == SWITCH_MEDIA_TYPE_AUDIO) && switch_true(switch_channel_get_variable_dup(smh->session->channel, "rtp_in_audio_eoc", SWITCH_FALSE, -1))) ||
					((type == SWITCH_MEDIA_TYPE_VIDEO) && switch_true(switch_channel_get_variable_dup(smh->session->channel, "rtp_in_video_eoc", SWITCH_FALSE, -1))) ||
					((type == SWITCH_MEDIA_TYPE_TEXT) && switch_true(switch_channel_get_variable_dup(smh->session->channel, "rtp_in_text_eoc", SWITCH_FALSE, -1)))) {
					/*fallthrough*/
				} else if (!until_us) {
					until_us = now_us + (switch_time_t)pac_ms * 1000;
					switch_channel_set_variable_printf(smh->session->channel, "rtp_ice_pac_until_us", "%" SWITCH_TIME_T_FMT, until_us);
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG,
										"ICE PAC: delaying failure %u ms waiting for trickled/prflx candidates (type=%s)\n", pac_ms, type2str(type));
					return SWITCH_STATUS_SUCCESS;
				} else if (now_us < until_us) {
					return SWITCH_STATUS_SUCCESS;
				} else {
					switch_channel_set_variable(smh->session->channel, "rtp_ice_pac_until_us", NULL);
				}
			} else {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_DEBUG, "%s Trickle ICE enabled but RTP session not ready yet, will prepare later.\n", 
					switch_channel_get_name(smh->session->channel));
				return SWITCH_STATUS_SUCCESS;
			}
		}
	...
	if (engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_addr && engine->ice_in.cands[engine->ice_in.chosen[0]][0].con_port) {
		...
		{
			const icand_t *C = &engine->ice_in.cands[engine->ice_in.chosen[0]][0];
			const char *pfx = (engine->type == SWITCH_MEDIA_TYPE_VIDEO) ? "remote_video_ice_" : (engine->type == SWITCH_MEDIA_TYPE_AUDIO) ? "remote_audio_ice_" : "remote_text_ice_";
			...
			if (C->raddr && *C->raddr) {
				switch_channel_set_variable_printf(smh->session->channel, "%srel_addr", pfx, "%s", C->raddr);
				switch_channel_set_variable_printf(smh->session->channel, "%srel_port", pfx, "%u", (unsigned)C->rport);
			}
		}
	}
	...
	if (engine->ice_in.cands[engine->ice_in.chosen[1]][1].con_port) {
		...
		{
			const icand_t *C = &engine->ice_in.cands[engine->ice_in.chosen[1]][1];
			const char *pfx = (engine->type == SWITCH_MEDIA_TYPE_VIDEO) ? "remote_video_rtcp_ice_" : (engine->type == SWITCH_MEDIA_TYPE_AUDIO) ? "remote_audio_rtcp_ice_" : "remote_text_rtcp_ice_";
	...
	if (engine->new_ice) {
		if (switch_rtp_ready(engine->rtp_session) && engine->ice_in.cands[engine->ice_in.chosen[0]][0].ready) {
			...
			if (trickle_on) {
				...
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
					"Updating %s RTP remote address for trickle ICE: %s:%d\n", type2str(type),
...
#endif
					/* Check for legacy DTLS requirement */
					if (switch_channel_var_true(smh->session->channel, "legacyDTLS")) {
						switch_channel_clear_flag(smh->session->channel, CF_WANT_DTLSv1_2);
						want_DTLSv1_2 = 0;
					}

					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO,
						"Resetting %s DTLS after trickle ICE candidate change\n", type2str(type));

					if (engine->rtcp_mux > 0) {
...
SWITCH_DECLARE(void) switch_core_session_set_ice(switch_core_session_t *session)
{
	...
	/* We check session channel is secure or CF_AVPF_MOZ which indicates the remote SDP 
	   has AVPF profiles, which in WebRTC always comes with security (DTLS fingerprint). */
	if (switch_core_media_has_crypto(session)) {
		switch_channel_set_flag(session->channel, CF_AVPF);
	}

	/* Keep ICE independent; this enables ICE capability on the channel. */
...
SWITCH_DECLARE(int16_t) switch_core_media_validate_common_audio_sdp(switch_core_session_t *session, const char *r_sdp, switch_sdp_type_t sdp_type)
{
	...
	int got_crypto = 0, got_avp = 0, got_savp = 0, got_webrtc = 0;
	sdp_parser_t *parser = NULL;
	sdp_session_t *sdp;
	switch_rtp_engine_t *a_engine;
	const switch_codec_implementation_t *codec_array[SWITCH_MAX_CODECS];
	...
	const switch_codec_implementation_t *near_match = NULL;
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Validate common audio SDP.\n");

	a_engine = &smh->engines[SWITCH_MEDIA_TYPE_AUDIO];
	total_codecs = switch_core_media_retrieve_codecs(session, codec_array);
		...
		if (m->m_type == sdp_media_audio && m->m_port) {
			...
			for (attr = m->m_attributes; attr; attr = attr->a_next) {
				if (!strcasecmp(attr->a_name, "ptime") && attr->a_value) {
					...
					got_crypto = switch_core_session_validate_incoming_crypto(session, SWITCH_MEDIA_TYPE_AUDIO, attr->a_value, sdp_type);
				}
			}
			...
			for (map = m->m_rtpmaps; map; map = map->rm_next) {
				...
				for (i = 0; i < total_codecs; i++) {
					...
					if (match && bit_rate && map_bit_rate &&
					...
					if (match && remote_codec_rate && codec_rate &&
							remote_codec_rate != codec_rate &&
							(!strcasecmp(map->rm_encoding, "pcma") || !strcasecmp(map->rm_encoding, "pcmu"))) {
						/* if the sampling rate is specified and doesn't match, this is not a codec match for G.711 */
						match = 0;
					}

					if (match) {
						if ((ptime && codec_ms && codec_ms * 1000 != imp->microseconds_per_packet) ||
								remote_codec_rate != codec_rate) {
...
#define SWITCH_BUNDLE_MID_EXT_URI "urn:ietf:params:rtp-hdrext:sdes:mid"

static switch_media_type_t switch_core_media_bundle_sdp_media_type(sdp_media_e type)
{
	switch (type) {
	case sdp_media_audio:
		return SWITCH_MEDIA_TYPE_AUDIO;
	case sdp_media_video:
		return SWITCH_MEDIA_TYPE_VIDEO;
	case sdp_media_text:
		return SWITCH_MEDIA_TYPE_TEXT;
	default:
		return SWITCH_MEDIA_TYPE_TOTAL;
	}
}

static uint8_t switch_core_media_parse_mid_extmap(const char *value, switch_bool_t *is_mid,
											  switch_bool_t *declared_sends, switch_bool_t *declared_receives)
{
	const char *p = value, *sp;
	char idpart[32] = { 0 };
	char *slash, *direction = NULL, *end = NULL;
	size_t n;
	long id;
	...
	if (declared_sends) {
		*declared_sends = SWITCH_FALSE;
	}
	if (declared_receives) {
		*declared_receives = SWITCH_FALSE;
	}
	...
	id = strtol(idpart, &end, 10);
	if (end == idpart || *end || id <= 0 || id > 14) {
		return 0;
	}

	if (!direction || !*direction || !strcasecmp(direction, "sendrecv")) {
		if (declared_sends) *declared_sends = SWITCH_TRUE;
		if (declared_receives) *declared_receives = SWITCH_TRUE;
	...
	if (is_mid && !strncasecmp(sp, SWITCH_BUNDLE_MID_EXT_URI, strlen(SWITCH_BUNDLE_MID_EXT_URI)) &&
		(sp[strlen(SWITCH_BUNDLE_MID_EXT_URI)] == '\0' || sp[strlen(SWITCH_BUNDLE_MID_EXT_URI)] == ' ' || sp[strlen(SWITCH_BUNDLE_MID_EXT_URI)] == '\t')) {
		*is_mid = SWITCH_TRUE;
	}
...
static void switch_core_media_parse_mid_extmap_list(sdp_attribute_t *attrs, uint8_t inherited_send_ext_id,
												 uint8_t inherited_recv_ext_id, uint8_t *send_ext_id, uint8_t *recv_ext_id)
{
	sdp_attribute_t *attr;
	uint8_t local_send_ext_id = 0, local_recv_ext_id = 0;
	uint8_t non_mid_send[15] = { 0 }, non_mid_recv[15] = { 0 };
	switch_bool_t mid_seen = SWITCH_FALSE, mid_send_collision = SWITCH_FALSE, mid_recv_collision = SWITCH_FALSE;

	for (attr = attrs; attr; attr = attr->a_next) {
		switch_bool_t is_mid = SWITCH_FALSE;
		switch_bool_t remote_sends = SWITCH_FALSE;
		switch_bool_t remote_receives = SWITCH_FALSE;
		uint8_t id;

		if (zstr(attr->a_name) || strcasecmp(attr->a_name, "extmap") || zstr(attr->a_value)) continue;

		id = switch_core_media_parse_mid_extmap(attr->a_value, &is_mid, &remote_sends, &remote_receives);
		if (!id) continue;

		if (is_mid) {
			mid_seen = SWITCH_TRUE;
			if (remote_receives) {
				if (local_send_ext_id && local_send_ext_id != id) mid_send_collision = SWITCH_TRUE;
				local_send_ext_id = id;
			}
			if (remote_sends) {
				if (local_recv_ext_id && local_recv_ext_id != id) mid_recv_collision = SWITCH_TRUE;
				local_recv_ext_id = id;
			}
		} else {
			if (remote_receives) non_mid_send[id] = 1;
			if (remote_sends) non_mid_recv[id] = 1;
		}
	}

	if (!mid_seen) {
		local_send_ext_id = inherited_send_ext_id;
		local_recv_ext_id = inherited_recv_ext_id;
	}
	if (mid_send_collision) local_send_ext_id = 0;
	if (mid_recv_collision) local_recv_ext_id = 0;
	if (local_send_ext_id && non_mid_send[local_send_ext_id]) local_send_ext_id = 0;
	if (local_recv_ext_id && non_mid_recv[local_recv_ext_id]) local_recv_ext_id = 0;

	if (send_ext_id) *send_ext_id = local_send_ext_id;
	if (recv_ext_id) *recv_ext_id = local_recv_ext_id;
}

static void switch_core_media_clear_mid_extensions(switch_rtp_engine_t *engine)
{
	uint8_t i;

	if (!engine) return;

	for (i = 0; i < MAX_RTP_EXTENSIONS; i++) {
		if (engine->rtp_send_extensions[i].type == SWITCH_MEDIA_EXTENSION_MID) {
			memset(&engine->rtp_send_extensions[i], 0, sizeof(engine->rtp_send_extensions[i]));
		}
		if (engine->rtp_recv_extensions[i].type == SWITCH_MEDIA_EXTENSION_MID) {
			memset(&engine->rtp_recv_extensions[i], 0, sizeof(engine->rtp_recv_extensions[i]));
		}
	}
}

static void switch_core_media_replace_mid_extensions(switch_rtp_engine_t *engine, uint8_t send_ext_id, uint8_t recv_ext_id)
{
	if (!engine) return;

	switch_core_media_clear_mid_extensions(engine);
	if (send_ext_id) switch_core_media_set_send_extension(engine, send_ext_id, SWITCH_MEDIA_EXTENSION_MID);
	if (recv_ext_id) switch_core_media_set_recv_extension(engine, recv_ext_id, SWITCH_MEDIA_EXTENSION_MID);
}

static void switch_core_media_store_negotiated_mid(switch_mid_negotiation_t *state, const char *mid)
{
	size_t mid_len;

	if (!state || zstr(mid)) return;

	mid_len = strlen(mid);
	if (mid_len > SWITCH_SDP_MID_MAX_LEN) {
		state->mid[0] = '\0';
		state->send_ext_id = 0;
		state->recv_ext_id = 0;
		return;
	...
	switch_copy_string(state->mid, mid, sizeof(state->mid));
	if (mid_len > SWITCH_RTP_MID_MAX_LEN) {
		state->send_ext_id = 0;
		state->recv_ext_id = 0;
	}
}

static void switch_core_media_clear_pending_mid(switch_media_handle_t *smh)
{
	uint8_t i;
	...
	for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
		memset(&smh->engines[i].pending_mid, 0, sizeof(smh->engines[i].pending_mid));
	}
	memset(&smh->pending_bundle, 0, sizeof(smh->pending_bundle));
	smh->mid_offer_pending = 0;
	switch_mutex_unlock(smh->sdp_mutex);
}

static void switch_core_media_store_pending_mid(switch_media_handle_t *smh, const switch_mid_negotiation_t states[SWITCH_MEDIA_TYPE_TOTAL],
									 const switch_bundle_group_t *bundle)
{
	uint8_t i;

	if (!smh || !states || !bundle) return;

	switch_mutex_lock(smh->sdp_mutex);
	for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
		smh->engines[i].pending_mid = states[i];
	}
	smh->pending_bundle = *bundle;
	smh->mid_offer_pending = 1;
	switch_mutex_unlock(smh->sdp_mutex);
}

static switch_bool_t switch_core_media_snapshot_pending_mid(switch_media_handle_t *smh, switch_mid_negotiation_t states[SWITCH_MEDIA_TYPE_TOTAL],
										 switch_bundle_group_t *bundle)
{
	uint8_t i;
	switch_bool_t pending = SWITCH_FALSE;

	if (!smh || !states || !bundle) return SWITCH_FALSE;

	switch_mutex_lock(smh->sdp_mutex);
	if (smh->mid_offer_pending) {
		for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
			states[i] = smh->engines[i].pending_mid;
		}
		*bundle = smh->pending_bundle;
		pending = SWITCH_TRUE;
	}
	switch_mutex_unlock(smh->sdp_mutex);
...
static void switch_core_media_publish_bundle_variable(switch_core_session_t *session, const switch_bundle_group_t *bundle)
{
	char mids[(SWITCH_BUNDLE_MAX_MIDS * (SWITCH_BUNDLE_MAX_MID_LEN + 1)) + 1] = "";
	uint32_t i;

	if (!session || !bundle || bundle->state != SWITCH_BUNDLE_STATE_ACCEPTED) {
		if (session) switch_channel_set_variable(session->channel, "rtp_group_bundle", NULL);
		return;
	}

	for (i = 0; i < bundle->offered_mid_count; i++) {
		if (i) switch_snprintf(mids + strlen(mids), sizeof(mids) - strlen(mids), " ");
		switch_snprintf(mids + strlen(mids), sizeof(mids) - strlen(mids), "%s", bundle->offered_mids[i]);
	}
	switch_channel_set_variable(session->channel, "rtp_group_bundle", mids);
}

static void switch_core_media_sync_mid_state_locked(switch_core_session_t *session);

static void switch_core_media_commit_mid(switch_core_session_t *session, const switch_mid_negotiation_t states[SWITCH_MEDIA_TYPE_TOTAL],
								  const switch_bundle_group_t *bundle)
{
	static const char *mid_variables[SWITCH_MEDIA_TYPE_TOTAL] = { "rtp_audio_mid", "rtp_video_mid", "rtp_text_mid" };
	switch_media_handle_t *smh;
	uint8_t i;

	if (!session || !(smh = session->media_handle) || !states) return;

	switch_mutex_lock(smh->sdp_mutex);
	if (bundle) {
		smh->bundle = *bundle;
		switch_core_media_publish_bundle_variable(session, &smh->bundle);
	}
	for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
		switch_core_media_replace_mid_extensions(&smh->engines[i], states[i].send_ext_id, states[i].recv_ext_id);
		switch_channel_set_variable(session->channel, mid_variables[i], zstr(states[i].mid) ? NULL : states[i].mid);
		memset(&smh->engines[i].pending_mid, 0, sizeof(smh->engines[i].pending_mid));
	}
	memset(&smh->pending_bundle, 0, sizeof(smh->pending_bundle));
	smh->mid_offer_pending = 0;
	switch_core_media_sync_mid_state_locked(session);
	switch_mutex_unlock(smh->sdp_mutex);
}

static void switch_core_media_local_mid_extmaps(switch_core_session_t *session, switch_media_type_t type,
										 char *mid, switch_size_t mid_len, uint8_t *send_ext_id, uint8_t *recv_ext_id)
{
	switch_media_handle_t *smh;
	sdp_parser_t *parser = NULL;
	sdp_session_t *sdp;
	sdp_media_t *m;
	sdp_attribute_t *attr;
	uint8_t session_send_ext_id = 0, session_recv_ext_id = 0;
	uint8_t media_send_ext_id = 0, media_recv_ext_id = 0;
	switch_bool_t is_mid = SWITCH_FALSE, declared_sends = SWITCH_FALSE, declared_receives = SWITCH_FALSE;
	switch_bool_t media_mid_seen = SWITCH_FALSE;

	if (mid && mid_len) mid[0] = '\0';
	if (send_ext_id) *send_ext_id = 0;
	if (recv_ext_id) *recv_ext_id = 0;
	if (!session || !(smh = session->media_handle) || zstr(smh->mparams->local_sdp_str) || !mid || mid_len < 2) {
		return;
	}

	if (!(parser = sdp_parse(NULL, smh->mparams->local_sdp_str, (int)strlen(smh->mparams->local_sdp_str), 0)) ||
	...
	for (attr = sdp->sdp_attributes; attr; attr = attr->a_next) {
		if (!zstr(attr->a_name) && !strcasecmp(attr->a_name, "extmap") && !zstr(attr->a_value)) {
			uint8_t ext_id = switch_core_media_parse_mid_extmap(attr->a_value, &is_mid, &declared_sends, &declared_receives);

			if (ext_id && is_mid) {
				if (declared_sends) session_send_ext_id = ext_id;
				if (declared_receives) session_recv_ext_id = ext_id;
			}
		}
	}

	for (m = sdp->sdp_media; m; m = m->m_next) {
		if (switch_core_media_bundle_sdp_media_type(m->m_type) != type || !m->m_port) continue;

		for (attr = m->m_attributes; attr; attr = attr->a_next) {
			if (!zstr(attr->a_name) && !strcasecmp(attr->a_name, "mid") && !zstr(attr->a_value)) {
				if (strlen(attr->a_value) <= SWITCH_RTP_MID_MAX_LEN) {
					switch_copy_string(mid, attr->a_value, mid_len);
				} else {
					mid[0] = '\0';
				}
			} else if (!zstr(attr->a_name) && !strcasecmp(attr->a_name, "extmap") && !zstr(attr->a_value)) {
				uint8_t ext_id = switch_core_media_parse_mid_extmap(attr->a_value, &is_mid, &declared_sends, &declared_receives);

				if (ext_id && is_mid) {
					media_mid_seen = SWITCH_TRUE;
					if (declared_sends) media_send_ext_id = ext_id;
					if (declared_receives) media_recv_ext_id = ext_id;
				}
			}
		}
		break;
	...
	if (zstr(mid)) return;
	if (send_ext_id) *send_ext_id = media_mid_seen ? media_send_ext_id : session_send_ext_id;
	if (recv_ext_id) *recv_ext_id = media_mid_seen ? media_recv_ext_id : session_recv_ext_id;
}

static void switch_core_media_sync_mid_state_locked(switch_core_session_t *session)
{
	switch_media_handle_t *smh;
	switch_rtp_engine_t *ae, *ve, *te;
	switch_rtp_engine_t *engines[3];
	switch_media_type_t types[3] = { SWITCH_MEDIA_TYPE_AUDIO, SWITCH_MEDIA_TYPE_VIDEO, SWITCH_MEDIA_TYPE_TEXT };
	uint8_t i;

	if (!session || !(smh = session->media_handle)) return;

	ae = &smh->engines[SWITCH_MEDIA_TYPE_AUDIO];
	ve = &smh->engines[SWITCH_MEDIA_TYPE_VIDEO];
	te = &smh->engines[SWITCH_MEDIA_TYPE_TEXT];
	engines[0] = ae;
	engines[1] = ve;
	engines[2] = te;

	for (i = 0; i < 3; i++) {
		switch_rtp_engine_t *engine = engines[i];
		const rtp_extension_t *send_ext;
		const rtp_extension_t *recv_ext;
		char mid[SWITCH_RTP_MID_MAX_LEN + 1] = "";
		uint8_t local_send_ext_id = 0, local_recv_ext_id = 0;
		uint8_t configured_send_ext_id = 0, configured_recv_ext_id = 0;
		uint8_t j;
		switch_bool_t shared_video_rtp;

		if (!engine || !engine->rtp_session) continue;
		shared_video_rtp = engine == ve && ve->bundled_with_audio && ve->rtp_session == ae->rtp_session;

		send_ext = switch_core_media_get_send_extension_by_type(engine, SWITCH_MEDIA_EXTENSION_MID);
		recv_ext = switch_core_media_get_recv_extension_by_type(engine, SWITCH_MEDIA_EXTENSION_MID);
		switch_core_media_local_mid_extmaps(session, types[i], mid, sizeof(mid), &local_send_ext_id, &local_recv_ext_id);

		/* Keep only mappings negotiated by current local SDP because these tables feed BUNDLE video writes. */
		if (send_ext && (!local_send_ext_id || send_ext->id != local_send_ext_id || zstr(mid))) {
			for (j = 0; j < MAX_RTP_EXTENSIONS; j++) {
				if (engine->rtp_send_extensions[j].type == SWITCH_MEDIA_EXTENSION_MID) {
					memset(&engine->rtp_send_extensions[j], 0, sizeof(engine->rtp_send_extensions[j]));
				}
			}
			send_ext = NULL;
		}
		if (recv_ext && (!local_recv_ext_id || recv_ext->id != local_recv_ext_id)) {
			for (j = 0; j < MAX_RTP_EXTENSIONS; j++) {
				if (engine->rtp_recv_extensions[j].type == SWITCH_MEDIA_EXTENSION_MID) {
					memset(&engine->rtp_recv_extensions[j], 0, sizeof(engine->rtp_recv_extensions[j]));
				}
			}
			recv_ext = NULL;
		}
		if (send_ext && send_ext->id && !zstr(mid)) configured_send_ext_id = local_send_ext_id;
		if (recv_ext && recv_ext->id) configured_recv_ext_id = local_recv_ext_id;

		engine->active_mid_send_ext_id = configured_send_ext_id;
		memset(engine->active_mid, 0, sizeof(engine->active_mid));
		if (configured_send_ext_id) switch_copy_string(engine->active_mid, mid, sizeof(engine->active_mid));

		if (shared_video_rtp) continue;
		switch_rtp_configure_mid(engine->rtp_session, configured_send_ext_id,
			configured_send_ext_id ? mid : NULL, configured_recv_ext_id);
	}

}

static void switch_core_media_sync_mid_state(switch_core_session_t *session)
{
	switch_media_handle_t *smh;
...
static switch_bool_t switch_core_media_bundle_negotiated(switch_media_handle_t *smh)
{
	switch_bool_t negotiated;

	if (!smh) return SWITCH_FALSE;
	switch_mutex_lock(smh->sdp_mutex);
	negotiated = smh->bundle.state == SWITCH_BUNDLE_STATE_ACCEPTED ? SWITCH_TRUE : SWITCH_FALSE;
	switch_mutex_unlock(smh->sdp_mutex);
	return negotiated;
}
...
static switch_bundle_policy_t switch_core_media_bundle_policy(const switch_media_handle_t *smh)
{
	...
	if (zstr(policy_value)) {
		policy_value = switch_core_get_variable("rtp-bundle");
	}
	if (zstr(policy_value)) {
...
static switch_bool_t switch_core_media_bundle_should_offer(const switch_media_handle_t *smh, const switch_bundle_group_t *bundle)
{
...
static void switch_core_media_raw_rejected_mid(const char *sdp_str, int wanted_mline_index,
									char *mid, switch_size_t mid_len);

static void switch_core_media_bundle_populate_from_sdp(switch_media_handle_t *smh, sdp_session_t *sdp,
										   const char *sdp_str, switch_bool_t local_sdp, switch_bundle_group_t *bundle)
{
	...
	for (attr = sdp->sdp_attributes; attr; attr = attr->a_next) {
		...
		if (zstr(attr->a_name) || strcasecmp(attr->a_name, "group") || zstr(attr->a_value)) {
			continue;
		}
		...
		if (!strncasecmp(value, "BUNDLE", 6) && (value[6] == '\0' || value[6] == ' ' || value[6] == '\t')) {
			bundle_group_status = switch_bundle_group_set_offered_mids(bundle, value);
			break;
		}
	}
	switch_core_media_parse_mid_extmap_list(sdp->sdp_attributes, 0, 0, &session_send_ext_id, &session_recv_ext_id);

	for (m = sdp->sdp_media; m; m = m->m_next, mline_index++) {
		const char *mid = NULL;
		char rejected_mid[SWITCH_SDP_MID_MAX_LEN + 1] = "";
		switch_media_type_t media_type;
		switch_bool_t bundle_only = SWITCH_FALSE;
		switch_bool_t rtcp_mux = SWITCH_FALSE;
		switch_bool_t rtcp_mux_only = SWITCH_FALSE;
		uint8_t remote_send_ext_id = 0, remote_recv_ext_id = 0;
		uint8_t remote_mid_ext_id;
		switch_bundle_mline_t *bundle_mline;
		sdp_rtpmap_t *map;

		switch_core_media_parse_mid_extmap_list(m->m_attributes, session_send_ext_id, session_recv_ext_id,
			&remote_send_ext_id, &remote_recv_ext_id);
		remote_mid_ext_id = local_sdp ? remote_send_ext_id : remote_recv_ext_id;
		...
		for (attr = m->m_attributes; attr; attr = attr->a_next) {
			...
			if (!strcasecmp(attr->a_name, "mid") && !zstr(attr->a_value)) {
				mid = attr->a_value;
			} else if (!strcasecmp(attr->a_name, "bundle-only")) {
				bundle_only = SWITCH_TRUE;
			} else if (!strcasecmp(attr->a_name, "rtcp-mux")) {
				rtcp_mux = SWITCH_TRUE;
			} else if (!strcasecmp(attr->a_name, "rtcp-mux-only")) {
				rtcp_mux_only = SWITCH_TRUE;
				rtcp_mux = SWITCH_TRUE;
			}
		}
		media_type = switch_core_media_bundle_sdp_media_type(m->m_type);
		if (!m->m_port && zstr(mid) && media_type < SWITCH_MEDIA_TYPE_TOTAL) {
			switch_core_media_raw_rejected_mid(sdp_str, mline_index, rejected_mid, sizeof(rejected_mid));
			mid = rejected_mid;
		}

		bundle_mline = switch_bundle_group_add_mline(bundle,
			mline_index,
			media_type,
			mid,
			(switch_port_t)m->m_port,
			rtcp_mux,
			bundle_only,
			rtcp_mux_only);
		...
		if (remote_mid_ext_id && (!mid || strlen(mid) <= SWITCH_RTP_MID_MAX_LEN)) {
			switch_bundle_mline_set_remote_mid_ext(bundle_mline, remote_mid_ext_id);
		}

		for (map = m->m_rtpmaps; map; map = map->rm_next) {
			if (map->rm_pt >= 0 && map->rm_pt <= 255) {
				switch_bundle_mline_add_payload_type(bundle_mline, (switch_payload_t)map->rm_pt);
			}
		}
	}
...
static const char *switch_core_media_sdp_mid(sdp_attribute_t *attrs)
{
	sdp_attribute_t *attr;
...
static void switch_core_media_raw_rejected_mid(const char *sdp_str, int wanted_mline_index,
									char *mid, switch_size_t mid_len)
{
	const char *line, *line_end, *value, *value_end, *port_str;
	switch_bool_t rejected = SWITCH_FALSE;
	size_t value_len;
	int mline_index = -1;

	if (!mid || !mid_len) return;
	mid[0] = '\0';
	if (zstr(sdp_str)) return;

	for (line = sdp_str; *line; line = line_end) {
		line_end = strpbrk(line, "\r\n");
		if (!line_end) line_end = line + strlen(line);

		if ((size_t)(line_end - line) > 2 && !strncmp(line, "m=", 2)) {
			mline_index++;
			port_str = NULL;
			if ((size_t)(line_end - line) > 8 && !strncmp(line, "m=audio ", 8)) {
				port_str = line + 8;
			} else if ((size_t)(line_end - line) > 8 && !strncmp(line, "m=video ", 8)) {
				port_str = line + 8;
			} else if ((size_t)(line_end - line) > 7 && !strncmp(line, "m=text ", 7)) {
				port_str = line + 7;
			}
			rejected = port_str && strtoul(port_str, NULL, 10) == 0 ? SWITCH_TRUE : SWITCH_FALSE;
		} else if (mline_index == wanted_mline_index && rejected &&
			(size_t)(line_end - line) > 6 && !strncmp(line, "a=mid:", 6)) {
			value = line + 6;
			value_end = value;
			while (value_end < line_end && *value_end != ' ' && *value_end != '\t') value_end++;
			value_len = (size_t)(value_end - value);
			if (value_len && value_len < mid_len) {
				memcpy(mid, value, value_len);
				mid[value_len] = '\0';
			}
			return;
		}
...
static void switch_core_media_populate_mid_states_from_sdp(sdp_session_t *sdp, const char *sdp_str,
									switch_mid_negotiation_t states[SWITCH_MEDIA_TYPE_TOTAL])
{
	sdp_media_t *m;
	uint8_t session_send_ext_id = 0, session_recv_ext_id = 0, i;
	int mline_index = 0;

	if (!sdp || !states) return;

	switch_core_media_parse_mid_extmap_list(sdp->sdp_attributes, 0, 0,
		&session_send_ext_id, &session_recv_ext_id);
	for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
		states[i].send_ext_id = session_send_ext_id;
		states[i].recv_ext_id = session_recv_ext_id;
	}
	...
	for (m = sdp->sdp_media; m; m = m->m_next, mline_index++) {
		...
		media_type = switch_core_media_bundle_sdp_media_type(m->m_type);
		if (media_type >= SWITCH_MEDIA_TYPE_TOTAL) continue;

		states[media_type].send_ext_id = 0;
		states[media_type].recv_ext_id = 0;
		if (m->m_port) {
			switch_core_media_parse_mid_extmap_list(m->m_attributes, session_send_ext_id, session_recv_ext_id,
				&states[media_type].send_ext_id, &states[media_type].recv_ext_id);
		}

		mid = switch_core_media_sdp_mid(m->m_attributes);
		if (zstr(mid) && !m->m_port) {
			switch_core_media_raw_rejected_mid(sdp_str, mline_index, rejected_mid, sizeof(rejected_mid));
			mid = rejected_mid;
		}
		if (!zstr(mid)) switch_core_media_store_negotiated_mid(&states[media_type], mid);
	}
}

static switch_bool_t switch_core_media_bundle_has_mid(const switch_bundle_group_t *bundle, const char *mid)
{
	uint32_t i;

	if (!bundle || zstr(mid)) return SWITCH_FALSE;
	for (i = 0; i < bundle->offered_mid_count; i++) {
		if (!strcmp(bundle->offered_mids[i], mid)) return SWITCH_TRUE;
	}

	return SWITCH_FALSE;
}

static switch_bool_t switch_core_media_bundle_has_media(const switch_bundle_group_t *bundle, switch_media_type_t type)
{
	uint32_t i;

	if (!bundle || bundle->state != SWITCH_BUNDLE_STATE_ACCEPTED) return SWITCH_FALSE;
	for (i = 0; i < bundle->mline_count; i++) {
		if (bundle->mlines[i].media_type == type && bundle->mlines[i].in_bundle_group && !bundle->mlines[i].rejected) {
			return SWITCH_TRUE;
		}
	}
...
static switch_media_type_t switch_core_media_bundle_tag_media(const switch_bundle_group_t *bundle)
{
	uint32_t i;
	...
	for (i = 0; i < bundle->mline_count; i++) {
		if (bundle->mlines[i].bundle_tag || !strcmp(bundle->mlines[i].mid, bundle->bundle_tag_mid)) {
			return bundle->mlines[i].media_type;
		}
	}
...
static switch_bool_t switch_core_media_bundle_audio_video_negotiated(switch_media_handle_t *smh)
{
	switch_bool_t negotiated;

	if (!smh) return SWITCH_FALSE;
	switch_mutex_lock(smh->sdp_mutex);
	negotiated = switch_core_media_bundle_has_media(&smh->bundle, SWITCH_MEDIA_TYPE_AUDIO) &&
		switch_core_media_bundle_has_media(&smh->bundle, SWITCH_MEDIA_TYPE_VIDEO) ? SWITCH_TRUE : SWITCH_FALSE;
	switch_mutex_unlock(smh->sdp_mutex);

	return negotiated;
}

static switch_status_t switch_core_media_reconcile_video_bundle(switch_core_session_t *session, switch_bool_t bundle_video,
												 switch_rtp_flag_t flags[SWITCH_RTP_FLAG_INVALID], const char **err)
{
	switch_media_handle_t *smh;
	switch_rtp_engine_t *a_engine, *v_engine;
	switch_rtp_t *target_rtp = NULL;
	switch_bool_t video_is_bundled;
	switch_bool_t target_created = SWITCH_FALSE;
	switch_port_t remote_rtcp_port;
	const char *rport;
	...
	a_engine = &smh->engines[SWITCH_MEDIA_TYPE_AUDIO];
	v_engine = &smh->engines[SWITCH_MEDIA_TYPE_VIDEO];
	video_is_bundled = v_engine->bundled_with_audio && a_engine->rtp_session &&
		v_engine->rtp_session == a_engine->rtp_session ? SWITCH_TRUE : SWITCH_FALSE;
	if (bundle_video == video_is_bundled && v_engine->bundled_with_audio == video_is_bundled) return SWITCH_STATUS_SUCCESS;

	if (v_engine->bundled_with_audio) {
		bundle_drain_thread_stop(session);
		switch_core_media_flush_queued_read_frames(v_engine);
		if (v_engine->bundle_write_state) switch_rtp_write_state_reset(v_engine->bundle_write_state);
		if (a_engine->rtp_session) {
	...
	if (bundle_video) {
		target_rtp = a_engine->rtp_session;
	} else if (v_engine->unbundled_rtp_session) {
		target_rtp = v_engine->unbundled_rtp_session;
	} else {
		target_rtp = switch_rtp_new(a_engine->local_sdp_ip,
		...
		if (!switch_rtp_ready(target_rtp)) {
			if (target_rtp) switch_rtp_destroy(&target_rtp);
			return SWITCH_STATUS_FALSE;
		}
		target_created = SWITCH_TRUE;
	}

	/* Frame paths hold this read lock for every transport operation. Swap the
	 * complete topology under the write lock so one operation cannot observe
	 * mixed old/new RTP ownership or a transient NULL session. */
	switch_thread_rwlock_wrlock(v_engine->dtls_init_rwlock);
	if (!bundle_video) {
		remote_rtcp_port = v_engine->remote_rtcp_port;
		if (!remote_rtcp_port &&
			(rport = switch_channel_get_variable(session->channel, "rtp_remote_video_rtcp_port"))) {
			remote_rtcp_port = (switch_port_t)atoi(rport);
		}
		if (switch_rtp_set_remote_address(target_rtp,
			v_engine->cur_payload_map->remote_sdp_ip,
			v_engine->cur_payload_map->remote_sdp_port,
			remote_rtcp_port, SWITCH_TRUE, err) != SWITCH_STATUS_SUCCESS) {
			switch_thread_rwlock_unlock(v_engine->dtls_init_rwlock);
			if (target_created) switch_rtp_destroy(&target_rtp);
			return SWITCH_STATUS_FALSE;
		}
	}
	if (bundle_video) {
		if (v_engine->rtp_session && v_engine->rtp_session != a_engine->rtp_session) {
			v_engine->unbundled_rtp_session = v_engine->rtp_session;
			switch_rtp_break(v_engine->unbundled_rtp_session);
		}
		v_engine->rtp_session = target_rtp;
		v_engine->bundled_with_audio = 1;
	} else {
		v_engine->rtp_session = target_rtp;
		v_engine->unbundled_rtp_session = NULL;
...
static void switch_core_media_filter_answer_bundle(switch_bundle_group_t *bundle, switch_bool_t accept_audio,
										 switch_bool_t accept_video, switch_bool_t accept_text)
{
	...
	for (i = 0; i < bundle->offered_mid_count; i++) {
		switch_bundle_mline_t *mline = switch_bundle_group_find_mline_by_mid(bundle, bundle->offered_mids[i]);
		switch_bool_t accepted = SWITCH_FALSE;

		if (!mline || mline->rejected) continue;
		if (mline->media_type == SWITCH_MEDIA_TYPE_AUDIO) accepted = accept_audio;
		else if (mline->media_type == SWITCH_MEDIA_TYPE_VIDEO) accepted = accept_video;
		else if (mline->media_type == SWITCH_MEDIA_TYPE_TEXT) accepted = accept_text;
		if (!accepted) continue;

		switch_copy_string(accepted_mids[accepted_count], bundle->offered_mids[i], sizeof(accepted_mids[accepted_count]));
		accepted_count++;
	}

	memset(bundle->offered_mids, 0, sizeof(bundle->offered_mids));
	bundle->offered_mid_count = accepted_count;
	bundle->bundle_tag_mline_index = -1;
	bundle->bundle_tag_mid[0] = '\0';
	for (i = 0; i < bundle->mline_count; i++) {
		bundle->mlines[i].in_bundle_group = 0;
		bundle->mlines[i].bundle_tag = 0;
	}
	for (i = 0; i < accepted_count; i++) {
		switch_copy_string(bundle->offered_mids[i], accepted_mids[i], sizeof(bundle->offered_mids[i]));
		if (!i) switch_copy_string(bundle->bundle_tag_mid, accepted_mids[i], sizeof(bundle->bundle_tag_mid));
		for (j = 0; j < bundle->mline_count; j++) {
			if (!strcmp(bundle->mlines[j].mid, accepted_mids[i])) {
				bundle->mlines[j].in_bundle_group = 1;
				if (!i) {
					bundle->mlines[j].bundle_tag = 1;
					bundle->bundle_tag_mline_index = bundle->mlines[j].mline_index;
				}
			}
		}
	}
...
static void switch_core_media_ignore_answer_mid(switch_mid_negotiation_t states[SWITCH_MEDIA_TYPE_TOTAL],
									 const switch_mid_negotiation_t offered_states[SWITCH_MEDIA_TYPE_TOTAL],
									 switch_bundle_group_t *bundle)
{
	uint8_t i;

	for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
		states[i].send_ext_id = 0;
		states[i].recv_ext_id = 0;
		states[i].mid[0] = '\0';
		if (offered_states && !zstr(offered_states[i].mid)) {
			switch_copy_string(states[i].mid, offered_states[i].mid, sizeof(states[i].mid));
		}
	}
	if (bundle) switch_bundle_group_init(bundle, bundle->policy);
}

static void switch_core_media_validate_answer_mid(switch_core_session_t *session, sdp_session_t *answer_sdp,
									 const char *answer_sdp_str,
									 switch_mid_negotiation_t states[SWITCH_MEDIA_TYPE_TOTAL], switch_bundle_group_t *answer_bundle)
{
	switch_media_handle_t *smh;
	sdp_parser_t *offer_parser = NULL;
	sdp_session_t *offer_sdp;
	sdp_media_t *offer_m, *answer_m;
	switch_mid_negotiation_t offered_states[SWITCH_MEDIA_TYPE_TOTAL] = { { 0 } };
	switch_bundle_group_t offered_bundle = { 0 };
	uint8_t i;
	int mline_index = 0;
	switch_bool_t invalid_mid = SWITCH_FALSE;

	if (!session || !(smh = session->media_handle) || !answer_sdp || !states || !answer_bundle ||
		zstr(smh->mparams->local_sdp_str) ||
		!(offer_parser = sdp_parse(NULL, smh->mparams->local_sdp_str, (int)strlen(smh->mparams->local_sdp_str), 0)) ||
		!(offer_sdp = sdp_session(offer_parser))) {
		if (offer_parser) sdp_parser_free(offer_parser);
		switch_core_media_ignore_answer_mid(states, NULL, answer_bundle);
		return;
	}

	switch_core_media_populate_mid_states_from_sdp(offer_sdp, smh->mparams->local_sdp_str, offered_states);

	offer_m = offer_sdp->sdp_media;
	answer_m = answer_sdp->sdp_media;
	while (offer_m && answer_m) {
		switch_media_type_t offer_type = switch_core_media_bundle_sdp_media_type(offer_m->m_type);
		switch_media_type_t answer_type = switch_core_media_bundle_sdp_media_type(answer_m->m_type);
		const char *offer_mid = switch_core_media_sdp_mid(offer_m->m_attributes);
		const char *answer_mid = switch_core_media_sdp_mid(answer_m->m_attributes);
		char raw_offer_mid[SWITCH_SDP_MID_MAX_LEN + 1] = "";
		char raw_answer_mid[SWITCH_SDP_MID_MAX_LEN + 1] = "";

		if (offer_type < SWITCH_MEDIA_TYPE_TOTAL && !offer_m->m_port && zstr(offer_mid)) {
			switch_core_media_raw_rejected_mid(smh->mparams->local_sdp_str, mline_index,
				raw_offer_mid, sizeof(raw_offer_mid));
			offer_mid = raw_offer_mid;
		}
		if (answer_type < SWITCH_MEDIA_TYPE_TOTAL && !answer_m->m_port && zstr(answer_mid)) {
			switch_core_media_raw_rejected_mid(answer_sdp_str, mline_index,
				raw_answer_mid, sizeof(raw_answer_mid));
			answer_mid = raw_answer_mid;
		}

		if (offer_type != answer_type ||
			((!zstr(offer_mid) || !zstr(answer_mid)) && (zstr(offer_mid) || zstr(answer_mid) || strcmp(offer_mid, answer_mid)))) {
			invalid_mid = SWITCH_TRUE;
			break;
		}
		offer_m = offer_m->m_next;
		answer_m = answer_m->m_next;
		mline_index++;
	}
	if (offer_m || answer_m) invalid_mid = SWITCH_TRUE;

	if (invalid_mid) {
		switch_core_media_ignore_answer_mid(states, offered_states, answer_bundle);
		sdp_parser_free(offer_parser);
		return;
	}

	for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
		uint8_t offered_send = offered_states[i].send_ext_id;
		uint8_t offered_recv = offered_states[i].recv_ext_id;

		if (states[i].send_ext_id && states[i].send_ext_id != offered_recv) {
			states[i].send_ext_id = 0;
		}
		if (states[i].recv_ext_id && states[i].recv_ext_id != offered_send) {
			states[i].recv_ext_id = 0;
		}
		if (zstr(offered_states[i].mid)) {
			states[i].send_ext_id = 0;
			states[i].recv_ext_id = 0;
			states[i].mid[0] = '\0';
		}
	...
	switch_core_media_bundle_populate_from_sdp(smh, offer_sdp, smh->mparams->local_sdp_str, SWITCH_TRUE, &offered_bundle);
	if (answer_bundle->state == SWITCH_BUNDLE_STATE_ACCEPTED) {
		if (offered_bundle.state != SWITCH_BUNDLE_STATE_ACCEPTED) {
			switch_bundle_group_init(answer_bundle, answer_bundle->policy);
		} else {
			for (i = 0; i < answer_bundle->offered_mid_count; i++) {
				if (!switch_core_media_bundle_has_mid(&offered_bundle, answer_bundle->offered_mids[i])) {
					switch_bundle_group_init(answer_bundle, answer_bundle->policy);
					break;
				}
			}
	...
	if (answer_bundle->state == SWITCH_BUNDLE_STATE_ACCEPTED &&
		switch_core_media_bundle_tag_media(answer_bundle) != SWITCH_MEDIA_TYPE_AUDIO) {
		/* This implementation shares video on the audio RTP transport only. Do not
		 * commit an answer that selected an unsupported non-audio transport owner. */
		switch_bundle_group_init(answer_bundle, answer_bundle->policy);
	}
...
SWITCH_DECLARE(uint8_t) switch_core_media_negotiate_sdp(switch_core_session_t *session, const char *r_sdp, uint8_t *proceed, switch_sdp_type_t sdp_type)
{
	...
	int got_video_crypto = 0, got_audio = 0, saw_audio = 0, saw_video = 0, got_avp = 0, got_savp = 0, got_udptl = 0, got_webrtc = 0, got_text = 0, got_text_crypto = 0, got_msrp = 0;
	int scrooge = 0;
	sdp_parser_t *parser = NULL;
	sdp_session_t *sdp;
	...
	if (sdp_type == SDP_OFFER) switch_core_media_clear_pending_mid(smh);
	...
	switch_core_media_populate_mid_states_from_sdp(sdp, r_sdp, mid_states);
...

	{
		...
		for (i = 0; smh->crypto_suite_order[i] != CRYPTO_INVALID; i++) {
			switch_rtp_crypto_key_type_t j = SUITES[smh->crypto_suite_order[i]].type;
			a_engine->ssec[j].remote_crypto_tag = SWITCH_REMOTE_CRYPTO_TAG_INVALID;
			v_engine->ssec[j].remote_crypto_tag = SWITCH_REMOTE_CRYPTO_TAG_INVALID;
			t_engine->ssec[j].remote_crypto_tag = SWITCH_REMOTE_CRYPTO_TAG_INVALID;
		}
	}
		...
		if (got_udptl && m->m_type == sdp_media_image) {
			...
			if (sdp->sdp_attributes) {
				for (attr = sdp->sdp_attributes; attr; attr = attr->a_next) {
					...
					if (!strcasecmp(attr->a_name, "group") && attr->a_value && !strncasecmp(attr->a_value, "BUNDLE", 6) &&
						candidate_bundle.state == SWITCH_BUNDLE_STATE_ACCEPTED) {
						if (switch_core_media_bundle_has_media(&candidate_bundle, SWITCH_MEDIA_TYPE_AUDIO)) {
							a_engine->rtcp_mux = 1;
						}
						if (v_engine && switch_core_media_bundle_has_media(&candidate_bundle, SWITCH_MEDIA_TYPE_VIDEO)) {
							v_engine->rtcp_mux = 1;
						}
						if (t_engine && switch_core_media_bundle_has_media(&candidate_bundle, SWITCH_MEDIA_TYPE_TEXT)) {
							t_engine->rtcp_mux = 1;
						}
					}
			...
			for (attr = m->m_attributes; attr; attr = attr->a_next) {
				if (!strcasecmp(attr->a_name, "rtcp") && attr->a_value && !skip_rtcp) {
					...
					maxptime = atoi(attr->a_value);
				} else if (got_crypto < 2 && !strcasecmp(attr->a_name, "crypto") && !zstr(attr->a_value)) {
					int crypto_tag;
					...
					if (switch_core_session_check_incoming_crypto(session, "rtp_has_crypto", SWITCH_MEDIA_TYPE_AUDIO, crypto, crypto_tag, sdp_type)) {
						got_crypto += 2;
					}
			...
			for (map = m->m_rtpmaps; map; map = map->rm_next) {
				...
				if (!switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) && !cng_pt && !strcasecmp(rm_encoding, "CN")) {
					cng_pt = smh->mparams->cng_pt = (switch_payload_t) map->rm_pt;
				...
				if (codec_ms && a_engine->read_impl.microseconds_per_packet && switch_true(switch_channel_get_variable(session->channel, "telnyx-strict-ptime"))) {
					if (codec_ms != a_engine->read_impl.microseconds_per_packet / 1000) {
						continue;
					}
				}
					...
					if (match) {
						if (scrooge) {
							...
							if (!switch_true(switch_channel_get_variable(session->channel, "telnyx-strict-ptime"))) {
								...
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
												  "Audio Codec Compare [%s:%d:%u:%d:%u:%d] is a near match but does not match our ptime\n",
												  imp->iananame, imp->ianacode, codec_rate, imp->microseconds_per_packet / 1000, bit_rate, imp->number_of_channels);
							}
							continue;
						}
...
//					++amrwb_offerings_rejected_n;
			...
			if (m_idx) {
				...
				for(j = 0; j < m_idx && smh->num_negotiated_codecs < SWITCH_MAX_CODECS; j++) {
					...
					if (j == 0) {
						...
						if (a_engine->read_impl.iananame && strcasecmp(a_engine->cur_payload_map->iananame, a_engine->read_impl.iananame)) {
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Assuming %s as the codec but we check further down for the previously negotiated one\n", a_engine->cur_payload_map->iananame);
						}
					}
			...
			if (match) {
				...
				if (a_engine->read_impl.iananame) {
					...
					if (switch_core_codec_ready(&a_engine->read_codec)) {
						switch_set_flag(&a_engine->read_codec, SWITCH_CODEC_FLAG_RESET_PENDING);
					}
					if (changed_pt) {
			...
			if (best_te) {
				const char* val = switch_channel_get_variable(session->channel, "dtmf_type");
				...
				if (!switch_false(switch_channel_get_variable(channel, "rtp_info_when_no_2833")) ||
					(!zstr(val) && strcasecmp(val, "info") == 0)) {
			...
			if (switch_core_media_bundle_has_media(&candidate_bundle, SWITCH_MEDIA_TYPE_VIDEO)) {
				got_video_rtcp_mux = 1;
				v_engine->rtcp_mux = 1;
			}
 ...
 t38_done:
	...
	if (match || vmatch || tmatch || fmatch) {
		if (sdp_type == SDP_OFFER) {
			switch_core_media_store_pending_mid(smh, mid_states, &candidate_bundle);
		} else if (sdp_type == SDP_ANSWER) {
			switch_core_media_commit_mid(session, mid_states, &candidate_bundle);
		}
	}
...
static void *SWITCH_THREAD_FUNC video_write_thread(switch_thread_t *thread, void *obj)
{
	...
	/* The per-frame write path holds dtls_init_rwlock while it uses RTP. */
	...
	if (v_engine->cur_payload_map && v_engine->cur_payload_map->remote_sdp_ip && v_engine->cur_payload_map->remote_sdp_port) {
		if (!strcmp(v_engine->cur_payload_map->remote_sdp_ip, rip) && atoi(rvp) == v_engine->cur_payload_map->remote_sdp_port) {
			...
			if (switch_rtp_ready(v_engine->rtp_session)) {
				...
				if (!scm_should_use_m_port(session, v_engine)) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Trickle ICE on: deferring video remote address from m= line\n");
				} else if (switch_rtp_set_remote_address(v_engine->rtp_session, v_engine->cur_payload_map->remote_sdp_ip,
	...
	if (t_engine->cur_payload_map && t_engine->cur_payload_map->remote_sdp_ip && t_engine->cur_payload_map->remote_sdp_port) {
		if (!strcmp(t_engine->cur_payload_map->remote_sdp_ip, rip) && atoi(rvp) == t_engine->cur_payload_map->remote_sdp_port) {
			...
			if (switch_rtp_ready(t_engine->rtp_session)) {
				...
				if (!scm_should_use_m_port(session, t_engine)) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Trickle ICE on: deferring text remote address from m= line\n"); 
				} else if (switch_rtp_set_remote_address(t_engine->rtp_session, t_engine->cur_payload_map->remote_sdp_ip,
...
SWITCH_DECLARE(void) switch_core_media_fork_do_fire_start_event(switch_core_session_t *session, switch_fork_state_t *fork, const char *fmr)
{
	...
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, "telnyx_media_stream::start") == SWITCH_STATUS_SUCCESS) {
		switch_channel_event_set_data(session->channel, event);
		switch_event_add_header(event, SWITCH_STACK_BOTTOM, "telnyx_media_streaming_start_time", "%" SWITCH_TIME_T_FMT, fork->start_time);
...
SWITCH_DECLARE(void) switch_core_media_fork_do_fire_stop_event(switch_core_session_t *session, switch_fork_state_t *fork, uint32_t last_duration)
{
	...
	if( switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, "telnyx_media_stream::stop") == SWITCH_STATUS_SUCCESS) {
		...
		switch_event_add_header(event, SWITCH_STACK_BOTTOM, "telnyx_media_streaming_stop_time", "%" SWITCH_TIME_T_FMT, fork->stop_time);
...
SWITCH_DECLARE(void) switch_core_media_deactivate_rtp(switch_core_session_t *session)
{
	...
	/* Stop drain thread before video engine teardown */
	bundle_drain_thread_stop(session);
	if (v_engine->bundled_with_audio && a_engine->rtp_session) {
		switch_rtp_clear_flag(a_engine->rtp_session, SWITCH_RTP_FLAG_BUNDLE_ACCEPT_ANY_PT);
	}
	...
	if (v_engine->unbundled_rtp_session == v_engine->rtp_session) {
		v_engine->unbundled_rtp_session = NULL;
	}
	if (v_engine->rtp_session) {
		if (v_engine->rtp_session == a_engine->rtp_session) {
			v_engine->rtp_session = NULL;
		} else {
			switch_rtp_destroy(&v_engine->rtp_session);
		}
	} else if (!v_engine->unbundled_rtp_session && v_engine->local_sdp_port) {
		switch_rtp_release_port(smh->mparams->rtpip, v_engine->local_sdp_port);
	}
	if (v_engine->unbundled_rtp_session) {
		switch_rtp_destroy(&v_engine->unbundled_rtp_session);
	}
...
static void gen_ice(switch_core_session_t *session, switch_media_type_t type, const char *ip, switch_port_t port)
{
	...
	if (engine->rtp_session && switch_core_media_trickle_enabled(session) && switch_rtp_trickle_is_registered(engine->rtp_session)) {
		...
		mline_index = (type == SWITCH_MEDIA_TYPE_AUDIO) ? 0 :  
						   (type == SWITCH_MEDIA_TYPE_VIDEO) ? 1 :
						   (type == SWITCH_MEDIA_TYPE_TEXT)  ? 2 : 0;
		if (engine->ice_out.cands[0][0].foundation) {
			switch_snprintf(cand.foundation, sizeof(cand.foundation), "%s", engine->ice_out.cands[0][0].foundation);
		}
		cand.component_id = engine->ice_out.cands[0][0].component_id ? engine->ice_out.cands[0][0].component_id : 1;
		switch_snprintf(cand.transport, sizeof(cand.transport), "%s", engine->ice_out.cands[0][0].transport ? engine->ice_out.cands[0][0].transport : "udp");
		cand.priority = engine->ice_out.cands[0][0].priority;
		switch_snprintf(cand.ip, sizeof(cand.ip), "%s", engine->ice_out.cands[0][0].con_addr ? engine->ice_out.cands[0][0].con_addr : "0.0.0.0");
		cand.port = (int)engine->ice_out.cands[0][0].con_port;
		switch_snprintf(cand.cand_type, sizeof(cand.cand_type), "%s", engine->ice_out.cands[0][0].cand_type ? engine->ice_out.cands[0][0].cand_type : "host");
		cand.rel_addr[0] = '\0';
		cand.rel_port = 0;
		switch_rtp_trickle_emit_local_candidate(engine->rtp_session, &cand, NULL /* mid */, mline_index, 0 /* not end */);
		switch_rtp_trickle_emit_local_candidate(engine->rtp_session, &cand, NULL /* mid */, mline_index, 1 /* end-of-candidates */);
	}
}
...

{
	...
	switch_bool_t bundle_video = SWITCH_FALSE;
	switch_bool_t reconcile_video = SWITCH_FALSE;
	...
	if (switch_channel_up(session->channel)) {
		...
		/* Register trickle ICE callback now that RTP session is created */
		if (trickle_is_enabled(session)) {
			const char *mid;
			int mline_index;
			mid = "0"; 
			mline_index = 0; /* force audio */
			switch_core_media_trickle_register(session, mid, mline_index);
		}
	...
	if (switch_rtp_ready(a_engine->rtp_session)) {
		...
		/* Prepare trickle ICE if enabled and not already prepared */
		if (trickle_accept_enabled(session) && a_engine->rtp_session) {
			switch_rtp_prepare_trickle_ice(a_engine->rtp_session, IPR_RTP, &a_engine->ice_in);
			if (!a_engine->rtcp_mux) {
				switch_rtp_prepare_trickle_ice(a_engine->rtp_session, IPR_RTCP, &a_engine->ice_in);
			}
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
								"%s Trickle ICE prepared for audio RTP session\n",
								switch_channel_get_name(switch_core_session_get_channel(session)));
		}
	...
	text:
		...
		if (switch_channel_test_flag(session->channel, CF_TEXT_POSSIBLE) && t_engine->cur_payload_map->rm_encoding && t_engine->cur_payload_map->remote_sdp_port) {
			...
			if (switch_rtp_ready(t_engine->rtp_session)) {
				...
				/* Prepare trickle ICE if enabled and not already prepared */
				if (trickle_accept_enabled(session)) {
					switch_rtp_prepare_trickle_ice(t_engine->rtp_session, IPR_RTP, &t_engine->ice_in);
					if (!t_engine->rtcp_mux) {
						switch_rtp_prepare_trickle_ice(t_engine->rtp_session, IPR_RTCP, &t_engine->ice_in);
					}
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
										"%s Trickle ICE prepared for text RTP session\n",
										switch_channel_get_name(switch_core_session_get_channel(session)));
				}
	...
	video:
		...
		if (switch_channel_test_flag(session->channel, CF_VIDEO_POSSIBLE) && v_engine->cur_payload_map->rm_encoding && v_engine->cur_payload_map->remote_sdp_port) {
			/******************************************************************************************/
			bundle_video = switch_core_media_bundle_audio_video_negotiated(smh) &&
				v_engine->rtcp_mux > 0 && a_engine->rtp_session ? SWITCH_TRUE : SWITCH_FALSE;
			reconcile_video = v_engine->bundled_with_audio != bundle_video ||
				(bundle_video && v_engine->rtp_session != a_engine->rtp_session) ||
				(!bundle_video && v_engine->rtp_session && v_engine->rtp_session == a_engine->rtp_session);
			if (!reconcile_video && v_engine->rtp_session && is_reinvite && switch_core_media_engine_owns_rtp(v_engine)) {
				//const char *ip = switch_channel_get_variable(session->channel, SWITCH_LOCAL_MEDIA_IP_VARIABLE);
				//const char *port = switch_channel_get_variable(session->channel, SWITCH_LOCAL_MEDIA_PORT_VARIABLE);
			...
			if (!reconcile_video && v_engine->rtp_session) {
				goto video_up;
			}
			...
			if (reconcile_video &&
				switch_core_media_reconcile_video_bundle(session, bundle_video, flags, &err) != SWITCH_STATUS_SUCCESS) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
					"Unable to reconcile video RTP transport topology: [%s]\n", switch_str_nil(err));
				switch_channel_hangup(session->channel, SWITCH_CAUSE_INCOMPATIBLE_DESTINATION);
				status = SWITCH_STATUS_FALSE;
				goto end;
			}
			
			/* Share transport only when both audio and video were accepted in BUNDLE. */
			if (bundle_video) {
				const char *video_ssrc;
				switch_rtp_set_bundle_has_video(a_engine->rtp_session, SWITCH_TRUE);
				if (a_engine->cur_payload_map) {
					/* capture local audio PT before video set_default_payload overwrites
				 ...
				 * bundle_frame_dup(). Do not use switch_frame_buffer_dup/free()
				...
				v_engine->bundled_with_audio = 0;
				if (!v_engine->rtp_session) v_engine->rtp_session = switch_rtp_new(a_engine->local_sdp_ip,
												 v_engine->local_sdp_port,
														 v_engine->cur_payload_map->remote_sdp_ip,
														 v_engine->cur_payload_map->remote_sdp_port,
														 v_engine->cur_payload_map->pt,
			...
			if (switch_rtp_ready(v_engine->rtp_session)) {
				const char *ssrc;
				switch_bool_t video_owns_rtp = switch_core_media_engine_owns_rtp(v_engine);

				switch_channel_set_flag(session->channel, CF_VIDEO);
				switch_core_session_start_video_thread(session);

				if (!video_owns_rtp) {
					goto video_transport_done;
				}
				...
				/* Prepare trickle ICE if enabled and not already prepared */
				if (trickle_accept_enabled(session)) {
					switch_rtp_prepare_trickle_ice(v_engine->rtp_session, IPR_RTP, &v_engine->ice_in);
					if (!v_engine->rtcp_mux) {
						switch_rtp_prepare_trickle_ice(v_engine->rtp_session, IPR_RTCP, &v_engine->ice_in);
					}
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
										"%s Trickle ICE prepared for video RTP session\n",
										switch_channel_get_name(switch_core_session_get_channel(session)));
				}
 ...
 video_up:
	...
	if (session && v_engine && switch_core_media_engine_owns_rtp(v_engine)) {
	...
	switch_core_media_sync_mid_state(session);
...
static void switch_core_media_append_mid_extmaps(char *buf, size_t buflen, uint8_t send_ext_id, uint8_t recv_ext_id)
{
	if (!buf) return;

	if (send_ext_id && send_ext_id == recv_ext_id && send_ext_id <= 14) {
		switch_snprintf(buf + strlen(buf), buflen - strlen(buf),
			"a=extmap:%u urn:ietf:params:rtp-hdrext:sdes:mid\r\n", send_ext_id);
	} else {
		if (send_ext_id && send_ext_id <= 14) {
			switch_snprintf(buf + strlen(buf), buflen - strlen(buf),
				"a=extmap:%u/sendonly urn:ietf:params:rtp-hdrext:sdes:mid\r\n", send_ext_id);
		}
		if (recv_ext_id && recv_ext_id <= 14) {
			switch_snprintf(buf + strlen(buf), buflen - strlen(buf),
				"a=extmap:%u/recvonly urn:ietf:params:rtp-hdrext:sdes:mid\r\n", recv_ext_id);
		}
	}
}
...
{
	const rtp_extension_t *send_ext = NULL, *recv_ext = NULL;

	if (send_ext_id) *send_ext_id = 0;
	if (recv_ext_id) *recv_ext_id = 0;
	if (!engine) return;

	if (use_pending && pending) {
		if (!zstr(pending->mid) && strlen(pending->mid) <= SWITCH_RTP_MID_MAX_LEN) {
			if (send_ext_id) *send_ext_id = pending->send_ext_id;
			if (recv_ext_id) *recv_ext_id = pending->recv_ext_id;
		}
		return;
	}
	...
	send_ext = switch_core_media_get_send_extension_by_type(engine, SWITCH_MEDIA_EXTENSION_MID);
	recv_ext = switch_core_media_get_recv_extension_by_type(engine, SWITCH_MEDIA_EXTENSION_MID);
	if (send_ext_id && send_ext) *send_ext_id = send_ext->id;
	if (recv_ext_id && recv_ext) *recv_ext_id = recv_ext->id;
}
...
static void generate_m(switch_core_session_t *session, char *buf, size_t buflen,
							   ...
							   switch_sdp_type_t sdp_type, const switch_bundle_group_t *bundle, int *audio_mid_emitted,
							   uint8_t mid_send_ext_id, uint8_t mid_recv_ext_id, const char *negotiated_mid)
{
	int i = 0;
	int rate;
	...
	if (secure && (!switch_channel_test_flag(session->channel, CF_DTLS) || a_engine->local_dtls_fingerprint.type)) {
	...
	switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "\r\n");
	if (mid_send_ext_id || mid_recv_ext_id) {
		switch_core_media_append_mid_extmaps(buf, buflen, mid_send_ext_id, mid_recv_ext_id);
	}
	...
	if ((!audio_mid_emitted || !*audio_mid_emitted) &&
		((sdp_type == SDP_ANSWER && !zstr(negotiated_mid)) ||
		 (sdp_type != SDP_ANSWER && switch_core_media_bundle_should_offer(smh, bundle))) &&
		!switch_channel_var_true(session->channel, "rtp_no_audio_mid") &&
		!switch_channel_var_true(session->channel, "rtp_no_attr_mid")) {
		audio_mid = sdp_type == SDP_ANSWER ? negotiated_mid :
			switch_channel_get_variable_dup(session->channel, "rtp_audio_mid", SWITCH_FALSE, -1);
	...
	if (smh->mparams->rtcp_audio_interval_msec) {
		if (a_engine->rtcp_mux > 0 || switch_core_media_bundle_has_media(bundle, SWITCH_MEDIA_TYPE_AUDIO)) {
	...
	if (a_engine->ice_out.cands[0][0].ready) {
		...
		if (switch_channel_test_flag(session->channel, CF_REINVITE)) {
			a_engine->ssrc = switch_rtp_get_new_ssrc(a_engine->rtp_session);
		}
	...
	if (switch_core_media_trickle_enabled(session)) {
		switch_snprintf(buf + strlen(buf), buflen - strlen(buf), "a=ice-options:trickle\r\n");
	}
...
SWITCH_DECLARE(void) switch_core_media_gen_local_sdp(switch_core_session_t *session, switch_sdp_type_t sdp_type, const char *ip, switch_port_t port, const char *sr, int force)
{
	...
	char srbuf[128] = "";
	char groupbuf[(SWITCH_BUNDLE_MAX_MIDS * (SWITCH_BUNDLE_MAX_MID_LEN + 2)) + 20] = "";
	const char *var_val;
	const char *username;
	const char *fmtp_out;
	...
	const char* text_mid = switch_channel_get_variable_dup(session->channel, "rtp_text_mid", SWITCH_FALSE, -1);
	...
	v_engine = &smh->engines[SWITCH_MEDIA_TYPE_VIDEO];
	t_engine = &smh->engines[SWITCH_MEDIA_TYPE_TEXT];
	sdp_bundle = &smh->bundle;
	if (sdp_type == SDP_ANSWER) {
		pending_mid_answer = switch_core_media_snapshot_pending_mid(smh, pending_mid_states, &pending_bundle);
		if (pending_mid_answer) sdp_bundle = &pending_bundle;
		if (pending_mid_answer) {
			audio_mid = zstr(pending_mid_states[SWITCH_MEDIA_TYPE_AUDIO].mid) ? NULL : pending_mid_states[SWITCH_MEDIA_TYPE_AUDIO].mid;
			video_mid = zstr(pending_mid_states[SWITCH_MEDIA_TYPE_VIDEO].mid) ? NULL : pending_mid_states[SWITCH_MEDIA_TYPE_VIDEO].mid;
			text_mid = zstr(pending_mid_states[SWITCH_MEDIA_TYPE_TEXT].mid) ? NULL : pending_mid_states[SWITCH_MEDIA_TYPE_TEXT].mid;
		}
		for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
			switch_core_media_answer_mid_extmaps(&smh->engines[i], &pending_mid_states[i], pending_mid_answer,
				&sdp_mid_send_ext_ids[i], &sdp_mid_recv_ext_ids[i]);
		}
	}
	...
	trickle = trickle_is_enabled(session);
	...
	if (!force && !ip && zstr(sr)
		...
		if (sdp_type == SDP_ANSWER && pending_mid_answer) switch_core_media_clear_pending_mid(smh);
	...
	if (zstr(sr)) {
		...
		if ((var_val = switch_channel_get_variable(session->channel, "origination_audio_mode"))) {
			if (!strcasecmp(var_val, "sendonly") || !strcasecmp(var_val, "recvonly") || !strcasecmp(var_val, "sendrecv") || !strcasecmp(var_val, "inactive")) {
				sr = var_val;
			}
	...
	if (sdp_type == SDP_OFFER) {
		...
		if (offer_text_media && !t_engine->local_sdp_port &&
			switch_core_media_choose_port(session, SWITCH_MEDIA_TYPE_TEXT, 0) != SWITCH_STATUS_SUCCESS) {
			offer_text_media = SWITCH_FALSE;
		}
		if (offer_text_media && !t_engine->adv_sdp_port) offer_text_media = SWITCH_FALSE;
	}
	if (sdp_type == SDP_OFFER && switch_channel_test_flag(session->channel, CF_ICE)) {
		int configured_mid_ext_id = 1;
		...
		switch_mutex_lock(smh->sdp_mutex);
		for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
			const rtp_extension_t *send_mid_ext, *recv_mid_ext;
			switch_rtp_engine_t *engine;

			if (!offered_mid_media[i]) continue;
			engine = &smh->engines[i];
			send_mid_ext = switch_core_media_get_send_extension_by_type(engine, SWITCH_MEDIA_EXTENSION_MID);
			recv_mid_ext = switch_core_media_get_recv_extension_by_type(engine, SWITCH_MEDIA_EXTENSION_MID);
			if (send_mid_ext && send_mid_ext->id >= 1 && send_mid_ext->id <= 14) sdp_mid_send_ext_ids[i] = send_mid_ext->id;
			if (recv_mid_ext && recv_mid_ext->id >= 1 && recv_mid_ext->id <= 14) sdp_mid_recv_ext_ids[i] = recv_mid_ext->id;
			if (!sdp_mid_send_ext_ids[i] && engine->active_mid_send_ext_id >= 1 && engine->active_mid_send_ext_id <= 14) {
				sdp_mid_send_ext_ids[i] = engine->active_mid_send_ext_id;
			}
		}
		switch_mutex_unlock(smh->sdp_mutex);
		...
		for (i = 0; i < SWITCH_MEDIA_TYPE_TOTAL; i++) {
			if (!offered_mid_media[i]) continue;
			if (!sdp_mid_send_ext_ids[i] && !sdp_mid_recv_ext_ids[i]) {
				sdp_mid_send_ext_ids[i] = sdp_mid_recv_ext_ids[i] = (uint8_t)configured_mid_ext_id;
			}
			if (!have_session_mid_extmap) {
				offer_session_mid_send_ext_id = sdp_mid_send_ext_ids[i];
				offer_session_mid_recv_ext_id = sdp_mid_recv_ext_ids[i];
				have_session_mid_extmap = SWITCH_TRUE;
			} else if (offer_session_mid_send_ext_id != sdp_mid_send_ext_ids[i] ||
				offer_session_mid_recv_ext_id != sdp_mid_recv_ext_ids[i]) {
				offer_mid_extmaps_media_scoped = SWITCH_TRUE;
			}
		}

		if (!offer_mid_extmaps_media_scoped) {
			memset(sdp_mid_send_ext_ids, 0, sizeof(sdp_mid_send_ext_ids));
	...
	if (sdp_type == SDP_ANSWER && pending_mid_answer) {
		switch_core_media_filter_answer_bundle(&pending_bundle,
			...
			!bundle_no_attr_mid && !bundle_no_video_mid && !zstr(video_mid) && bundle_has_vid,
			SWITCH_FALSE);
	}

	if (sdp_type == SDP_ANSWER && sdp_bundle->state == SWITCH_BUNDLE_STATE_ACCEPTED) {
		uint32_t bundle_mid_index;

		switch_snprintf(groupbuf, sizeof(groupbuf), "a=group:BUNDLE");
		for (bundle_mid_index = 0; bundle_mid_index < sdp_bundle->offered_mid_count; bundle_mid_index++) {
		...
		else switch_snprintf(groupbuf, sizeof(groupbuf), "a=group:BUNDLE %s\r\n", bundle_audio_mid);
	}
	...
	switch_snprintf(buf, SDPBUFLEN,
					...
					"%s"
					"%s",
					username, smh->owner_id, smh->session_id, family, ip, username, family, ip, srbuf, groupbuf);
	...
	/* Only advertise ICE in SDP if:
	 *  - the channel is secure (DTLS or SDES), OR
	 *  - the admin explicitly forces it with channel var:
	 *    advertise_ice_when_remote_absent=true
	 *  This avoids emitting ICE attributes for non-ICE/non-secure clients. */
	if (switch_channel_test_flag(smh->session->channel, CF_ICE)) {
		const char *adv_ice = switch_channel_get_variable(session->channel, "advertise_ice_when_remote_absent");
		switch_bool_t force_advertise = switch_true(adv_ice);
		if (switch_core_media_has_crypto(session) || force_advertise) {
			gen_ice(session, SWITCH_MEDIA_TYPE_AUDIO, ip, port);
			switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf),
			                "a=msid-semantic: WMS %s\r\n", smh->msid);
		}

		/* RFC 8838/8840: Advertise Trickle support when enabled */
		if (use_trickle) {
			switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=ice-options:trickle\r\n");
		}
		/* RTP header extensions compatibility flag (JSEP/Unified Plan) */
		switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=extmap-allow-mixed\r\n");

		if (sdp_type == SDP_OFFER && !offer_mid_extmaps_media_scoped) {
			switch_core_media_append_mid_extmaps(buf, SDPBUFLEN,
				offer_session_mid_send_ext_id, offer_session_mid_recv_ext_id);
		}
	}

	if (a_engine->codec_negotiated && !switch_channel_test_flag(session->channel, CF_NOSDP_REINVITE)) {
		if (switch_core_media_bundle_has_media(sdp_bundle, SWITCH_MEDIA_TYPE_AUDIO) && a_engine->rtcp_mux < 1) {
			a_engine->rtcp_mux = 1;
		}
		...
		if (sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_AUDIO] || sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_AUDIO]) {
			switch_core_media_append_mid_extmaps(buf, SDPBUFLEN,
				sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_AUDIO]);
		}
		...
		if (switch_channel_var_true(session->channel, "rtp_no_audio_mid") || switch_channel_var_true(session->channel, "rtp_no_attr_mid")) {
			if (sdp_type != SDP_ANSWER || !pending_mid_answer) switch_channel_set_variable(session->channel, "rtp_audio_mid", NULL);
		} else if (sdp_type != SDP_ANSWER || !pending_mid_answer || !zstr(audio_mid)) {
			switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=mid:%s\r\n", !zstr(audio_mid) ? audio_mid : "audio");
			if ((sdp_type != SDP_ANSWER || !pending_mid_answer) && zstr(audio_mid)) {
		...
		if (a_engine->ice_out.cands[0][0].ready) {
			...
			if (trickle) {
			/* advertise trickle capability */
				switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=ice-options:trickle\r\n");
			}
		}

		if (a_engine->crypto_type != CRYPTO_INVALID && (!switch_channel_test_flag(session->channel, CF_DTLS) || a_engine->local_dtls_fingerprint.type) &&
		...
		if (switch_channel_test_flag(session->channel, CF_AVPF) || (mult && switch_false(mult))) {
			...
			if ((!a_engine->no_crypto && switch_channel_test_flag(session->channel, CF_SECURE)) ||
				switch_channel_test_flag(session->channel, CF_DTLS)) {
				generate_m(session, buf, SDPBUFLEN, port, family, ip, 0, append_audio, sr, use_cng, cng_type, map, 1, sdp_type, sdp_bundle,
					&audio_mid_emitted, sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], audio_mid);
				bp = (buf + strlen(buf));
			...
			if (both) {
				generate_m(session, bp, SDPBUFLEN - strlen(buf), port, family, ip, 0, append_audio, sr, use_cng, cng_type, map, 0, sdp_type, sdp_bundle,
					&audio_mid_emitted, sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], audio_mid);
			}
			...
			for (i = 0; i < smh->mparams->num_codecs; i++) {
				...
				if (cur_ptime != this_ptime) {
					...
					if ((!a_engine->no_crypto && switch_channel_test_flag(session->channel, CF_SECURE)) ||
						switch_channel_test_flag(session->channel, CF_DTLS)) {
						generate_m(session, bp, SDPBUFLEN - strlen(buf), port, family, ip, cur_ptime, append_audio, sr, use_cng, cng_type, map, 1, sdp_type, sdp_bundle,
							&audio_mid_emitted, sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], audio_mid);
						bp = (buf + strlen(buf));
					...
					if (both) {
						generate_m(session, bp, SDPBUFLEN - strlen(buf), port, family, ip, cur_ptime, append_audio, sr, use_cng, cng_type, map, 0, sdp_type, sdp_bundle,
							&audio_mid_emitted, sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_AUDIO], audio_mid);
					}
				}
 ...
 video:
		...
		if (switch_channel_test_flag(session->channel, CF_VIDEO_SDP_RECVD)) {
			...
			switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "m=video 0 %s 19\r\n",
							get_media_profile_name(session,
											   ...
											   v_engine->crypto_type != CRYPTO_INVALID || switch_channel_test_flag(session->channel, CF_DTLS), AVP_UNDEFINED));
			if (sdp_type == SDP_ANSWER && pending_mid_answer && !zstr(video_mid)) {
				switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=mid:%s\r\n", video_mid);
			}
		}
	} else {
		if (switch_channel_direction(session->channel) == SWITCH_CALL_DIRECTION_INBOUND) {
			if (switch_channel_test_flag(smh->session->channel, CF_DTLS) && !v_engine->local_dtls_fingerprint.type) {
		...
		if ((v_port = v_engine->adv_sdp_port)) {
			...
			for (loops = 0; loops < 2; loops++) {
				...
				if (sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_VIDEO] || sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_VIDEO]) {
					switch_core_media_append_mid_extmaps(buf, SDPBUFLEN,
						sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_VIDEO], sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_VIDEO]);
				}
				...
				if (switch_channel_var_true(session->channel, "rtp_no_video_mid") || switch_channel_var_true(session->channel, "rtp_no_attr_mid")) {
					if (sdp_type != SDP_ANSWER || !pending_mid_answer) switch_channel_set_variable(session->channel, "rtp_video_mid", NULL);
				} else if (sdp_type != SDP_ANSWER || !pending_mid_answer || !zstr(video_mid)) {
				...
				if (loops == 0 && switch_channel_test_flag(session->channel, CF_SECURE) && (!switch_channel_test_flag(session->channel, CF_DTLS) || v_engine->local_dtls_fingerprint.type)) {
	...
	if (sdp_type == SDP_ANSWER && !switch_channel_test_flag(session->channel, CF_RTT)) {
		if (switch_channel_test_flag(session->channel, CF_TEXT_SDP_RECVD)) {
			...
			switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "m=text 0 %s 19\r\n",
							get_media_profile_name(session,
											   (switch_channel_test_flag(session->channel, CF_SECURE)
												&& switch_channel_direction(session->channel) == SWITCH_CALL_DIRECTION_OUTBOUND) ||
											   t_engine->crypto_type != CRYPTO_INVALID || switch_channel_test_flag(session->channel, CF_DTLS), AVP_UNDEFINED));
			if (pending_mid_answer && !zstr(text_mid)) {
				switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=mid:%s\r\n", text_mid);
			}
		}
	} else if ((sdp_type == SDP_OFFER && offer_text_media) ||
			   (sdp_type != SDP_OFFER &&
				(switch_channel_test_flag(session->channel, CF_WANT_RTT) || switch_channel_test_flag(session->channel, CF_RTT) ||
		...
		if (switch_channel_direction(session->channel) == SWITCH_CALL_DIRECTION_INBOUND) {
			if (switch_channel_test_flag(smh->session->channel, CF_DTLS) && !t_engine->local_dtls_fingerprint.type) {
		...
		if ((t_port = t_engine->adv_sdp_port)) {
			...
			for (loops = 0; loops < 2; loops++) {
				...
				switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "\r\n");
				if (sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_TEXT] || sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_TEXT]) {
					switch_core_media_append_mid_extmaps(buf, SDPBUFLEN,
						sdp_mid_send_ext_ids[SWITCH_MEDIA_TYPE_TEXT], sdp_mid_recv_ext_ids[SWITCH_MEDIA_TYPE_TEXT]);
				}
				if (pending_mid_answer && !zstr(text_mid)) {
					switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=mid:%s\r\n", text_mid);
				}
				...
				if (loops == 0 && switch_channel_test_flag(session->channel, CF_SECURE) && (!switch_channel_test_flag(session->channel, CF_DTLS) || t_engine->local_dtls_fingerprint.type)) {
	...
	if (sdp_type == SDP_ANSWER && pending_mid_answer) {
		switch_core_media_commit_mid(session, pending_mid_states, &pending_bundle);
	}
...
SWITCH_DECLARE (void) switch_core_media_recover_session(switch_core_session_t *session, const char* force_local_ip, const char* force_advertized_ip, switch_bool_t change_port)
{
	...
	old_port = switch_channel_get_variable_dup(session->channel, SWITCH_LOCAL_MEDIA_PORT_VARIABLE, SWITCH_FALSE, -1);
	
	if (force_local_ip) {
		switch_channel_set_variable(session->channel, SWITCH_LOCAL_MEDIA_IP_VARIABLE, force_local_ip);
	}
	
	if (force_advertized_ip) {
		switch_channel_set_variable(session->channel, SWITCH_ADVERTISED_MEDIA_IP_VARIABLE, force_advertized_ip);
	}
	
	ip = switch_channel_get_variable(session->channel, SWITCH_LOCAL_MEDIA_IP_VARIABLE);

	if (switch_channel_test_flag(session->channel, CF_PROXY_MODE)  || !ip) {
		return;
	} else {
		a_ip = force_advertized_ip ? force_advertized_ip : switch_channel_get_variable(session->channel, SWITCH_ADVERTISED_MEDIA_IP_VARIABLE);
	...
	if (change_port) {
		port = switch_rtp_request_port(ip);
	} else {
		port = (switch_port_t)atoi(old_port);
	}
	switch_channel_set_variable_printf(session->channel, SWITCH_LOCAL_MEDIA_PORT_VARIABLE, "%d", port);
	...
	a_engine->adv_sdp_port = a_engine->local_sdp_port = port;
	a_engine->codec_negotiated = 1;
	...
	if (switch_channel_test_flag(session->channel, CF_VIDEO)) {
		...
		v_engine->adv_sdp_port = v_engine->local_sdp_port = port;
																	  ...
																	  int stream_id)
{
	...
	if (!(switch_channel_test_flag(session->channel, CF_VIDEO_READY) || (flags & SWITCH_IO_FLAG_FORCE))) {
		/* On a BUNDLE leg, the shared transport (ICE+DTLS) is already up via
																...
																int stream_id)
{
	...
	if (frame->codec) {
		...
		case SWITCH_STATUS_SUCCESS:
			...
			if (!session->raw_write_frame.rate) {
	...
	if (session->write_codec) {
		...
		if (perfect) {
			...
			if (write_frame->datalen < session->write_impl.decoded_bytes_per_packet) {
				memset((uint8_t *)write_frame->data + write_frame->datalen, 255, session->write_impl.decoded_bytes_per_packet - write_frame->datalen);
			...
			while (switch_buffer_inuse(session->raw_write_buffer) >= session->write_impl.decoded_bytes_per_packet) {
				...
				case SWITCH_STATUS_SUCCESS:
					...
					session->enc_write_frame.payload = session->write_impl.ianacode;
					session->enc_write_frame.flags = 0;
					if (!session->enc_write_frame.rate) {
						session->enc_write_frame.rate = session->write_impl.actual_samples_per_second;
					}
				...
				case SWITCH_STATUS_NOOP:
					...
					enc_frame->payload = enc_frame->codec->implementation->ianacode;
					if (!enc_frame->rate) {
						enc_frame->rate = session->write_impl.actual_samples_per_second;
					}
...
{
	return (switch_channel_test_flag(session->channel, CF_DTLS) ||
			switch_channel_test_flag(session->channel, CF_SECURE) ||
			switch_channel_test_flag(session->channel, CF_AVPF_MOZ));
}

static switch_status_t switch_core_media_set_extension(rtp_extension_t *extensions, uint8_t id, uint8_t type)
{
	uint8_t i = 0;
	...
	while (i < MAX_RTP_EXTENSIONS) {

		if (extensions[i].id == 0 || extensions[i].type == type) {
			extensions[i].id = id;
			extensions[i].type = type;
			return SWITCH_STATUS_SUCCESS;
		}
		++i;
	}
...
SWITCH_DECLARE(switch_status_t) switch_core_media_set_recv_extension(switch_rtp_engine_t *engine, uint8_t id, uint8_t ext_type)
{
	...
	return switch_core_media_set_extension(engine->rtp_recv_extensions, id, ext_type);
}
...
SWITCH_DECLARE(switch_status_t) switch_core_media_set_send_extension(switch_rtp_engine_t *engine, uint8_t id, uint8_t ext_type)
{
	...
	return switch_core_media_set_extension(engine->rtp_send_extensions, id, ext_type);
}

static rtp_extension_t* switch_core_media_get_extension_by_id(rtp_extension_t *extensions, uint8_t id)
{
	uint8_t i = 0;
...
{
	if (!engine) return NULL;
	return switch_core_media_get_extension_by_id(engine->rtp_recv_extensions, id);
}

SWITCH_DECLARE(rtp_extension_t *) switch_core_media_get_send_extension_by_id(switch_rtp_engine_t *engine, uint8_t id)
{
	if (!engine) return NULL;
	return switch_core_media_get_extension_by_id(engine->rtp_send_extensions, id);
}

static rtp_extension_t* switch_core_media_get_extension_by_type(rtp_extension_t *extensions, uint8_t type)
{
	uint8_t i = 0;
...
{
	if (!engine) return NULL;
	return switch_core_media_get_extension_by_type(engine->rtp_recv_extensions, type);
}

SWITCH_DECLARE(rtp_extension_t *) switch_core_media_get_send_extension_by_type(switch_rtp_engine_t *engine, uint8_t type)
{
	if (!engine) return NULL;
	return switch_core_media_get_extension_by_type(engine->rtp_send_extensions, type);
}

static void scm_on_local_candidate(void *user_data, const char *mid, int mline_index, const switch_rtp_ice_cand_t *cand, int end_of_candidates)
{
	switch_core_session_t *session;
	switch_event_t *ev;
	switch_channel_t *channel;
	...
	if (cand->rel_port > 0) {
		switch_snprintf(rport_str, sizeof(rport_str), "%d", cand->rel_port);
	}

	wrote = switch_snprintf(cand_line, sizeof(cand_line),
							"a=candidate:%s %d %s %lu %s %d typ %s%s%s%s%s",
							cand->foundation,
							cand->component_id,
							cand->transport,
							cand->priority,
							...
							cand->port,
							cand->cand_type,
							cand->rel_addr[0] ? " raddr " : "",
							cand->rel_addr,
							cand->rel_port ? " rport " : "",
							cand->rel_port ? rport_str : "");
	(void)wrote;

	if (switch_event_create_subclass(&ev, SWITCH_EVENT_CUSTOM, TRICKLE_EVENT_SUBCLASS) == SWITCH_STATUS_SUCCESS) {
		switch_event_add_header_string(ev, SWITCH_STACK_BOTTOM, "Call-UUID", switch_core_session_get_uuid(session));
		switch_event_add_header_string(ev, SWITCH_STACK_BOTTOM, "a-candidate", cand_line);
		switch_event_add_header_string(ev, SWITCH_STACK_BOTTOM, "mid", mid ? mid : "");
		switch_event_add_header(ev, SWITCH_STACK_BOTTOM, "mline-index", "%d", mline_index);
		switch_event_add_header(ev, SWITCH_STACK_BOTTOM, "end-of-candidates", "%d", end_of_candidates ? 1 : 0);
		switch_event_fire(&ev);
	}
}

static switch_media_type_t scm_type_from_mindex(int mline_index, const char *mid)
{
	if (mid && *mid) {
		if (!strcmp(mid, "audio") || !strcmp(mid, "0")) return SWITCH_MEDIA_TYPE_AUDIO;
		if (!strcmp(mid, "video") || !strcmp(mid, "1")) return SWITCH_MEDIA_TYPE_VIDEO;
	}

	if (mline_index == 0) return SWITCH_MEDIA_TYPE_AUDIO;
	if (mline_index == 1) return SWITCH_MEDIA_TYPE_VIDEO;
	return SWITCH_MEDIA_TYPE_AUDIO; /* safe default */
}
...
SWITCH_DECLARE(void) switch_core_media_trickle_register(switch_core_session_t *session, const char *mid, int mline_index)
{
	switch_media_type_t type;
	switch_rtp_t *rtp;
	type = scm_type_from_mindex(mline_index, mid);
	rtp  = switch_core_media_get_rtp_session(session, type);

	if (rtp) {
		switch_rtp_set_ice_candidate_cb(rtp, scm_on_local_candidate, (void *)session);
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
			"Trickle ICE callback registered for %s media (mid=%s, mline=%d)\n",
			type == SWITCH_MEDIA_TYPE_AUDIO ? "audio" : 
			type == SWITCH_MEDIA_TYPE_VIDEO ? "video" : "text",
			mid ? mid : "(null)", mline_index);
	} else {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
			"Failed to register trickle ICE callback - no RTP session for %s media (mid=%s, mline=%d)\n",
			type == SWITCH_MEDIA_TYPE_AUDIO ? "audio" : 
			type == SWITCH_MEDIA_TYPE_VIDEO ? "video" : "text",
			mid ? mid : "(null)", mline_index);
	}
...
SWITCH_DECLARE(switch_status_t) switch_core_media_trickle_add_remote_candidate(switch_core_session_t *session, int mline_index, const char *mid, const char *cand_line)
{
	...
	type = scm_type_from_mindex(mline_index, mid);
	rtp  = switch_core_media_get_rtp_session(session, type);
	if (!rtp) return SWITCH_STATUS_FALSE;
	return switch_rtp_add_trickle_remote_candidate(rtp, mline_index, mid, cand_line);
}
...
SWITCH_DECLARE(switch_status_t) switch_core_media_trickle_end_of_candidates(switch_core_session_t *session, int mline_index, const char *mid)
{
	...
	rtp  = switch_core_media_get_rtp_session(session, type);
	if (!rtp) return SWITCH_STATUS_FALSE;
	return switch_rtp_trickle_end_of_candidates(rtp, mline_index, mid);
}

SWITCH_DECLARE(int) switch_core_media_trickle_enabled(switch_core_session_t *session) { return trickle_is_enabled(session); }
SWITCH_DECLARE(int) switch_core_media_trickle_accept(switch_core_session_t *session) { return trickle_accept_enabled(session); }
...
SWITCH_DECLARE(switch_status_t) switch_core_media_add_trickle_candidate(switch_core_session_t *session, int mline_index, const char *mid, const char *raw_candidate)
{
	...
	{
		switch_media_type_t mtype = SWITCH_MEDIA_TYPE_AUDIO;

		if ((mid && !zstr(mid) && !strcasecmp(mid, "video")) || mline_index == 1) {
			mtype = SWITCH_MEDIA_TYPE_VIDEO;
		}

		rtp = switch_core_media_get_rtp_session(session, mtype);
	}

	if (!rtp) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING, "Trickle ICE: no RTP found for m=%d mid=%s\n", mline_index, switch_str_nil(mid));
		return SWITCH_STATUS_FALSE;
	}

	return switch_rtp_add_trickle_remote_candidate(rtp, mline_index, mid, raw_candidate);
}
...
SWITCH_DECLARE(void) switch_core_media_trickle_recheck_all(switch_core_session_t *session, void *sdp_session, switch_sdp_type_t sdp_type)
{
	...
	sdp_session_t *parsed_sdp = (sdp_session_t*)sdp_session;
	
	if (!smh || !parsed_sdp) return;

	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, 
		"Trickle recheck: Using provided parsed SDP session\n");
	
	/* Find media descriptors for each media type and call check_ice */
	for (sdp_media_t *m = parsed_sdp->sdp_media; m; m = m->m_next) {
		if (m->m_type == sdp_media_audio) {
			check_ice(smh, SWITCH_MEDIA_TYPE_AUDIO, parsed_sdp, m, sdp_type);
		} else if (m->m_type == sdp_media_video) {
			check_ice(smh, SWITCH_MEDIA_TYPE_VIDEO, parsed_sdp, m, sdp_type);
		} else if (m->m_type == sdp_media_text) {
			check_ice(smh, SWITCH_MEDIA_TYPE_TEXT, parsed_sdp, m, sdp_type);
		}
	}
}

static void trickle_maybe_recheck_ice(switch_core_session_t *session, switch_media_handle_t *smh, void *sdp_session, switch_sdp_type_t sdp_type, int force_now)
{
	switch_channel_t *chan;
	uint64_t now_us, last_us = 0;
	const char *last_s;
	sdp_session_t *parsed_sdp = (sdp_session_t*)sdp_session;

	if (!session || !smh || !parsed_sdp) return;

	chan = switch_core_session_get_channel(session);
	now_us = switch_micro_time_now();
	last_s = chan ? switch_channel_get_variable(chan, "trickle_last_check_us") : NULL;
	if (last_s) last_us = (uint64_t) strtoull(last_s, NULL, 10);

	if (!force_now && last_us && (now_us - last_us) < TRICKLE_RECHECK_DEBOUNCE_US) {
		return;
	}
	...
	if (chan) {
		switch_channel_set_variable_printf(chan, "trickle_last_check_us", "%" PRIu64, now_us);
	}
}

SWITCH_DECLARE(switch_status_t) switch_core_media_trickle_remote_candidate_and_check(switch_core_session_t *session, 
									switch_media_handle_t *smh, void *sdp_session, switch_sdp_type_t sdp_type,
									const char *mid, int mline_index,
									const char *cand_line, int end_of_candidates)
{
	switch_status_t st = switch_core_media_trickle_remote_candidate(session, mid, mline_index, cand_line, end_of_candidates);
	if (st == SWITCH_STATUS_SUCCESS) {
		const char *pac_ms_str = switch_channel_get_variable_dup(session->channel, "rtp_ice_pac_ms", SWITCH_FALSE, -1);
		uint32_t pac_ms = (uint32_t)(pac_ms_str ? atoi(pac_ms_str) : ICE_PAC_DEFAULT_MS);
		
		switch_time_t until = switch_time_now() + (switch_time_t)pac_ms * 1000;
		switch_channel_set_variable_printf(session->channel, "rtp_ice_pac_until_us", "%" SWITCH_TIME_T_FMT, until);
		trickle_maybe_recheck_ice(session, smh, sdp_session, sdp_type, end_of_candidates ? 1 : 0);
	}
	return st;
}

SWITCH_DECLARE(switch_status_t) switch_core_media_trickle_remote_candidate_and_recheck(switch_core_session_t *session,
												switch_media_handle_t *smh,
												void *sdp_session,
												switch_sdp_type_t sdp_type,
												const char *mid,
												int mline_index,
												const char *cand_line,
												int end_of_candidates)
{
	switch_status_t st;

	st = switch_core_media_trickle_remote_candidate(session, mid, mline_index, cand_line, end_of_candidates);
	if (st != SWITCH_STATUS_SUCCESS) {
		return st;
	}

	switch_core_media_trickle_recheck_all(session, sdp_session, sdp_type);
	return SWITCH_STATUS_SUCCESS;
}
...
SWITCH_DECLARE(switch_status_t) switch_core_media_get_chosen_ice_candidate(switch_core_session_t *session, switch_media_type_t type, char **out_addr, switch_port_t *out_port)
{
	...
	/* Check if a candidate was chosen */
	if (engine->ice_in.chosen[0] < 0) {
		return SWITCH_STATUS_FALSE;
	}

	chosen_idx = engine->ice_in.chosen[0];
	chosen = &engine->ice_in.cands[chosen_idx][0];

	*out_addr = chosen->con_addr;
	*out_port = chosen->con_port;
