																				  ...
																				  const char *arg, int32_t *flags)
{
	...
	if ((application_interface = switch_loadable_module_get_application_interface(app)) == 0) {
		if (strcmp(app, "spandsp_start_dtmf") && switch_channel_get_variable(session->channel, "inbound_bypass_media") && switch_channel_get_variable(session->channel, "ignore_spandsp_error")){
