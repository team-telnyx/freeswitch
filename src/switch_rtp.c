...
/* Forward decls */
static switch_status_t parse_cand_line(const char *line, switch_rtp_ice_cand_t *out);
...
struct switch_rtp {
	...
	uint8_t ext_total_len; /* bytes of last parsed single extension element */
	switch_bool_t ext_has_ext;
	uint16_t ext_profile; /* extension profile (0xBEDE) */
	uint16_t ext_length_words;  /* header->length (32-bit word count) */
	ext_mid_t ext_mid; /* local MID SDES config (sender side) */
};
...
SWITCH_DECLARE(void) switch_rtp_handle_extensions(switch_rtp_t *rtp_session);
...
static switch_rtp_t *switch_rtp_bundle_find_target_by_mid(switch_rtp_t *rtp_session, const char *mid)
{
	switch_core_session_t *session;
	switch_rtp_t *target;
	switch_media_type_t types[] = { SWITCH_MEDIA_TYPE_AUDIO, SWITCH_MEDIA_TYPE_VIDEO, SWITCH_MEDIA_TYPE_TEXT };
	size_t i;

	if (!rtp_session || zstr(mid)) {
		return NULL;
	}

	session = rtp_session->session;
	if (!session) {
		return NULL;
	}

	for (i = 0; i < (sizeof(types) / sizeof(types[0])); i++) {
		target = switch_core_media_get_rtp_session(session, types[i]);
		if (target && target->ext_mid.mid[0] && !strcmp(target->ext_mid.mid, mid)) {
			return target;
		}
	}
        ...
        switch_rtp_ext_info_t *info)
{
	...
	if (rtp_session->write_msg.header.x && rtp_session->write_msg.ext) {
		/* Validate ext pointer is within body buffer */
		char *ext_ptr = (char *)rtp_session->write_msg.ext;
		if (ext_ptr >= rtp_session->write_msg.body &&
			ext_ptr < rtp_session->write_msg.body + sizeof(rtp_session->write_msg.body)) {
		...
		/* Validate ext pointer is within body buffer */
		char *ext_ptr = (char *)rtp_session->send_msg.ext;
		if (ext_ptr >= rtp_session->send_msg.body &&
			ext_ptr < rtp_session->send_msg.body + sizeof(rtp_session->send_msg.body)) {
			info->has_ext = SWITCH_TRUE;
...
#ifndef TRICKLE_EOC_MAX_ENTRIES
#define TRICKLE_EOC_MAX_ENTRIES 4096
#endif

typedef struct trickle_eoc_kv_s {
	const void *rtp;
	char        mid[64];
	int         mline_index;
	struct trickle_eoc_kv_s *next;
} trickle_eoc_kv_t;
...
static void trickle_eoc_seal(switch_rtp_t *rtp_session, const char *mid, int mline_index)
{
	...
	if (trickle_eoc_is_sealed((const void *)rtp_session, mmid, mline_index)) {
		return;
	}
	...
	if (g_trickle_eoc_size >= TRICKLE_EOC_MAX_ENTRIES) {
		...
		while (it && it->next) { prev = it; it = it->next; }
		if (it) {
			if (prev) {
				prev->next = NULL;
			} else {
				g_trickle_eoc_head = NULL;
			}
			switch_safe_free(it);
	...
	while (it) {
		if (it->rtp == rtp) {
			...
			if (prev) {
				...
				g_trickle_eoc_head = it->next;
			}
			it = it->next;
			switch_safe_free(stale);
			 if (g_trickle_eoc_size > 0) g_trickle_eoc_size--;
			continue;
		}
		prev = it;
...
static inline switch_log_level_t ice_dbg_level(const switch_rtp_t *rtp_session);
static inline void ice_log_cand(const switch_rtp_t *rtp_session, const char *action, const char *reason,
                                const char *mid, int mline_index,
                                int component_id, const char *transport,
                                const char *ip, int port,
                                const char *extra_fmt, ...);
static inline int addr_family_guess(const char *ip);
static inline switch_bool_t ice_remote_ip_is_acceptable(const switch_rtp_t *rtp_session, const char *ip, const char *transport, int *out_family);
...
#ifdef DEBUG_2833
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "start %u [%c]\n", ts, key);
#endif
				rtp_session->dtmf_data.in_digit_ts = ts;
				rtp_session->dtmf_data.last_in_digit_ts = ts;
				rtp_session->dtmf_data.first_digit = key;
...
static int rtp_bundle_write(switch_rtp_t *video_session, switch_rtp_t *audio_master,
							rtp_msg_t *send_msg, void *data, uint32_t datalen, switch_payload_t payload, uint32_t timestamp, switch_frame_flag_t *flags);
...
static void do_pp_mos(switch_rtp_t *rtp_session)
{
	...
	if (rtp_session->stats.inbound.pp_packet_counter >= PP_MOS_PACKET_COUNTER_MAX_SIZE) {
		if (rtp_session->stats.inbound.pp_packet_lost >= PP_MOS_PACKET_COUNTER_MAX_SIZE) {
			rtp_session->stats.inbound.pp_lost_percentage = 100;
		} else {
			rtp_session->stats.inbound.pp_lost_percentage = ((double)rtp_session->stats.inbound.pp_packet_lost/PP_MOS_PACKET_COUNTER_MAX_SIZE * 100.0);
		}
		rtp_session->stats.inbound.pp_packet_counter = 0;
		rtp_session->stats.inbound.pp_packet_lost = 0;
	}
	...
	if (rtp_session->stats.inbound.recved > 0) {
		...
		unsigned int packet_loss = rtp_session->stats.inbound.pp_lost_percentage;
		double mos = 0;

		// Take the average latency, add jitter, but add an impact to jitter
		// then add 10 for protocol latencies
		R = (rtt/1000 + rtp_session->stats.inbound.mean_interval * rtp_mos_jitter_penalty + 10);
...
static void check_jitter(switch_rtp_t *rtp_session)
{
	...
	if (rtp_session->stats.inbound.last_processed_seq > 0 && seq > (int)(rtp_session->stats.inbound.last_processed_seq + 1)) {
		...
		rtp_session->stats.inbound.pp_packet_lost += lost;
		rtp_session->stats.inbound.pp_packet_counter += lost;
...
static int check_rtcp_and_ice(switch_rtp_t *rtp_session)
{
	...
	if (rtp_session->rtcp_sock_output && rtp_session->flags[SWITCH_RTP_FLAG_ENABLE_RTCP] && !rtp_session->flags[SWITCH_RTP_FLAG_RTCP_PASSTHRU] && rtcp_ok) {
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND]) {
			...
			if (stat) {
				char *em = NULL;
				uint32_t ssrc_rtcp = 0;

				ssrc_rtcp = ntohl(*(uint32_t *)rtp_session->rtcp_send_msg.body);

				switch_srtp_err_to_txt(stat, &em);
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR,
						"SRTP RTCP protect failed (%d: %s) pt=%u cnt=%u len_words=%u ssrc=%u mki=%d bytes_in=%ld bytes_out=%d\n", stat, em ? em : "?",
						(unsigned)rtp_session->rtcp_send_msg.header.type,
						(unsigned)rtp_session->rtcp_send_msg.header.count,
						(unsigned)ntohs(rtp_session->rtcp_send_msg.header.length),
						(unsigned)ssrc_rtcp,
						rtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND_MKI] ? 1 : 0,
						(long)rtcp_bytes,
						sbytes);
				switch_mutex_unlock(rtp_session->ice_mutex);
				goto end;
...
#endif
		if (switch_rtp_sendto(rtp_session, rtp_session->rtcp_sock_output, rtp_session->rtcp_remote_addr, 0, (void *)&rtp_session->rtcp_send_msg, &rtcp_bytes ) != SWITCH_STATUS_SUCCESS) {
...
static switch_status_t enable_remote_rtcp_socket(switch_rtp_t *rtp_session, const char **err) {
	...
	if (rtp_session->flags[SWITCH_RTP_FLAG_ENABLE_RTCP]) {
		...
		if (switch_sockaddr_info_get(&rtp_session->rtcp_remote_addr, rtp_session->eff_remote_host_str, SWITCH_UNSPEC,
			...
			RTP_SET_ERR(err, "Local Address Error!");
			return SWITCH_STATUS_FALSE;
		} else {
			const char *host;
			char bufa[50];
		...
		RTP_SET_ERR(err, "RTCP NOT ACTIVE!");
	}
...
static switch_status_t enable_local_rtcp_socket(switch_rtp_t *rtp_session, const char **err) {
	...
	if (rtp_session->flags[SWITCH_RTP_FLAG_ENABLE_RTCP]) {
		if (switch_sockaddr_info_get(&rtp_session->rtcp_local_addr, host, SWITCH_UNSPEC, port+1, 0, rtp_session->pool) != SWITCH_STATUS_SUCCESS) {
			RTP_SET_ERR(err, "RTCP Local Address Error!");
			goto done;
		}
		...
		if (switch_socket_bind(rtcp_new_sock, rtp_session->rtcp_local_addr) != SWITCH_STATUS_SUCCESS) {
			RTP_SET_ERR(err, "RTCP Bind Error!");
			goto done;
		}
		...
		if (switch_sockaddr_info_get(&rtp_session->rtcp_from_addr, switch_get_addr(bufa, sizeof(bufa), rtp_session->from_addr),
			...
			RTP_SET_ERR(err, "RTCP From Address Error!");
			goto done;
		}
															  ...
															  switch_bool_t change_adv_addr, const char **err)
{
	...
	*err = "Success";
	...
	if (status == SWITCH_STATUS_SUCCESS && rtp_session->ready < 2) {
		rtp_session->ready = 2;
	}
...
SWITCH_DECLARE(switch_status_t) switch_rtp_fork_set(switch_rtp_t *rtp_session, switch_fork_direction_t direction, const char *host, switch_port_t port, uint32_t ssrc, const char *cmd, const char *codec_iananame)
{
	...
	switch_copy_string(codec_lwc, codec_iananame, FORK_CODEC_NAME_LEN);
	...
	switch_copy_string(fork->rtp_codec, rtp_codec_iananame, FORK_CODEC_NAME_LEN);
	fork->rtp_codec[FORK_CODEC_NAME_LEN - 1] = 0;
	switch_string_tolower(fork->rtp_codec);

	switch_copy_string(fork->fork_codec, fork->rtp_codec, FORK_CODEC_NAME_LEN);
	...
	if (!zstr(codec_iananame)) {
		...
		switch_copy_string(fork->fork_codec, codec_lwc, FORK_CODEC_NAME_LEN);
...
fork_setup:
	...
	fork->ssrc = ssrc;
	if (!zstr(codec_iananame)) {
		switch_copy_string(fork->codec_iananame, codec_iananame, FORK_CODEC_NAME_LEN);
		fork->codec_iananame[FORK_CODEC_NAME_LEN - 1] = 0;
	}
	fork->transcoding = transcoding;
	if (!zstr(cmd)) {
		switch_copy_string(fork->cmd, cmd, 500);
		fork->cmd[499] = '\0';
	}
...
SWITCH_DECLARE(switch_status_t) switch_rtp_fork_set_id(switch_rtp_t *rtp_session, const char *id)
{
	...
	switch_copy_string(rtp_session->fork.id, id, 40);
...
SWITCH_DECLARE(switch_status_t) switch_rtp_fork_set_local_address(switch_rtp_t *rtp_session, const char *ip, uint16_t port)
{
	...
	switch_copy_string(rtp_session->fork.local_ip, ip, 100);
												  ...
												  switch_rtp_flag_t flags[SWITCH_RTP_FLAG_INVALID], char *timer_name, const char **err, switch_memory_pool_t *pool, uint32_t poll_timeout_s)
{
	...
	*new_rtp_session = NULL;
	...
	rtp_session->stats.inbound.pp_R = 0;
	rtp_session->stats.inbound.pp_mos = 1;
	rtp_session->stats.inbound.cumulative_R = 0;
	rtp_session->stats.inbound.cumulative_mos = 1;
	rtp_session->stats.inbound.pp_packet_counter = 0;
	rtp_session->stats.inbound.pp_packet_lost = 0;
	rtp_session->stats.inbound.pp_lost_percentage = 0.0;
	rtp_session->send_msg.header.ssrc = htonl(rtp_session->ssrc);
	...
	rtp_session->send_msg.ext = NULL;
	rtp_session->send_msg.ebody = NULL;
	...
	rtp_session->recv_mid[0] = '\0';
	
	rtp_session->ext_mid.ext_id  = 0;
	rtp_session->ext_mid.enabled = 0;
	memset(rtp_session->ext_mid.mid, 0, sizeof(rtp_session->ext_mid.mid));
											  ...
											  switch_rtp_flag_t flags[SWITCH_RTP_FLAG_INVALID], char *timer_name, const char **err, switch_memory_pool_t *pool)
{
	...
	if (zstr(rx_host)) {
		RTP_SET_ERR(err, "Missing local host");
		goto end;
	}

	if (!rx_port) {
		RTP_SET_ERR(err, "Missing local port");
		goto end;
	}

	if (zstr(tx_host)) {
		RTP_SET_ERR(err, "Missing remote host");
		goto end;
	}

	if (!tx_port) {
		RTP_SET_ERR(err, "Missing remote port");
		goto end;
	}
...
{
	uint8_t is_ready = switch_rtp_ready(rtp_session);

	/* Allow bundled video sessions (no socket) to create jitterbuffer for MID demux */
	if (!is_ready && !rtp_session->is_bundle) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
						  "Video buffer size: RTP not ready and not bundled (ready=%d, is_bundle=%d)\n",
						  is_ready, rtp_session->is_bundle);
		return SWITCH_STATUS_FALSE;
	}

	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
					  "Video buffer size: Proceeding (ready=%d, is_bundle=%d, frames=%u, max=%u)\n",
					  is_ready, rtp_session->is_bundle, frames, max_frames);
	...
	if (!rtp_session->vb) {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_INFO,
						  "Created video jitterbuffer (frames=%u-%u, is_bundle=%d)\n",
						  frames, max_frames, rtp_session->is_bundle);
	} else {
		switch_jb_set_frames(rtp_session->vb, frames, max_frames);
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
						  "Updated video jitterbuffer frames (frames=%u-%u)\n", frames, max_frames);
	}
...
SWITCH_DECLARE(void) switch_rtp_destroy(switch_rtp_t **rtp_session)
{
	...
	trickle_eoc_cleanup((const void *)*rtp_session);
	...
	if ((*rtp_session)->flags[SWITCH_RTP_FLAG_EXT_AUDIO_LEVEL]) {
		switch_rtp_disable_audio_level_extension(*rtp_session);
	}
...
static switch_bool_t rtp_payload_matches(switch_rtp_t *rtp_session, uint8_t pt)
{
...
#define return_cng_frame() do { if (!rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) do_cng = 1; goto timer_check; } while (0)
									   ...
									   payload_map_t **pmapP, switch_status_t poll_status, switch_bool_t return_jb_packet)
{
 ...
 more:
	...
	*bytes = sizeof(rtp_msg_t);
	...
	/* Debug logging for video packet reception */
	if (rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) {
		static int video_poll_count = 0;
		if (++video_poll_count % 500 == 1) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
							  "VIDEO RTP RECV: poll_status=%d bytes=%ld is_bundle=%d vb=%p count=%d\n",
							  poll_status, (long)*bytes, rtp_session->is_bundle,
							  (void*)rtp_session->vb, video_poll_count);
		}
	}
	...
	if (*bytes) {
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) {
			static int video_recv_count = 0;
			if (++video_recv_count % 200 == 1) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
							  "VIDEO RX TRACE: bytes=%ld ssrc=%u mid='%s' ready=%d bundle=%d master=%d\n",
		...
		/* Debug: Log if video packet doesn't have RTP version 2 */
		if (rtp_session->flags[SWITCH_RTP_FLAG_VIDEO] && !rtp_session->has_rtp) {
			static int video_bad_version_count = 0;
			if (++video_bad_version_count % 100 == 1) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
								  "VIDEO BAD VERSION: bytes=%ld version=%d first_byte=0x%02x count=%d\n",
								  (long)*bytes, rtp_session->recv_msg.header.version,
								  b[0], video_bad_version_count);
			}
		}
		...
		if (rtp_session->has_rtp) {
			...
			if (!rtp_session->flags[SWITCH_RTP_FLAG_PROXY_MEDIA] && !rtp_session->flags[SWITCH_RTP_FLAG_UDPTL] &&
				rtp_session->last_rtp_hdr.pt != SWITCH_RTP_CNG_PAYLOAD &&
				rtp_session->last_rtp_hdr.pt != rtp_session->recv_te &&
	...
	if (*bytes && rtp_session->flags[SWITCH_RTP_FLAG_DEBUG_RTP_READ]) {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG_CLEAN(rtp_session->session), SWITCH_LOG_CONSOLE,
						  "R %s b=%4ld %s:%u %s:%u %s:%u pt=%d ts=%u seq=%u m=%d x=%d\n",
						  ...
						  rtp_session->last_rtp_hdr.m, rtp_session->last_rtp_hdr.x);
...
fork_end:
		...
		switch_rtp_handle_extensions(rtp_session);

		/* BUNDLE demultiplexing: Route packets to correct media based on MID or SSRC.
		 * RFC 8843: Use MID header extension for demux, with SSRC as fallback optimization.
		 * RTP header extensions are parsed in switch_rtp_handle_extensions() and stored in rtp_session->recv_mid.
		 */
		if (rtp_session->is_bundle_master) {
			switch_rtp_t *target = NULL;
			uint32_t packet_ssrc = ntohl(rtp_session->recv_msg.header.ssrc);

			/* Try MID-based demux first (RFC 8843 primary method) */
			if (rtp_session->recv_mid[0] && strcmp(rtp_session->recv_mid, rtp_session->ext_mid.mid)) {
				target = switch_rtp_bundle_find_target_by_mid(rtp_session, rtp_session->recv_mid);

				if (target && target != rtp_session) {
					/* Learn SSRC→MID mapping for future packets without MID */
					if (target->bundle_ssrc == 0 || target->bundle_ssrc != packet_ssrc) {
						target->bundle_ssrc = packet_ssrc;
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_INFO,
										  "BUNDLE: Learned SSRC mapping (SSRC=%u → MID=%s)\n",
										  packet_ssrc, rtp_session->recv_mid);
					}

					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
									  "BUNDLE: MID demux (recv_mid='%s', SSRC=%u, target=%p)\n",
									  rtp_session->recv_mid, packet_ssrc, (void*)target);
				} else if (!target) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
									  "BUNDLE: Unknown MID '%s' (SSRC=%u), dropping\n",
									  rtp_session->recv_mid, packet_ssrc);
					goto more;
				}
			}
			/* Fallback: SSRC-based demux for packets without MID (handles client bugs) */
			else if (!rtp_session->recv_mid[0] && packet_ssrc != 0) {
				/* Check if this SSRC matches a known bundle_ssrc */
				switch_media_type_t types[] = { SWITCH_MEDIA_TYPE_AUDIO, SWITCH_MEDIA_TYPE_VIDEO, SWITCH_MEDIA_TYPE_TEXT };
				size_t i;
				for (i = 0; i < (sizeof(types) / sizeof(types[0])); i++) {
					switch_rtp_t *candidate = switch_core_media_get_rtp_session(rtp_session->session, types[i]);
					if (candidate && candidate->bundle_ssrc == packet_ssrc && candidate != rtp_session) {
						target = candidate;
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
										  "BUNDLE: SSRC fallback demux (SSRC=%u, no MID, target=%p)\n",
										  packet_ssrc, (void*)target);
						break;
					}
				}

				if (!target) {
					/* Packet without MID and unknown SSRC - keep on this session */
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
								  "BUNDLE: Packet without MID, unknown SSRC=%u, processing as audio\n",
								  packet_ssrc);
				}
			}

			if (!target && !rtp_session->recv_mid[0]) {
				switch_rtp_t *video = switch_core_media_get_rtp_session(rtp_session->session, SWITCH_MEDIA_TYPE_VIDEO);
				switch_rtp_t *text = switch_core_media_get_rtp_session(rtp_session->session, SWITCH_MEDIA_TYPE_TEXT);
				uint8_t packet_pt = rtp_session->recv_msg.header.pt;

				if (!rtp_payload_matches(rtp_session, packet_pt)) {
					if (video && video != rtp_session && rtp_payload_matches(video, packet_pt)) {
						target = video;
					} else if (text && text != rtp_session && rtp_payload_matches(text, packet_pt)) {
						target = text;
					}
				}
				...
				if (target) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
								  "BUNDLE: Payload fallback demux (pt=%u, target=%p)\n",
								  packet_pt, (void *)target);
				}
			}

			/* Route packet to target if found */
			if (target && target != rtp_session) {
				/* Sanity check packet size - RTP packets should be < 16KB */
				if (*bytes > 0 && *bytes <= SWITCH_RTP_MAX_BUF_LEN) {
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
									  "BUNDLE: Invalid packet size %ld, dropping\n", (long)*bytes);
				}
		...
		if (rtp_session->has_rtp) {
			...
			} else if (!rtp_session->flags[SWITCH_RTP_FLAG_PROXY_MEDIA] && !rtp_session->flags[SWITCH_RTP_FLAG_UDPTL]) {
				/* No extension header - clear stale pointers to prevent write path from using garbage */
				rtp_session->recv_msg.ext = NULL;
				rtp_session->recv_msg.ebody = NULL;
			}
		...
		if (rtp_session->last_rtp_hdr.pt == rtp_session->recv_te) {
		...
		} else if (rtp_session->last_rtp_hdr.pt == rtp_session->cng_pt || rtp_session->last_rtp_hdr.pt == SWITCH_RTP_CNG_PAYLOAD) {
			...
			rtp_session->stats.inbound.pp_packet_counter++;
		}
	...
	if (rtp_session->has_rtp && *bytes) {
		...
		/* Debug: Log video packet before JB check */
		if (rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) {
			static int video_jb_check_count = 0;
			if (++video_jb_check_count % 100 == 1) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
								  "VIDEO JB CHECK: bytes=%ld vb=%p pause_jb=%d jb_valid=%d pt=%d seq=%u count=%d\n",
								  (long)*bytes, (void*)rtp_session->vb, rtp_session->pause_jb,
								  jb_valid(rtp_session), rtp_session->last_rtp_hdr.pt,
								  ntohs(rtp_session->last_rtp_hdr.seq), video_jb_check_count);
			}
		}

		if (rtp_session->vb && !rtp_session->pause_jb && jb_valid(rtp_session)) {
			/* Enhanced debug logging to trace oversized packet source */
			if (*bytes > SWITCH_RTP_MAX_BUF_LEN) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
								  "VIDEO JB: Oversized packet %ld bytes (max %d), seq=%u ts=%u ssrc=%u is_bundle=%d\n",
								  (long)*bytes, SWITCH_RTP_MAX_BUF_LEN,
								  ntohs(rtp_session->last_rtp_hdr.seq),
								  ntohl(rtp_session->last_rtp_hdr.ts),
								  ntohl(rtp_session->last_rtp_hdr.ssrc),
								  rtp_session->is_bundle);
				*bytes = 0;
				return SWITCH_STATUS_FALSE;
			}
	...
	if (!*bytes || rtp_session->has_rtp) {
		...
		if (rtp_session->vb && !rtp_session->pause_jb && jb_valid(rtp_session)) {
			...
			/* Debug: Log video jitterbuffer get result */
			{
				static int video_jb_get_count = 0;
				if (++video_jb_get_count % 100 == 1) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
									  "VIDEO JB GET: vstatus=%d bytes=%ld seq=%u pt=%d count=%d\n",
									  vstatus, (long)*bytes, ntohs(rtp_session->recv_msg.header.seq),
									  rtp_session->recv_msg.header.pt, video_jb_get_count);
				}
			}
...
static void handle_nack(switch_rtp_t *rtp_session, uint32_t nack)
{
	...
	if (switch_jb_get_packet_by_seq(rtp_session->vbw, seq, (switch_rtp_packet_t *) send_msg, &bytes) == SWITCH_STATUS_SUCCESS) {
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_DEBUG_RTP_WRITE]) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG_CLEAN(rtp_session->session), SWITCH_LOG_CONSOLE,
							  "X %s b=%4ld %s:%u %s:%u %s:%u pt=%d ts=%u seq=%u m=%d x=%d\n",
							  rtp_session->session ? switch_channel_get_name(switch_core_session_get_channel(rtp_session->session)) : "NoName",
							  (long) bytes,
							  my_host, switch_sockaddr_get_port(rtp_session->local_addr),
							  old_host, rtp_session->remote_port,
							  tx_host, switch_sockaddr_get_port(rtp_session->rtcp_from_addr),
							  send_msg->header.pt, ntohl(send_msg->header.ts), ntohs(send_msg->header.seq), send_msg->header.m, send_msg->header.x);
...
static switch_status_t process_rtcp_packet(switch_rtp_t *rtp_session, switch_size_t *bytes)
{
	...
	if (remain < sizeof(switch_rtcp_hdr_t) || remain > sizeof(rtcp_msg_t)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR, "Invalid RTCP size: [%"SWITCH_SIZE_T_FMT"]\n", remain);
		return status;
	}
						   ...
						   payload_map_t **pmapP, switch_frame_flag_t *flags, switch_io_flag_t io_flags)
{
	...
	/* BUNDLE: Allow bundled video (no socket) to proceed to jitterbuffer read path.
	 * Bundled video receives packets via MID demux into its jitterbuffer (vb),
	 * so it doesn't need a socket to read from. */
	if (!switch_rtp_ready(rtp_session)) {
		if (!(rtp_session->is_bundle && rtp_session->vb && !rtp_session->flags[SWITCH_RTP_FLAG_SHUTDOWN])) {
			return -1;
		}
		/* Bundled video with jitterbuffer - proceed to read from jitterbuffer */
	}
	...
	/* BUNDLE: Bundled video (no socket) reads directly from jitterbuffer.
	 * Packets are placed into the jitterbuffer by the audio thread via MID demux. */
	...
	/* Event-driven wait for packet arrival instead of busy-spinning */
	{
		...
		case SWITCH_STATUS_RESTART:
		default:
			/* No packet available or break signal - return 0 (CNG will be generated)
			 * Clear ebody to ensure RTP_BODY() returns the statically allocated
			 * recv_msg.body buffer, preventing crashes from stale pointers. */
			rtp_session->recv_msg.ebody = NULL;
			ret = 0;
			break;
		}
			...
			if (rtp_session->flags[SWITCH_RTP_FLAG_TEXT]) {
				...
				if (rtp_session->sync_packets) {
					...
					if (!rtp_session->flags[SWITCH_RTP_FLAG_PAUSE]) {
						...
						rtp_session->stats.inbound.pp_packet_lost += rtp_session->sync_packets;
						rtp_session->stats.inbound.pp_packet_counter += rtp_session->sync_packets;
	...
	recvfrom:
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) {
			int jb_poll_result = (rtp_session->vb && !rtp_session->pause_jb) ? switch_jb_poll(rtp_session->vb) : 0;
			got_jb = (rtp_session->vb && !rtp_session->pause_jb && jb_poll_result);
			/* Debug: Log video JB poll result */
			{
	...
	rtcp:
								...
								if ((other_rtp_session = switch_channel_get_private(other_channel, "__rtcp_audio_rtp_session")) &&
									...
									if (switch_rtp_test_flag(other_rtp_session, SWITCH_RTP_FLAG_SECURE_SEND)) {
										...
										if (stat) {
											...
											ssrc_rtcp = ntohl(*(uint32_t *)other_rtp_session->rtcp_send_msg.body);

											switch_srtp_err_to_txt(stat, &em);
											switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR,
													"SRTP RTCP protect failed (%d: %s) pt=%u cnt=%u len_words=%u ssrc=%u mki=%d bytes_in=%ld bytes_out=%d\n",
													stat, em ? em : "?",
													(unsigned)other_rtp_session->rtcp_send_msg.header.type,
													(unsigned)other_rtp_session->rtcp_send_msg.header.count,
													(unsigned)ntohs(other_rtp_session->rtcp_send_msg.header.length),
													(unsigned)ssrc_rtcp,
													other_rtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND_MKI] ? 1 : 0,
													(long)rtcp_bytes,
													sbytes);
										}
		...
		if (rtp_session->last_rtp_hdr.pt == rtp_session->cng_pt || rtp_session->last_rtp_hdr.pt == SWITCH_RTP_CNG_PAYLOAD) {
			*flags |= SFF_NOT_AUDIO;
		} else {
		...
		if (bytes) {
			...
			if (rtp_session->last_rtp_hdr.pt && (rtp_session->last_rtp_hdr.pt == rtp_session->cng_pt || rtp_session->last_rtp_hdr.pt == SWITCH_RTP_CNG_PAYLOAD)) {
				return_cng_frame();
			}
		}
		...
		if (check || (bytes && !rtp_session->flags[SWITCH_RTP_FLAG_USE_TIMER])) {
			if (!bytes && rtp_session->flags[SWITCH_RTP_FLAG_USE_TIMER]) {	/* We're late! We're Late! */
				...
				if (!rtp_session->flags[SWITCH_RTP_FLAG_PAUSE] && !rtp_session->flags[SWITCH_RTP_FLAG_DTMF_ON] && !rtp_session->dtmf_data.in_digit_ts
					...
					rtp_session->stats.inbound.pp_packet_lost++;
					rtp_session->stats.inbound.pp_packet_counter++;
					do_mos(rtp_session);
				}
...
static int8_t calculate_audio_level(int16_t* samples, int offset, int length, int overload)
{
	double rms = 0, db = 0;
	...
	{
		db = 20 * log10(rms);
		if (db < RTP_EXT_MIN_AUDIO_LEVEL)
			db = RTP_EXT_MIN_AUDIO_LEVEL;
		else if (db > RTP_EXT_MAX_AUDIO_LEVEL)
			db = RTP_EXT_MAX_AUDIO_LEVEL;
	}
	else
	{
		db = RTP_EXT_MIN_AUDIO_LEVEL;
...
static int rtp_add_extension_header(switch_rtp_t *rtp_session, rtp_msg_t *send_msg, uint8_t score, void *data, switch_size_t *datalen)
{
	...
	if (!(rtp_session && send_msg && datalen)) {
		return 0;
	}

	id = (uint8_t)(rtp_session->ext_audio_level_data.ext_id > 0 ? rtp_session->ext_audio_level_data.ext_id : 1);
	if (id > 14) id = 14;

	body = (uint8_t *)send_msg->body;

	elem_hdr = (uint8_t)(((id & 0x0F) << 4) | 0x00);

	if (send_msg->header.x && body[0] == 0xBE && body[1] == 0xDE) {
		uint16_t ext_words = (uint16_t)((body[2] << 8) | body[3]);
		int cur_payload_len = (int)(ext_words * 4);
		uint8_t *payload_start = body + 4;
		uint8_t *rtp_payload = payload_start + cur_payload_len;
		int cur_payload_bytes = (int)(*datalen);
		int add_len = 2;
		int new_payload_len = cur_payload_len + add_len;

		pad_needed = (4 - (new_payload_len & 3)) & 3;

		if (cur_payload_bytes > 0) {
			memmove(rtp_payload + add_len + pad_needed, rtp_payload, (size_t)cur_payload_bytes);
		}

		payload_start[cur_payload_len] = elem_hdr;
		payload_start[cur_payload_len + 1] = (uint8_t)score;
		if (pad_needed) {
			memset(payload_start + cur_payload_len + add_len, 0, (size_t)pad_needed);
		}

		{
			uint16_t new_words = (uint16_t)((new_payload_len + pad_needed) / 4);
			body[2] = (uint8_t)((new_words >> 8) & 0xFF);
			body[3] = (uint8_t)(new_words & 0xFF);
        }

		*datalen += (switch_size_t)(add_len + pad_needed);
		return (int)(*datalen);
	} else {
		int new_payload_len = 2; /* elem_hdr + 1-byte value */
		int pad = (4 - (new_payload_len & 3)) & 3; /* will be 2 bytes */
		int total_insert = 4 + new_payload_len + pad; /* header + payload + pad */

		memmove(body + total_insert, body, (size_t)*datalen);

		body[0] = 0xBE; body[1] = 0xDE;
		body[2] = 0x00; body[3] = 0x01;

		body[4] = elem_hdr;
		body[5] = (uint8_t)score;
		if (pad) {
			memset(body + 6, 0, (size_t)pad);
		}

		send_msg->header.x = 1;
		*datalen += (switch_size_t)total_insert;
		return (int)(*datalen);
	}
}

static switch_size_t rtp_strip_header_extensions(rtp_msg_t *send_msg, switch_size_t *packet_len)
{
	switch_size_t removed = 0;
	size_t csrc_len;
	size_t header_len;
	size_t ext_bytes;
	...
	if (!send_msg || !packet_len || !*packet_len) {
		return 0;
	}
	...
	csrc_len = send_msg->header.cc * 4;
	header_len = rtp_header_len + csrc_len;
	...
	body = (uint8_t *)send_msg->body;
	ext_hdr = body + csrc_len;
	ext_words = (uint16_t)((ext_hdr[2] << 8) | ext_hdr[3]);

	if (ext_words > SWITCH_RTP_MAX_BUF_LEN_WORDS) {
		return 0;
	}

	ext_bytes = 4 + ((size_t)ext_words * 4);

	if (*packet_len < header_len + ext_bytes) {
		return 0;
	}

	payload_bytes = *packet_len - header_len - ext_bytes;

	if (payload_bytes) {
		memmove(ext_hdr, ext_hdr + ext_bytes, payload_bytes);
	}

	*packet_len -= (switch_size_t)ext_bytes;
	removed = (switch_size_t)ext_bytes;
	if (send_msg->header.x) {
		send_msg->header.x = 0;
	}
	send_msg->ext = NULL;
...
/* BUNDLE: Write video packets through audio session's socket for bundled media.
 * This is called when video RTP session has no socket (is_bundle && !sock_output).
 * Video packets are sent through the audio (bundle master) session's socket with
 * MID extension added for demux at the remote end. */
static int rtp_bundle_write(switch_rtp_t *video_session, switch_rtp_t *audio_master,
							rtp_msg_t *send_msg, void *data, uint32_t datalen,
							switch_payload_t payload, uint32_t timestamp, switch_frame_flag_t *flags)
{
	switch_size_t bytes;
	int ret;
	uint32_t this_ts = 0;
	...
	if (!switch_rtp_ready(audio_master)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(video_session->session), SWITCH_LOG_DEBUG1,
						  "BUNDLE: Audio master not ready for write\n");
		return -1;
	}

	if (!rtp_write_ready(audio_master, datalen, __LINE__)) {
		return 0;
	}

	WRITE_INC(video_session);

	/* Build packet using video session's properties */
	if (send_msg) {
		if (send_msg != &video_session->send_msg) {
			if (datalen > sizeof(video_session->send_msg)) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(video_session->session), SWITCH_LOG_ERROR,
							  "BUNDLE: RTP packet too large (%u) for local buffer\n", datalen);
				return -1;
			}
			memcpy(&video_session->send_msg, send_msg, datalen);
			send_msg = &video_session->send_msg;
			send_msg->ext = NULL;
			{
				switch_size_t packet_len = (switch_size_t)datalen;
				if (rtp_strip_header_extensions(send_msg, &packet_len)) {
					datalen = (uint32_t)packet_len;
				}
			}
		}
		bytes = datalen;
		...
		/* Handle video timestamp normalization (similar to rtp_common_write for video) */
		if (!video_session->ts_norm.ts) {
			video_session->ts_norm.ts = (uint32_t) switch_rand() % 1000000 + 1;
		}

		if (!video_session->ts_norm.last_ssrc || video_session->ts_norm.last_external != external) {
			if (video_session->ts_norm.last_ssrc) {
				video_session->ts_norm.delta_ttl = 0;
				video_session->ts_norm.ts++;
			}
			video_session->ts_norm.last_ssrc = video_session->ssrc;
		}
		video_session->ts_norm.last_external = external;
		...
		video_session->ts_norm.last_frame = timestamp;

		/* Debug: Log timestamp normalization periodically */
		{
			static int ts_norm_log_count = 0;
			if (++ts_norm_log_count % 200 == 1) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(video_session->session), SWITCH_LOG_WARNING,
					"VIDEO TS NORM: in_ts=%u norm_delta=%u out_ts=%u count=%d\n",
					timestamp, video_session->ts_norm.delta,
					video_session->ts_norm.ts, ts_norm_log_count);
			}
		}
	...
	/* Set SSRC and sequence number from video session */
	send_msg->header.ssrc = htonl(video_session->ssrc);
	video_session->seq++;
	send_msg->header.seq = htons(video_session->seq);

	if (video_session->ts_norm.last_ssrc != video_session->ssrc) {
		switch_core_session_t *other_session;

		switch_core_session_request_video_refresh(video_session->session);
		switch_core_media_gen_key_frame(video_session->session);

		if (switch_core_session_get_partner(video_session->session, &other_session) == SWITCH_STATUS_SUCCESS) {
			switch_core_session_request_video_refresh(other_session);
	...
	/* Handle extension header byte count adjustment */
	if (send_msg->header.x && send_msg->ext) {
		char *body_start = send_msg->body;
		char *body_end = send_msg->body + sizeof(send_msg->body);
		char *ext_ptr = (char *)send_msg->ext;

		if (ext_ptr >= body_start && ext_ptr < body_end) {
			uint16_t ext_words = ntohs(send_msg->ext->length);
			if (ext_words <= SWITCH_RTP_MAX_BUF_LEN_WORDS) {
				bytes = (switch_size_t)(datalen + rtp_header_len + 4 + (ext_words * 4));
			} else {
				bytes = (switch_size_t)(datalen + rtp_header_len);
				send_msg->header.x = 0;
			}
		} else {
			send_msg->ext = NULL;
...
#ifdef ENABLE_SRTP
	/* Handle SRTP - use audio master when video lacks SRTP */
	if (!video_session->flags[SWITCH_RTP_FLAG_SECURE_SEND] && audio_master->flags[SWITCH_RTP_FLAG_SECURE_SEND]) {
		srtp_session = audio_master;
	}

	switch_mutex_lock(srtp_session->ice_mutex);
	if (srtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND] && srtp_session->send_ctx[srtp_session->srtp_idx_rtp]) {
		int sbytes = (int) bytes;
		srtp_err_status_t stat;

		if (srtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND_RESET]) {
			switch_rtp_clear_flag(srtp_session, SWITCH_RTP_FLAG_SECURE_SEND_RESET);
			srtp_dealloc(srtp_session->send_ctx[srtp_session->srtp_idx_rtp]);
			srtp_session->send_ctx[srtp_session->srtp_idx_rtp] = NULL;
			if (srtp_create(&srtp_session->send_ctx[srtp_session->srtp_idx_rtp],
							&srtp_session->send_policy[srtp_session->srtp_idx_rtp]) || !srtp_session->send_ctx[srtp_session->srtp_idx_rtp]) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(video_session->session), SWITCH_LOG_ERROR,
							  "BUNDLE: Error RE-Activating Secure RTP SEND for video\n");
				srtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND] = 0;
				ret = -1;
				switch_mutex_unlock(srtp_session->ice_mutex);
				goto end;
			}
		}

		if (!srtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND_MKI]) {
			stat = srtp_protect(srtp_session->send_ctx[srtp_session->srtp_idx_rtp], send_msg, &sbytes);
		} else {
			stat = srtp_protect_mki(srtp_session->send_ctx[srtp_session->srtp_idx_rtp], send_msg, &sbytes, 1, SWITCH_CRYPTO_MKI_INDEX);
		}

		if (stat) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(video_session->session), SWITCH_LOG_ERROR,
						  "BUNDLE: SRTP protect failed for video (%d)\n", stat);
		}
		bytes = sbytes;
	}
	switch_mutex_unlock(srtp_session->ice_mutex);
#endif
	{
		static int bundle_srtp_warn_count = 0;
		if ((!srtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND] || !srtp_session->send_ctx[srtp_session->srtp_idx_rtp]) &&
			(++bundle_srtp_warn_count % 200 == 1)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(video_session->session), SWITCH_LOG_WARNING,
						  "BUNDLE: video write without SRTP (audio_secure=%d video_secure=%d using=%s)\n",
						  audio_master->flags[SWITCH_RTP_FLAG_SECURE_SEND] ? 1 : 0,
						  video_session->flags[SWITCH_RTP_FLAG_SECURE_SEND] ? 1 : 0,
						  srtp_session == audio_master ? "audio" : "video");
		}
	}

	/* Send through audio master's socket to bundle master's remote address */
	if (switch_rtp_sendto(video_session, audio_master->sock_output, audio_master->remote_addr, 0, (void *)send_msg, &bytes) != SWITCH_STATUS_SUCCESS) {
		video_session->seq--;
		ret = -1;
		goto end;
	}
	...
	{
		static int bundle_video_out_count = 0;
		if (++bundle_video_out_count % 200 == 1) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(video_session->session), SWITCH_LOG_DEBUG,
						  "BUNDLE: video out ssrc=%u pt=%u mid=%s bytes=%ld\n",
						  video_session->ssrc, send_msg->header.pt,
						  video_session->ext_mid.mid[0] ? video_session->ext_mid.mid : "",
						  (long)bytes);
		}
	}

	/* Update video session stats */
	video_session->last_write_ts = this_ts;
	video_session->stats.outbound.raw_bytes += bytes;
	video_session->stats.outbound.packet_count++;
	video_session->stats.outbound.media_packet_count++;
	video_session->stats.outbound.media_bytes += bytes;

	if (video_session->flags[SWITCH_RTP_FLAG_ENABLE_RTCP]) {
		video_session->stats.rtcp.sent_pkt_count++;
	}

	ret = (int) bytes;

end:
	WRITE_DEC(video_session);
	return ret;
}
							...
							rtp_msg_t *send_msg, void *data, uint32_t datalen, switch_payload_t payload, uint32_t timestamp, switch_frame_flag_t *flags)
{
	...
	/* BUNDLE: Bundled video writes through audio's socket.
	 * Must check this BEFORE switch_rtp_ready() since bundled video has no socket. */
	if (rtp_session->is_bundle && rtp_session->flags[SWITCH_RTP_FLAG_VIDEO] && !rtp_session->is_bundle_master && rtp_session->session) {
		switch_rtp_t *bundle_master = switch_core_media_get_rtp_session(rtp_session->session, SWITCH_MEDIA_TYPE_AUDIO);

		if (bundle_master && bundle_master->is_bundle_master && bundle_master->sock_output) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
						  "BUNDLE: Writing %s via bundle master (rtp=%p master=%p)\n",
						  rtp_type(rtp_session), (void *)rtp_session, (void *)bundle_master);
			return rtp_bundle_write(rtp_session, bundle_master, send_msg, data, datalen, payload, timestamp, flags);
		}

		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
					  "BUNDLE: Cannot write %s - no bundle master available (master=%p)\n",
					  rtp_type(rtp_session), (void *)bundle_master);
		return -1;
	}

	if (send_msg && send_msg != &rtp_session->send_msg &&
		(rtp_session->ext_mid.enabled || rtp_session->flags[SWITCH_RTP_FLAG_EXT_AUDIO_LEVEL])) {
		if (datalen > sizeof(rtp_session->send_msg)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR,
						  "RTP: packet too large (%u) for local buffer\n", datalen);
			return -1;
		}

		memcpy(&rtp_session->send_msg, send_msg, datalen);
	...
	if (!switch_rtp_test_flag(rtp_session, SWITCH_RTP_FLAG_VIDEO)) {
		...
		if (!switch_rtp_test_flag(rtp_session, SWITCH_RTP_FLAG_UDPTL)) {
...
fork_done:
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_EXT_AUDIO_LEVEL] &&
			((rtp_session->last_rtp_hdr.pt == rtp_session->ext_audio_level_data.read_codec->implementation->ianacode) ||
			(rtp_session->last_rtp_hdr.pt == rtp_session->ext_audio_level_data.agreed_pt))) {
			...
			if (switch_core_codec_decode(&rtp_session->ext_audio_level_data.ext_codec,
										rtp_session->ext_audio_level_data.read_codec,
										data,
										datalen,
										rtp_session->ext_audio_level_data.read_codec->implementation->actual_samples_per_second,
										decoded, &len, &rate, &codec_flags) == SWITCH_STATUS_SUCCESS) {
				int8_t score = calculate_audio_level(decoded, 0, len, SHRT_MAX);
...
#endif
		if (send_msg->header.x && send_msg->ext) {
			/* Validate ext pointer is within the body buffer before dereferencing */
			char *body_start = send_msg->body;
			char *body_end = send_msg->body + sizeof(send_msg->body);
			char *ext_ptr = (char *)send_msg->ext;
			...
			if (ext_ptr >= body_start && ext_ptr < body_end) {
				uint16_t ext_words = ntohs(send_msg->ext->length);
				/* Sanity check extension length - corrupted values cause massive bytes inflation */
				if (ext_words <= SWITCH_RTP_MAX_BUF_LEN_WORDS) {
					/* 4 bytes (profile+length) + 4*words of extension data */
					bytes = (switch_size_t)(datalen + rtp_header_len + 4 + (ext_words * 4));
				} else {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
									  "RTP ext length corrupted: ext_words=%u (max %d), ignoring extension\n",
									  ext_words, SWITCH_RTP_MAX_BUF_LEN_WORDS);
					/* Fall back to basic calculation without extension */
					bytes = (switch_size_t)(datalen + rtp_header_len);
					send_msg->header.x = 0;
				}
			} else {
				/* ext pointer is invalid (garbage/uninitialized) - clear and ignore */
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
								  "RTP ext pointer invalid (outside body buffer), clearing extension flag\n");
				send_msg->ext = NULL;
				send_msg->header.x = 0;
			}
		}
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND]) {
			...
			if (stat) {
				...
				switch_srtp_err_to_txt(stat, &em);
				switch_log_printf(
					SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR,
					"SRTP protect failed (%d: %s) ssrc=%u seq=%u ts=%u mki=%d bytes_in=%ld bytes_out=%d x=%d ext_len_words=%d\n",
					stat, em ? em : "?", (unsigned)ntohl(rtp_session->write_msg.header.ssrc),
					(unsigned)ntohs(rtp_session->write_msg.header.seq),
					(unsigned)ntohl(rtp_session->write_msg.header.ts),
					rtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND_MKI] ? 1 : 0,
					bytes, sbytes,
					rtp_session->write_msg.header.x ? 1 : 0,
					rtp_session->write_msg.ext ? (int)ntohs(rtp_session->write_msg.ext->length) : -1
				);
			}
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_DEBUG_RTP_WRITE]) {
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG_CLEAN(rtp_session->session), SWITCH_LOG_CONSOLE,
							  "W WW %s b=%4ld %s:%u %s:%u %s:%u pt=%d ts=%u seq=%u m=%d\n",
							  rtp_session->session ? switch_channel_get_name(switch_core_session_get_channel(rtp_session->session)) : "NoName",
							  (long) bytes,
							  my_host, switch_sockaddr_get_port(rtp_session->local_addr),
							  old_host, rtp_session->remote_port,
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_NACK]) {
			...
			/* Only store properly packetized RTP packets in NACK buffer */
			if (bytes <= SWITCH_RTP_MAX_BUF_LEN) {
				switch_jb_put_packet(rtp_session->vbw, (switch_rtp_packet_t *)send_msg, bytes);
			} else {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
								  "NACK buffer: Skipping oversized packet %ld bytes (max %d), datalen=%u - video not properly packetized?\n",
								  (long)bytes, SWITCH_RTP_MAX_BUF_LEN, datalen);
			}
		}
...
#ifdef RTP_WRITE_PLOSS
		{
			...
			if (r <= 200) {
				...
				if (switch_rtp_sendto(rtp_session, rtp_session->sock_output, rtp_session->remote_addr, 0, (void *) send_msg, &bytes) != SWITCH_STATUS_SUCCESS) {
					if (!rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) {
						static int audio_sendto_fail_count = 0;
						if (++audio_sendto_fail_count % 50 == 1) {
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
										  "AUDIO WRITE: sendto failed (rtp_write_ploss)\n");
						}
					}
					rtp_session->seq--;
					ret = -1;
													  ...
													  switch_codec_t *codec)
{
	...
	memset(&rtp_session->ext_audio_level_data, 0, sizeof(rtp_session->ext_audio_level_data));

	if ((val = switch_channel_get_variable(switch_core_session_get_channel(rtp_session->session), "rtp_ext_audio_level_id"))) {
		ext_id = atoi(val);
	}

	if (switch_core_codec_init(&rtp_session->ext_audio_level_data.ext_codec,
							   codec->implementation->iananame,
							   codec->implementation->modname,
							   NULL,
							   codec->implementation->samples_per_second,
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG, "Activate RTP EXT audio level codec %s %dms\n", codec->implementation->iananame,
					  codec->implementation->microseconds_per_packet / 1000);
	rtp_session->ext_audio_level_data.read_codec = codec;
	rtp_session->ext_audio_level_data.agreed_pt = codec->agreed_pt;
	rtp_session->ext_audio_level_data.ext_id = ext_id > 0 ? ext_id : 1;

	switch_rtp_set_flag(rtp_session, SWITCH_RTP_FLAG_EXT_AUDIO_LEVEL);
	return SWITCH_STATUS_SUCCESS;
}
...
SWITCH_DECLARE(switch_status_t) switch_rtp_disable_audio_level_extension(switch_rtp_t *rtp_session)
{
	...
	switch_core_codec_destroy(&rtp_session->vad_data.vad_codec);
	switch_rtp_clear_flag(rtp_session, SWITCH_RTP_FLAG_EXT_AUDIO_LEVEL);
	return SWITCH_STATUS_SUCCESS;
}
...
SWITCH_DECLARE(switch_status_t) switch_rtp_offer_audio_level_extension(switch_core_session_t *session, char *buf, uint32_t buflen)
{
	...
	if ((val = switch_channel_get_variable(switch_core_session_get_channel(session), "rtp_ext_audio_level_id"))) {
		ext_id = atoi(val);
	}
...
SWITCH_DECLARE(int) switch_rtp_write_frame(switch_rtp_t *rtp_session, switch_frame_t *frame)
{
	...
	int r = 0;
	switch_status_t status;
	switch_bool_t bundle_video_write = SWITCH_FALSE;
	...
	bundle_video_write = (rtp_session->is_bundle && rtp_session->flags[SWITCH_RTP_FLAG_VIDEO] &&
						 !rtp_session->is_bundle_master && rtp_session->session);

	if ((!switch_rtp_ready(rtp_session) || !rtp_session->remote_addr) && !bundle_video_write) {
		return -1;
	}

	if (!rtp_write_ready(rtp_session, frame->datalen, __LINE__) && !bundle_video_write) {
		if (!rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) {
			static int audio_write_ready_count = 0;
			if (++audio_write_ready_count % 200 == 1) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
							  "AUDIO WRITE: rtp_write_ready=0 ready=%d remote=%p datalen=%u\n",
							  switch_rtp_ready(rtp_session), (void *)rtp_session->remote_addr, frame->datalen);
			}
		}
		return 0;
	}
	...
	if (!rtp_session->flags[SWITCH_RTP_FLAG_VIDEO]) {
		static int audio_write_enter_count = 0;
		if (++audio_write_enter_count % 200 == 1) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
						  "AUDIO WRITE: enter len=%u ready=%d remote=%p\n",
						  frame->datalen, switch_rtp_ready(rtp_session), (void *)rtp_session->remote_addr);
		}
	}
	...
	if (switch_test_flag(frame, SFF_PROXY_PACKET) || switch_test_flag(frame, SFF_UDPTL_PACKET) ||
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_DEBUG_RTP_WRITE]) {
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG_CLEAN(rtp_session->session), SWITCH_LOG_CONSOLE,
							  "W %s b=%4ld %s:%u %s:%u %s:%u pt=%d ts=%u seq=%u m=%d x=%d\n",
							  rtp_session->session ? switch_channel_get_name(switch_core_session_get_channel(rtp_session->session)) : "NoName",
							  (long) bytes,
							  my_host, switch_sockaddr_get_port(rtp_session->local_addr),
							  old_host, rtp_session->remote_port,
	...
	fwd = (rtp_session->flags[SWITCH_RTP_FLAG_RAW_WRITE] &&
		   (switch_test_flag(frame, SFF_RAW_RTP) || switch_test_flag(frame, SFF_RAW_RTP_PARSE_FRAME)) && frame->packet) ? 1 : 0;
#if DEBUG_RTP
...
SWITCH_DECLARE(switch_status_t) switch_rtp_write_raw(switch_rtp_t *rtp_session, void *data, switch_size_t *bytes, switch_bool_t process_encryption)
{
	...
	if (process_encryption) {
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND]) {
			...
			if (rtp_session->write_msg.header.x && rtp_session->write_msg.ext) {
				/* Validate ext pointer is within the body buffer before dereferencing */
				char *body_start = rtp_session->write_msg.body;
				char *body_end = rtp_session->write_msg.body + sizeof(rtp_session->write_msg.body);
				char *ext_ptr = (char *)rtp_session->write_msg.ext;
				...
				if (ext_ptr >= body_start && ext_ptr < body_end) {
					...
					*bytes += ext_bytes;
					...
					/* ext pointer is invalid - clear it */
			...
			if (stat) {
				...
				switch_log_printf(
					SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR,
					"SRTP protect failed (%d: %s) ssrc=%u seq=%u ts=%u mki=%d bytes_in=%d bytes_out=%d x=%d ext_len_words=%d\n",
					stat, em ? em : "?", (unsigned)ntohl(rtp_session->write_msg.header.ssrc),
					(unsigned)ntohs(rtp_session->write_msg.header.seq),
					(unsigned)ntohl(rtp_session->write_msg.header.ts),
					rtp_session->flags[SWITCH_RTP_FLAG_SECURE_SEND_MKI] ? 1 : 0,
...
SWITCH_DECLARE(void) switch_rtp_session_set_bundle_group_id(switch_rtp_t *rtp_session, const char * group_id)
{
	...
	if (group_id && *group_id) {
		rtp_session->bundle_group_id = switch_core_strdup(rtp_session->pool, group_id);
	} else {
		rtp_session->bundle_group_id = NULL;
	}
}
...
{
	return rtp_session->bundle_group_id;
}
...
SWITCH_DECLARE(void) switch_rtp_handle_extensions(switch_rtp_t *rtp_session)
{
	...
	/* per-packet MID: clear before parsing. */
	rtp_session->recv_mid[0] = '\0';

	rtp_session->ext_has_ext = SWITCH_FALSE;
	rtp_session->ext_profile = 0;
	rtp_session->ext_length_words = 0;
	rtp_session->ext_total_len = 0;
	...
	/* Clear stale ext pointer when no extension header present */
	if (!rtp_session->recv_msg.header.x) {
		rtp_session->recv_msg.ext = NULL;
	}

	if (!rtp_session->flags[SWITCH_RTP_FLAG_PROXY_MEDIA] &&
		!rtp_session->flags[SWITCH_RTP_FLAG_UDPTL] &&
		rtp_session->recv_msg.header.x) {

		uint16_t ext_len_field = 0;
		uint16_t ext_profile = 0;
		uint32_t total_ext_bytes = 0;
		uint8_t *ext_data_ptr = NULL;
		uint8_t ext_id = 0, ext_len = 0;
		uint8_t ext_header_len = 1;
		...
		if (ext_len_field > 0) {
			rtp_session->recv_rtp_exts_size = ext_len_field * 4 + 4;
			ext_profile = ntohs(rtp_session->recv_msg.ext->profile);
			rtp_session->ext_has_ext = SWITCH_TRUE;
			rtp_session->ext_profile = ext_profile;
			rtp_session->ext_length_words = ext_len_field;

			if (ext_profile == 0xbede) {
				total_ext_bytes = ext_len_field * 4;
				ext_data_ptr = (uint8_t *) rtp_session->recv_msg.ext + 4;

				while (total_ext_bytes >= 1) {
					ext_id = (*ext_data_ptr & 0xF0) >> 4;
					ext_len = *ext_data_ptr & 0x0F;
					ext_payload_len = ext_len + 1;

					if (ext_id == 0) {
						if (ext_len > 0) {
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING,
								"[%u] RTP extension parsing error: ID=0, LEN=%u\n", ssrc_recv, ext_len);
						}
						ext_data_ptr += 1;
						total_ext_bytes -= 1;
						continue;
					...
					if (rtp_session->ext_mid.enabled && ext_id == rtp_session->ext_mid.ext_id) {
						uint8_t copy_len = ext_payload_len;

						if (copy_len >= sizeof(rtp_session->recv_mid)) {
							copy_len = (uint8_t)(sizeof(rtp_session->recv_mid) - 1);
						}

						memcpy(rtp_session->recv_mid, ext_data_ptr + 1, copy_len);
						rtp_session->recv_mid[copy_len] = '\0';
					}

					rtp_session->ext_total_len = ext_header_len + ext_payload_len;
					if (rtp_session->ext_total_len > total_ext_bytes) {
						break;
					}

					ext_data_ptr += rtp_session->ext_total_len;
					total_ext_bytes = (total_ext_bytes > rtp_session->ext_total_len) ? (total_ext_bytes - rtp_session->ext_total_len) : 0;
				}
			}
		}
	}
...
SWITCH_DECLARE(switch_status_t) switch_rtp_enable_mid(switch_rtp_t *rtp_session, uint8_t ext_id, const char *mid)
{
	...
	if (!rtp_session || !mid || !ext_id || ext_id > 14) return SWITCH_STATUS_FALSE;
...
{
	if (!rtp_session) return;
	rtp_session->trickle.cb = cb;
	rtp_session->trickle.user_data = user_data;
}


 /* Choose RTP vs RTCP ICE context based on candidate component and RTCP-MUX */
 static switch_rtp_ice_t *select_ice_for_component(switch_rtp_t *rtp_session, int component_id)
 {
	switch_rtp_ice_t *ice = &rtp_session->ice; /* component 1 (RTP) by default */
	if (component_id == 2 && !rtp_session->flags[SWITCH_RTP_FLAG_RTCP_MUX]) {
		ice = &rtp_session->rtcp_ice;          /* component 2 (RTCP), when not muxed */
	}
	return ice;
 }

/* Ingest an incoming "a=candidate" (trickle) into the active ICE agent */
static switch_status_t switch_rtp_internal_add_remote_candidate(switch_rtp_t *rtp_session, const switch_rtp_ice_cand_t *cand, int mline_index, const char *mid)
{
	...
	if (!ice_remote_ip_is_acceptable(rtp_session, cand->ip, cand->transport, &fam)) {
		ice_log_cand(rtp_session, "drop", "invalid-addr", mid, mline_index, cand->component_id, cand->transport, cand->ip, cand->port,
				"family=%s", (fam == AF_INET) ? "ipv4" : (fam == AF_INET6) ? "ipv6" : "unspec");
		return SWITCH_STATUS_SUCCESS;
	}

	ice = select_ice_for_component(rtp_session, cand->component_id);
	if (!ice || !ice->ice_params) {
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
					"Trickle ICE: ICE not initialized yet for component %d (mid=%s index=%d), dropping candidate\n",
					 cand->component_id, mid ? mid : "(null)", mline_index);
	return SWITCH_STATUS_FALSE;
	}
	...
	if (ice->ice_params) {
		...
		for (uint32_t i = 0; i < existing; i++) {
			...
			const char *e_type  = ice->ice_params->cands[i][ice->proto].cand_type;
			/* match on foundation +addr +port + type if available */
			if (cand->foundation[0] && e_found && e_type && cand->cand_type[0] && e_addr) {
				if (!strcasecmp(e_found, cand->foundation) &&
			...
			if (cand->priority <= ice->ice_params->cands[i][ice->proto].priority) {
				insert_at = i + 1;
			}
		}
	...
	if (cand->foundation[0]) {
		ice->ice_params->cands[idx][ice->proto].foundation = switch_core_strdup(rtp_session->pool, cand->foundation);
	}
	
	if (cand->cand_type[0]) {
		ice->ice_params->cands[idx][ice->proto].cand_type = switch_core_strdup(rtp_session->pool, cand->cand_type);
	}
	
	if (cand->transport[0]) {
		ice->ice_params->cands[idx][ice->proto].transport = switch_core_strdup(rtp_session->pool, cand->transport);
	}
	
	if (cand->rel_addr[0]) {
		ice->ice_params->cands[idx][ice->proto].raddr = switch_core_strdup(rtp_session->pool, cand->rel_addr);
	}
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_INFO,
					...
					cand->transport, cand->ip, cand->port,
					(unsigned long)cand->priority, cand->component_id);

		return SWITCH_STATUS_SUCCESS;
#else
	(void)rtp_session; (void)cand; (void)mline_index; (void)mid;
	return SWITCH_STATUS_FALSE;
#endif
}
...
static switch_status_t switch_rtp_internal_end_of_candidates(switch_rtp_t *rtp_session, int mline_index, const char *mid)
{
	...
	return SWITCH_STATUS_SUCCESS;
#else
	(void)rtp_session; (void)mline_index; (void)mid;
	return SWITCH_STATUS_FALSE;
 #endif
}
...
SWITCH_DECLARE(switch_status_t) switch_rtp_add_trickle_remote_candidate(switch_rtp_t *rtp_session, int mline_index, const char *mid, const char *cand_line)
{
	...
	if (!rtp_session || !cand_line) return SWITCH_STATUS_FALSE;
	memset(&cand, 0, sizeof(cand));
	status = parse_cand_line(cand_line, &cand);
	if (status != SWITCH_STATUS_SUCCESS) return SWITCH_STATUS_FALSE;

	if (rtp_session->flags[SWITCH_RTP_FLAG_RTCP_MUX] && cand.component_id != 1) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
			"Trickle ICE: dropping component %d candidate due to RTCP-MUX (mid=%s)\n", cand.component_id, mid ? mid : "(null)");
		return SWITCH_STATUS_SUCCESS;
	}

#ifdef SWITCH_HAVE_ICE
	status = switch_rtp_internal_add_remote_candidate(rtp_session, &cand, mline_index, mid);
	return status;
#else
	(void)mline_index; (void)mid;
	return SWITCH_STATUS_FALSE;
#endif
}
...
{
#ifdef SWITCH_HAVE_ICE
	/* Inform ICE agent no more candidates will arrive for this m-line */
	return switch_rtp_internal_end_of_candidates(rtp_session, mline_index, mid);
#else
	(void)rtp_session; (void)mline_index; (void)mid;
	return SWITCH_STATUS_FALSE;
#endif
...
/* Utility: parse "a=candidate:..." into a struct. */
static switch_status_t get_token(const char *s, int idx, char *out, int outsz)
{
    /* tokens are space-separated after the prefix; idx starting at 0 for foundation */
    const char *p;
    int i, n, len;
    p = s;
    n = 0;
    ...
    for (i = 0; s[i]; ++i) {
        if (s[i] == ' ') {
            if (len > 0) {
                if (n == idx) {
                    int cplen = len < outsz - 1 ? len : outsz - 1;
                    memcpy(out, p, (size_t)cplen);
                    out[cplen] = '\0';
                    return SWITCH_STATUS_SUCCESS;
                }
                n++;
    ...
    if (len > 0 && n == idx) {
        int cplen2 = len < outsz - 1 ? len : outsz - 1;
        memcpy(out, p, (size_t)cplen2);
        out[cplen2] = '\0';
        return SWITCH_STATUS_SUCCESS;
    }
    return SWITCH_STATUS_FALSE;
...
static switch_status_t parse_cand_line(const char *line, switch_rtp_ice_cand_t *out)
{
    /* Accept both "a=candidate:" and raw "candidate:" */
   ...
   const char *s;
    ...
    if (!line || !out) return SWITCH_STATUS_FALSE;
    s = line;
    if (!strncmp(line, "a=candidate:", 12)) s = line + 12;
    else if (!strncmp(line, "candidate:", 10)) s = line + 10;

    memset(out, 0, sizeof(*out));

    ok = (get_token(s, 0, out->foundation, (int)sizeof(out->foundation)) == SWITCH_STATUS_SUCCESS);
    if (!ok) return SWITCH_STATUS_FALSE;
    if (get_token(s, 1, tok, sizeof(tok)) != SWITCH_STATUS_SUCCESS) return SWITCH_STATUS_FALSE;
    out->component_id = atoi(tok);
    if (get_token(s, 2, out->transport, (int)sizeof(out->transport)) != SWITCH_STATUS_SUCCESS) return SWITCH_STATUS_FALSE;
    if (get_token(s, 3, tok, sizeof(tok)) != SWITCH_STATUS_SUCCESS) return SWITCH_STATUS_FALSE;
    prio = strtol(tok, NULL, 10);
    out->priority = (unsigned long)prio;
    if (get_token(s, 4, out->ip, (int)sizeof(out->ip)) != SWITCH_STATUS_SUCCESS) return SWITCH_STATUS_FALSE;
    if (get_token(s, 5, tok, sizeof(tok)) != SWITCH_STATUS_SUCCESS) return SWITCH_STATUS_FALSE;
    port = atoi(tok);
    out->port = port;

    /* Scan the rest for typ/ raddr/ rport */
    {
        const char *typ = strstr(s, " typ ");
        const char *raddr = strstr(s, " raddr ");
        const char *rport = strstr(s, " rport ");
        if (typ) {
            typ += 5;
            get_token(typ, 0, out->cand_type, (int)sizeof(out->cand_type));
        }
        if (raddr) {
            raddr += 7;
            get_token(raddr, 0, out->rel_addr, (int)sizeof(out->rel_addr));
        }
        if (rport) {
            rport += 7;
            if (get_token(rport, 0, tok, sizeof(tok)) == SWITCH_STATUS_SUCCESS) {
                out->rel_port = atoi(tok);
            }
        }
    }
    return SWITCH_STATUS_SUCCESS;
...
SWITCH_DECLARE(switch_bool_t) switch_rtp_trickle_is_registered(switch_rtp_t *rtp_session)
{
	...
	return rtp_session->trickle.cb ? SWITCH_TRUE : SWITCH_FALSE;
}
...
SWITCH_DECLARE(void) switch_rtp_trickle_emit_local_candidate(switch_rtp_t *rtp_session, const switch_rtp_ice_cand_t *cand, const char *mid, int mline_index, int end_of_candidates)
{
	...
	if (!rtp_session) return;
	cb = rtp_session->trickle.cb;
	ud = rtp_session->trickle.user_data;
	if (!cb) return;

	if (cand && trickle_eoc_is_sealed(rtp_session, mid ? mid : "", mline_index)) {
		return;
	}
	...
	if (cand) {
		cb(ud, mid, mline_index, cand, end_of_candidates ? 1 : 0);
...
SWITCH_DECLARE(switch_status_t) switch_rtp_extmap_register(switch_rtp_t *rtp_session, uint8_t id, const char *uri)
{
	...
	if (!rtp_session || !uri || !*uri) return SWITCH_STATUS_FALSE;
	/* store in a tiny fixed map (ids 1..14 commonly used) */
	if (id == 0) return SWITCH_STATUS_FALSE;
	if (id < 16) {
		rtp_session->extmap_uri[id] = switch_core_strdup(rtp_session->pool, uri);
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,
		                  "RTP extmap registered id=%u uri=%s\n", id, uri);
		return SWITCH_STATUS_SUCCESS;
	}
	return SWITCH_STATUS_FALSE;
#else
	(void)rtp_session; (void)id; (void)uri;
	return SWITCH_STATUS_FALSE;
#endif
}
...
{
#if defined(SWITCH_HAVE_RTP) && defined(SWITCH_HAVE_DTLS)
	if (!rtp_session) return;
	/* Use the existing RTP session mutex (opaque to outside) to guard init. */
	switch_mutex_lock(rtp_session->flag_mutex);
	if (!rtp_session->flags[SWITCH_RTP_FLAG_DTLS_OK]) {
	/* Underlying DTLS init (implementation already present in RTP). */
	if (switch_rtp_dtls_init(rtp_session) == SWITCH_STATUS_SUCCESS) {
			rtp_session->flags[SWITCH_RTP_FLAG_DTLS_OK] = 1;
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG, "DTLS initialized\n");
		} else {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING, "DTLS init failed (will retry once later)\n");
		}
	}
	switch_mutex_unlock(rtp_session->flag_mutex);
#else
...
SWITCH_DECLARE(void) switch_rtp_mark_ice_connectivity_failed(switch_rtp_t *rtp_session, const char *reason)
{
	...
	if (!rtp_session) return;
	rtp_session->flags[SWITCH_RTP_FLAG_ICE_FAILED] = 1;
	switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, "rtp::ice-check-failed");
	if (event) {
		switch_channel_t *channel = switch_core_session_get_channel(rtp_session->session);
		if (channel) switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Unique-ID", switch_channel_get_uuid(channel));
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Reason", reason ? reason : "checks_failed");
		switch_event_fire(&event);
	}
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_WARNING, "ICE connectivity checks failed: %s\n", reason ? reason : "n/a");
#else
	(void)rtp_session; (void)reason;
#endif
}

SWITCH_DECLARE(uint32_t) switch_rtp_get_poll_timeout_ms(switch_rtp_t *r)
{
	if (!r) return 0;
#if defined(HAVE_POLL_TIMEOUT_FIELD)
	return r->poll_timeout;
#else
	return 5000;
#endif
}

SWITCH_DECLARE(uint32_t) switch_rtp_get_ms_per_packet(switch_rtp_t *r)
{
	return r ? r->ms_per_packet : 0;
}

SWITCH_DECLARE(uint32_t) switch_rtp_get_us_per_packet(switch_rtp_t *r)
{
	if (!r) return 0;
	if (r->ms_per_packet) {
		return r->ms_per_packet * 1000;
	}
#if defined(HAVE_SAMPLES_PER_INTERVAL) && defined(HAVE_SAMPLES_PER_SECOND)
	if (r->samples_per_interval && r->samples_per_second) {
		return (1000000U * r->samples_per_interval) / r->samples_per_second;
	}
#endif
	return 0;
}

SWITCH_DECLARE(switch_bool_t) switch_rtp_using_timer(switch_rtp_t *r)
{
	return (r && r->timer.timer_interface) ? SWITCH_TRUE : SWITCH_FALSE;
}

SWITCH_DECLARE(void) switch_rtp_prepare_trickle_ice(switch_rtp_t *rtp_session,  ice_proto_t proto, ice_t *ice_params)
{
	switch_rtp_ice_t *ice;
	if (!rtp_session || !ice_params) return;
...
static inline switch_channel_t *cand_get_channel(const switch_rtp_t *rtp_session)
{
	...
	if (!rtp_session || !rtp_session->session) return NULL;
	ch = switch_core_session_get_channel(rtp_session->session);
	return ch;
}

static inline switch_bool_t cand_channel_bool(const switch_rtp_t *rtp_session, const char *var, switch_bool_t dflt)
{
	switch_channel_t *ch;
	const char *v;
	
	ch = cand_get_channel(rtp_session);
	if (!ch) return dflt;
	v = switch_channel_get_variable(ch, var);
	return v ? (switch_true(v) ? SWITCH_TRUE : SWITCH_FALSE) : dflt;
}
...
static inline switch_bool_t ipv4_is_unspecified(const char *ip) { return !strcmp(ip, "0.0.0.0") ? SWITCH_TRUE : SWITCH_FALSE; }
static inline switch_bool_t ipv4_is_loopback(const char *ip)   { return !strncmp(ip, "127.", 4) ? SWITCH_TRUE : SWITCH_FALSE; }
static inline switch_bool_t ipv4_is_linklocal(const char *ip)  { return !strncmp(ip, "169.254.", 8) ? SWITCH_TRUE : SWITCH_FALSE; }
static inline switch_bool_t ipv4_is_multicast(const char *ip)
{
	...
	return (o1 >= 224 && o1 <= 239) ? SWITCH_TRUE : SWITCH_FALSE;
}

static inline switch_bool_t ipv6_eq(const char *ip, const char *lit)
{
	return (ip && lit && !strcasecmp(ip, lit)) ? SWITCH_TRUE : SWITCH_FALSE;
}
static inline switch_bool_t ipv6_has_prefix(const char *ip, const char *prefix)
{
	size_t n = strlen(prefix);

	if (!ip || !prefix) return SWITCH_FALSE;

	return strncasecmp(ip, prefix, n) == 0 ? SWITCH_TRUE : SWITCH_FALSE;
}
static inline switch_bool_t ipv6_is_unspecified(const char *ip) { return ipv6_eq(ip, "::"); }
static inline switch_bool_t ipv6_is_loopback(const char *ip)    { return ipv6_eq(ip, "::1"); }
static inline switch_bool_t ipv6_is_linklocal(const char *ip)   { return ipv6_has_prefix(ip, "fe80:") || ipv6_has_prefix(ip, "FE80:"); } /* fe80::/10 */
static inline switch_bool_t ipv6_is_multicast(const char *ip)   { return ipv6_has_prefix(ip, "ff")   || ipv6_has_prefix(ip, "FF"); }     /* ff00::/8  */
static inline switch_bool_t ipv6_is_ula(const char *ip)         { return ipv6_has_prefix(ip, "fc")   || ipv6_has_prefix(ip, "fd") ||
                                                                          ipv6_has_prefix(ip, "FC")   || ipv6_has_prefix(ip, "FD"); }   /* fc00::/7  */

static switch_bool_t ice_remote_ip_is_acceptable(const switch_rtp_t *rtp_session, const char *ip, const char *transport, int *out_family)
{
	int fam = addr_family_guess(ip);

	switch_bool_t allow_v6 = cand_channel_bool(rtp_session, "rtp_allow_ipv6", SWITCH_TRUE);
	switch_bool_t reject_ula = cand_channel_bool(rtp_session, "rtp_reject_unique_local_v6", SWITCH_FALSE);
	...
	if (fam == AF_INET) {
		if (ipv4_is_unspecified(ip) || ipv4_is_loopback(ip) ||
		    ipv4_is_linklocal(ip)   || ipv4_is_multicast(ip)) {
			return SWITCH_FALSE;
		}
		return SWITCH_TRUE;
	}
	...
	if (fam == AF_INET6) {
		...
		if (ipv6_is_unspecified(ip) || ipv6_is_loopback(ip) ||
		    ipv6_is_linklocal(ip)   || ipv6_is_multicast(ip)) {
			return SWITCH_FALSE;
		}
		if (reject_ula && ipv6_is_ula(ip)) {
			return SWITCH_FALSE;
		}
		return SWITCH_TRUE;
	}
...
static inline switch_log_level_t ice_dbg_level(const switch_rtp_t *rtp_session)
{
	...
	ch = (rtp_session && rtp_session->session) ? switch_core_session_get_channel(rtp_session->session) : NULL;
	v = ch ? switch_channel_get_variable(ch, "rtp_ice_debug") : NULL;

	return switch_true(v) ? SWITCH_LOG_INFO : SWITCH_LOG_DEBUG;
}

static void ice_log_cand(const switch_rtp_t *rtp_session,
                         ...
                         const char *extra_fmt, ...)
{
	...
	lvl = ice_dbg_level(rtp_session);
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session ? rtp_session->session : NULL), lvl,
	                  "Trickle ICE: %s%s mid=%s idx=%d comp=%d %s %s:%d%s%s\n",
	                  action, reason ? " (" : "",
	                  mid ? mid : "(null)", mline_index, component_id, transport ? transport : "-", ip ? ip : "-", port, reason ? ") " : "", reason ? reason : extra);
	if (!reason && extra[0]) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session ? rtp_session->session : NULL), lvl,
		                  "  %s\n", extra);
	}
...
SWITCH_DECLARE(void) switch_rtp_ice_end_of_candidates(switch_rtp_t *rtp_session)
{
	...
	if (!rtp_session->ice.cand_responsive) {
		rtp_session->ice.cand_responsive = 1;
	}

	rtp_session->ice.last_ok = switch_time_now();
	rtp_session->ice.missed_count = 0;
...
SWITCH_DECLARE(const char *) switch_rtp_session_get_recv_mid(switch_rtp_t *rtp_session)
{
	...
	return rtp_session->recv_mid[0] ? rtp_session->recv_mid : NULL;
}
...
SWITCH_DECLARE(rtp_extension_t *) switch_rtp_get_send_ext(switch_rtp_t *rtp_session, uint8_t type)
{
	...
	if (rtp_session->flags[SWITCH_RTP_FLAG_VIDEO] != 0) {
		engine = switch_core_media_get_engine(rtp_session->session, SWITCH_MEDIA_TYPE_VIDEO);
	} else if (rtp_session->flags[SWITCH_RTP_FLAG_TEXT] != 0) {
		engine = switch_core_media_get_engine(rtp_session->session, SWITCH_MEDIA_TYPE_TEXT);
	} else {
		engine = switch_core_media_get_engine(rtp_session->session, SWITCH_MEDIA_TYPE_AUDIO);
	}

	return switch_core_media_get_send_extension_by_type(engine, type);
}

SWITCH_DECLARE(void) switch_rtp_set_rtp_ext_total_size(switch_rtp_t *rtp_session, uint8_t len)
{
	rtp_session->send_rtp_exts_size = len;
}

SWITCH_DECLARE(uint32_t) switch_rtp_get_rtp_ext_total_size(switch_rtp_t *rtp_session)
{
	return rtp_session->send_rtp_exts_size;
}

SWITCH_DECLARE(uint8_t) switch_rtp_get_rtp_ext_recv_total_size(switch_rtp_t *rtp_session)
{
	return rtp_session->recv_rtp_exts_size;
}
