...
static switch_status_t rtp_apply_mid_extension(switch_rtp_t *rtp_session, rtp_msg_t *send_msg, switch_size_t *bytes, uint8_t ext_id, const char *mid, switch_bool_t drop_peer_extensions, switch_size_t trusted_payload_offset);
...
static int rtp_write_manual_state(switch_rtp_t *rtp_session, switch_rtp_write_state_t *write_state,
							  ...
							  switch_payload_t in_payload, uint16_t in_seq, uint32_t in_ssrc);
...
fork_done:
		...
		if (rtp_session->flags[SWITCH_RTP_FLAG_EXT_AUDIO_LEVEL] &&
		...
		}
			{
				...
				switch_status_t mid_status = rtp_apply_mid_extension(rtp_session, send_msg, &bytes, mid_ext_id, mid, drop_peer_extensions, trusted_payload_offset);
...
SWITCH_DECLARE(int) switch_rtp_write_frame_ex_state(switch_rtp_t *rtp_session, switch_frame_t *frame, switch_rtp_write_state_t *write_state, uint32_t ssrc, uint8_t mid_ext_id, const char *mid, switch_bool_t force_video, switch_payload_t payload_override)
{
	...
	if (switch_test_flag(frame, SFF_PROXY_PACKET) || switch_test_flag(frame, SFF_UDPTL_PACKET) ||
		...
		if (!rtp_session->flags[SWITCH_RTP_FLAG_UDPTL] && !switch_test_flag(frame, SFF_UDPTL_PACKET)) {
			...
			send_msg->header.ssrc = htonl(ssrc ? ssrc : rtp_session->ssrc);
			{
				if (rtp_apply_mid_extension(rtp_session, send_msg, &bytes, mid_ext_id, mid, SWITCH_TRUE, trusted_payload_offset) != SWITCH_STATUS_SUCCESS) {
					if (mid_ext_id > 0 && mid && *mid) {
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR,
							  ...
							  switch_payload_t in_payload, uint16_t in_seq, uint32_t in_ssrc)
{
	...
	bytes = rtp_header_len + datalen;
	{
		if (rtp_apply_mid_extension(rtp_session, &rtp_session->write_msg, &bytes, mid_ext_id, mid, (flags && (*flags & (SFF_RAW_RTP | SFF_RAW_RTP_PARSE_FRAME | SFF_EXTERNAL | SFF_PROXY_PACKET))) ? SWITCH_TRUE : SWITCH_FALSE, 0) != SWITCH_STATUS_SUCCESS) {
			if (mid_ext_id > 0 && mid && *mid) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_ERROR,
...
static switch_status_t rtp_apply_mid_extension(switch_rtp_t *rtp_session, rtp_msg_t *send_msg, switch_size_t *bytes, uint8_t ext_id, const char *mid, switch_bool_t drop_peer_extensions, switch_size_t trusted_payload_offset)
{
	...
	if (!(effective_ext_id && !zstr(effective_mid))) {
		...
		if (!effective_ext_id || zstr(mid_snapshot)) {
			return drop_peer_extensions ? rtp_strip_header_extensions(send_msg, bytes, trusted_payload_offset) : SWITCH_STATUS_SUCCESS;
		}
