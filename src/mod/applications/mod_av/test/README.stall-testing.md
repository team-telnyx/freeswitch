...
| `record_write_error_grace_ms` / `RECORD_WRITE_ERROR_GRACE_MS` | how long consecutive write failures are tolerated before the recording is abandoned. Defaults to 1000ms rather than off, because the alternative is retrying a dead destination on every frame. 0 never gives up, which is the behaviour before this setting existed; `RECORD_STOP_WRITE_ON_ERROR` gives up on the first failure |
