...
SWITCH_STANDARD_APP(stop_dtmf_session_generate_function)
{
	...
	/* Stop DTMF generation */
	switch_ivr_stop_inband_dtmf_generate_session(session);
	
	/* Also stop any ringback that might be playing */
	switch_ivr_broadcast(uuid, NULL, SMF_NONE);
	
	/* Clear the api_on_answer variable if it was set for ringback */
	if (switch_channel_get_variable(channel, "api_on_answer")) {
		const char *api_cmd = switch_channel_get_variable(channel, "api_on_answer");
		if (api_cmd && strstr(api_cmd, "uuid_break")) {
			switch_channel_set_variable(channel, "api_on_answer", NULL);
		}
	}
}
