...
void sofia_glue_attach_private(switch_core_session_t *session, sofia_profile_t *profile, private_object_t *tech_pvt, const char *channame)
{
	...
	if (tech_pvt->sofia_private && !zstr(tech_pvt->sofia_private->rtcp_audio_interval_msec))
	...
	if (tech_pvt->sofia_private && !zstr(tech_pvt->sofia_private->rtcp_audio_passthru_timeout_msec))
		tech_pvt->mparams.rtcp_audio_passthru_timeout_msec = tech_pvt->sofia_private->rtcp_audio_passthru_timeout_msec;

	if (tech_pvt->sofia_private && !zstr(tech_pvt->sofia_private->partner_rtcp_audio_passthru_timeout_msec))
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->rtp_media_timeout_msec > 0)
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->partner_rtp_media_timeout_msec > 0)
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->confirm_blind_transfer == SWITCH_TRUE)
		switch_channel_set_state_flag(tech_pvt->channel, CF_CONFIRM_BLIND_TRANSFER);

	if (tech_pvt->sofia_private && tech_pvt->sofia_private->bridge_accept_cng == SWITCH_TRUE)
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->partner_bridge_accept_cng == SWITCH_TRUE)
	...
	if (tech_pvt->sofia_private && !zstr(tech_pvt->sofia_private->bridge_forward_cng_interval))
	...
	if (tech_pvt->sofia_private && !zstr(tech_pvt->sofia_private->partner_bridge_forward_cng_interval))
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->bridge_forward_cng_once == SWITCH_TRUE)
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->partner_bridge_forward_cng_once == SWITCH_TRUE)
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->force_rtcp_passthru == SWITCH_TRUE)
	...
	if (tech_pvt->sofia_private && tech_pvt->sofia_private->partner_force_rtcp_passthru == SWITCH_TRUE)
...
switch_status_t sofia_glue_do_invite(switch_core_session_t *session)
{
	...
	const char *recover_direction = NULL;
	const char *recover_outgoing_contact = NULL;
	int require_timer = 1;
	uint8_t is_t38 = 0;
	...
	char call_id_uuid_str[SWITCH_UUID_FORMATTED_LENGTH + 1];
	...
	if (switch_channel_test_flag(tech_pvt->channel, CF_RECOVERING)) {
		...
		tt = switch_channel_get_variable(tech_pvt->channel, "sip_invite_transaction_type");
		if (tt && strcmp(tt, "uas") == 0) {
			recover_direction = "X-FS-SESSION-RECOVERY: uas";
			
		} else if (tt && strcmp(tt, "uac") == 0) {
			recover_direction = "X-FS-SESSION-RECOVERY: uac";
		}
		...
		recover_outgoing_contact = switch_channel_get_variable(tech_pvt->channel, "sip_outgoing_contact_uri");
	}
	...
	if ((val = switch_channel_get_variable(channel, "sip_identity")) || (val = switch_channel_get_variable(channel, "sip_h_identity"))) {
		identity = switch_core_session_strdup(session, val);
	}
	if ((val = switch_channel_get_variable(channel, "sip_identity_div")) || (val = switch_channel_get_variable(channel, "sip_h_identity_div"))) {
	...
	if (!tech_pvt->nh) {
		...
		if (zstr(tech_pvt->invite_contact)) {
			if (!zstr(recover_outgoing_contact)) {
				tech_pvt->invite_contact = switch_core_session_sprintf(session, "%s", recover_outgoing_contact);
			} else {
				const char *contact;
		...
		*/
		...
		if (!(call_id = switch_channel_get_variable(channel, "sip_invite_call_id"))) {
			if (sofia_test_pflag(tech_pvt->profile, PFLAG_UUID_AS_CALLID)) {
				...
				switch_channel_set_variable(channel, "sip_call_id", call_id);
			}
		}
		
		if (zstr(call_id)) {
			switch_uuid_str(call_id_uuid_str, sizeof(call_id_uuid_str));
			call_id = call_id_uuid_str;
			switch_channel_set_variable(channel, "sip_call_id", call_id);
		}
	...
	if ((val = switch_channel_get_variable(channel, "bleg_enable_3pcc"))) {
		if (switch_true(val)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(tech_pvt->session), SWITCH_LOG_INFO, "3PCC support forced by dialplan for %s\n",  switch_channel_get_name(channel));
	...
	prometheus_increment_outgoing_invite();
	...
	if (sofia_use_soa(tech_pvt)) {
		nua_invite(tech_pvt->nh,
				   ...
				   TAG_IF(!zstr(recover_direction), SIPTAG_HEADER_STR(recover_direction)),
		...
		nua_invite(tech_pvt->nh,
				   ...
				   TAG_IF(mp_payload, SIPTAG_PAYLOAD(mp_payload)),
				   TAG_IF(!mp_payload && !zstr(tech_pvt->mparams.local_sdp_str), SIPTAG_PAYLOAD_STR(tech_pvt->mparams.local_sdp_str)),
				   TAG_IF(rep, SIPTAG_REPLACES_STR(rep)), 
				   SOATAG_HOLD(holdstr), 
				   TAG_END());
	}
